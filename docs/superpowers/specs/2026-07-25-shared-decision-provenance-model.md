...
**`Source.expiry` 的 exact authority（v1.15 draft）** —— 上表只寫「`expiry: 期限`」，沒有格式、時區與比較語義。同一份 `exception-grant` 因此可以被判成已過期，也可以被判成未過期，而下游的 `expiredClauses` 完全以它為準。本版收斂為唯一規則：
...
處置     若日後出現任何 persisted Source.expiry 不合本節 grammar:
         **一律 fail-closed**，錯誤必須指名該 sourceId 與該欄位;
         必須由**明示的 correction 或 migration transaction** 處理。
         producer／reader **不得**寬鬆解析、**不得**補零、**不得**推測時區、
         **不得** silently normalize、**不得**先當作可用再事後修正。
         Source 為 immutable（INV-3），修正**只能**走既有 migration 路徑,
         **不得**原地改寫。
實作義務  promotion 之前，既有 writer 的 expiry 檢查仍是既有實作,
         本節**不**宣稱它已符合此 grammar；收緊是 promotion 之後的
         implementation obligation，**不是**已完成的事實。
```
...
### v1.15 lifecycle seed derivation（draft）—— `lifecycleAffectedClauses` 的唯一推導
...
**v1.15 acceptance matrix（draft；executable）** —— 本輪 spec-only，**不代表下列任何一格已有測試、已實作或已通過**。
