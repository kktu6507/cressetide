...
74. **條件 1（dependent membership）獨立成立**：與 AC75／77／78 同構的**單一變數**反例，payload **完全合法且只用已宣告欄位**（不得倚賴 caller 填寫 `status`／`priorTerminalRef`／`reopenedBy` —— 那三者是 writer-derived post-state，嚴格實作會先在 unknown-field／shape guard 拒絕，本 AC 就證不到 membership）。pre-state DP 的 current terminal **不是**本交易 Transition.subject，因此它不是 subject-dependent；其餘來源 2 條件在**可適用範圍內全部對位**（該 DP pre-state carrier 非 null，且對它宣告 `clear`），post-state 一律由 writer 依 canonical pre-state 與 closure 推導 → fail-closed。驗收必須確認拒絕來自**條件 1**，不得由 shape、carrier（條件 4）、applicability（條件 2）、prior（條件 7）或 trigger（條件 8）任一 guard 提前拒絕。
75. **條件 2（successor 不 applicable）獨立成立**：與 AC77／78 同樣是**單一變數**反例 —— DP 確實是 subject-dependent、dependent closure 確實處理該 DP、**pre-state carrier 非 null**，其餘條件（post-state open、terminal refs 皆空、`priorTerminalRef` 對位、`reopenedBy` 的 semantic member 與 v1 序列化對位、carrier 為 null、明示 `clear`）**全部對位**，唯一翻轉的是 successor 對該 DP **確實 applicable** → fail-closed（它應被 repoint，正確宣告是 `preserve`／`replace`／`unchanged-null` 之一）。驗收必須確認拒絕來自條件 2，**不得**由 carrier（條件 4）、shape、membership（條件 1）、prior（條件 7）或 trigger（條件 8）任一 guard 提前拒絕 —— 尤其不得讓 fixture 的 pre-state carrier 為 null 而先在條件 4 失敗。
