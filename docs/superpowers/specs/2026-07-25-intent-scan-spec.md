...
93. **規格放行不等於實作放行**（carrier：本文件 live status）：v1.10 已於 **2026-08-10 經使用者明確核准**（前置 draft 內容經獨立 Codex 審查並 ACCEPT；**本次未執行 agent-duel panel**），前一 approved 版本為 **v1.9**。**此 approval 只涵蓋 spec authority。**現行 `provenance-store.mjs` **仍只檢查 `REQ-`／`DEC-`／`ASSUM-` 前綴，從不驗 ULID body**，本輪亦未改動任何程式；因此**不得**因 promotion 宣稱 store validator 已實作 canonical ULID validation，**不得**宣稱 migration、implementation 或 Phase 2 READY。spec authority 生效**不等於**實作授權：**任何實作仍需另有對應的授權與驗證。**
