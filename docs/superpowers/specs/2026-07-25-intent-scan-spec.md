...
- 姊妹 spec：`2026-07-25-test-provenance-spec.md`（**現行生效契約：approved v1.4**；另有 **draft v1.5 — 待 panel**：隨上游 `provenanceVersion` 升版，其 canonical empty store 與 historical base-tree 的 read-only 版本行為**確有語義變更**，AC60 已補 historical v1／v2 分支）。§8 的 provenance store 與 store script 是兩者共用的 shared infrastructure，test-provenance spec 消費、不重定義。
