...
上列兩條 observable invariant 是**新增的 reader 義務**：**在 v1.14 promotion 當下**，當時已接受的 canonical v2 reader 尚未驗證它們，因此仍需另行 remediation 與獨立審查；**該 promotion-time boundary 不因後續實作而倒寫**。**promotion 之後**（status，非 normative；截至 2026-08-16）：commit `9f42628` 已實作這兩條 invariant、補齊 AC169 (16)–(20) evidence，並通過獨立審查 ACCEPT。這是後續 implementation acceptance，不是本節或 v1.14 promotion 自動修改了程式。
...
closed enums（v1）:
  language         : "javascript"
  framework        : "node:test"
  implementationId : "node-test-v2"   ← **v1.16（approved）**：`node-test-v1` 為歷史 identity，
                                        **不得**出現在 current mapping（見 §11b.8b）
...
|---|---|---|---|---|---|---|
| `node-test-v2` | `node:test` | `javascript` | 恰為 `["node-test-call"]` | 恰為 `["node-test-describe"]` | `leading-line-comments` | `line-comment-tid-v1` |

**任何其他 `framework` 搭配 `node-test-v2` → fail-closed**；**v1.16（approved）**：`node-test-v1` 是歷史 identity，出現在 current registry 中即 **fail-closed**；表中未列的欄位組合亦 fail-closed。每一欄都是**恰為**，不是「子集」或「可為空」—— v1 只有一個合法組合，registry 沒有自由度。
...
discovery（exact key set）:
  ...
  manifestDependencies : 字串陣列（canonical form；node-test-v2 綁定為 []）
  importSpecifiers     : 字串陣列（canonical form）

node-test-v2 的綁定值（closed；v1.16 approved —— 值不變，綁定的 identity 改變）:
...
**沒有 file-pattern 這一級（v1.6 第三輪修正；v1.12 收斂結果）** —— discovery 只有上列三級，**副檔名不參與 adapter 選定**。反例：一個 `.test.mjs` 檔只 import Playwright／Vitest 風格的 API 而**沒有** `node:test` 的 static import → 三級皆無 evidence → **`not-a-candidate`**（v1.12：**不是** whole-run fail-closed，見下方 §11b.4c），**不得**因副檔名而被選成 `node-test-v2`。
...
### 11b.8b Canonical declaration bytes 與 directive 排除（exact；v1.16 起含 `@src`）
...
hashing 之前   : 自該 range 逐行掃描，移除**合法且已附著**的 directive 行 ——
                 **v1.16 起為 @tid 與 @src 兩者**，以**同一個**演算法、
                 同一次掃描處理，連同其（已為 LF 的）行終止符；
                 檔尾最後一行則無終止符可移。移除後**不得留下任何 CR**。
                 —— **不得**為 @src 另立第二個演算法、第二個 range 或第二個 digest。
其餘           : 其他註解（含恰好提到 @src／@tid 的一般註解）、declaration bytes
                 一律保留，之後套用 §2 既有 canonical 規則（不 trim 內部空白）
不得移除       : malformed @tid／@src（不符 grammar／行內有其他內容）
                 unattached @tid／@src（與 declaration 之間有空白行）
                 —— 兩者本來就已 fail-closed，不會走到 hashing
                 —— 「合法且已附著」的判準沿用 §11b.8c 的 directive-intent
                    predicate 與 attachment block，**不在此重複、不在此放寬**
```

**v1.16 的直接後果（推導，非另立規則）** —— 移除合法附著的 `@src` 行之後：

```
tag-only change : base 與 head 的 canonical declaration bytes **完全相同**
                  ⇒ 兩側 bodyDigest **相等**
                  ⇒ §6 precedence 第 4 格（modified）**不**命中
                  ⇒ 第 5 格（retagged）**成為可達**
影響範圍         : 這**修復**的是 AC169 (2) 與 AC173 (a)(c) 的**可達性**，
                  **不改變**該兩條 AC 的任何 case、expected result 或 error boundary，
                  **也不改變** §6 precedence 的順序或任何一格的判準。
implementation
identity 必須升版  : **這是新的 digest 演算法，因此必須是新的 implementation identity。**
                  §11b.9b 的 `implementationIdentity.implementationId` **必須**由
                  `node-test-v1` 升為 **`node-test-v2`** —— **v1.16 的唯一 current
                  mapping，不再接受「或另一個新 identity」的說法：一個未釘死的
                  identity 名稱本身就是第二個 writer 分歧的入口。
                  §11b.3 的 **live closed enum、exact binding table 與 discovery
                  binding authority 三處同步閉合 `node-test-v2`**，`registryDigest`
                  因此也隨之改變。
                  —— `node-test-v1` **只**保留為**歷史資料的 identity**，
                     **不得**留在 current registry mapping 中。
                  —— **不得**讓兩套 digest 演算法共用 `node-test-v1`：
                     identity 存在的意義就是「同一個 identity ⇒ 同一份 bytes ⇒
                     同一個 digest」。若舊 writer 與新 writer 掛同一個 identity 卻
                     算出不同 digest，identity 就不再能證明任何事，而錯誤會以
                     「digest 不符」的面貌出現在與它無關的地方。
digest 相容性     : `bodyDigest`／`declarationDigest` 的**公式不變**，改變的是
                  **輸入 bytes** 與 **implementation identity**。
                  既有以 @src 入 digest 產生的 persisted inventory **可能存在**，
                  本版**不**宣稱不存在、**不**捏造 migration:
                    歷史 bytes    : 仍可作為**歷史**文件由 **canonical v2 reader**
                                   （`parseCanonicalInventoryV2`）解析 —— 它們是**舊演算法
                                   產生的 v2 文件**，**不是** legacy v1 envelope，
                                   **不得**稱之為 legacy-v1 reader 路徑
                    current proof : 在新 registry 之下**不得**作為 current
                                   freshness proof —— 其 `registryDigest` 與
                                   `implementationIdentity` 皆已不符，
                                   **必須**經既有 S3／registry mismatch 路徑
                                   **拒絕**並**重建**
                  —— 這不是資料遺失，是**正確的失效**：以舊演算法算出的 digest
                     本來就不能證明新演算法下的任何事。
禁止的替代做法    : **不得**以 producer 特例、第二份 digest、
                  tag-aware 比較或分類時的 ad-hoc 例外達成同一效果 ——
                  那會讓「兩個 body digest 相等」成為謊報而非事實。
```
...
### 11b.10c Populated inventory producer input carrier 與 governance-closure boundary（v1.15；approved）

本節定義**兩個 logical operation** 與其回傳形狀，並把 producer 取得與綁定輸入的方式收斂成唯一規則。`GovernanceSeedPreimage` 與 §11b.10b 的 `DiscoveryAnalysisPreimage` 同屬**純 in-memory** 契約：**不是** persisted artifact、**不**新增任何 inventory 欄位、**不**是第二份 governance authority，也**不**改寫 §6 的 matching 或 classification。

**為何需要本節** —— §11b.10b 的 root **exact shape 恰為** `{ baseTreeOid, headViewDigest, registryDigest, baseModules, headModules }`：其中**沒有** `inputProvenanceStoreDigest`，**也沒有**任何 governance seed。但 v2 envelope（§2／§11b.9c）**必須**帶 `inputProvenanceStoreDigest`，§6 ordered table 的第 6 格**必須**求值 `governanceHit`。producer 因此還需要第二份輸入，而「它由誰觀測、在哪一個瞬間觀測、與 discovery 那一份如何綁定」，既有 approved 文字**從未**決定。兩個都說得通的 producer 會讓 envelope 宣告的 freshness carrier 與實際據以分類的 preimage 來自**不同瞬間** —— §11b.9c 的 preimage-persistence 修正擋不到它：那一段管的是 record 寫入端，不是 inventory 產生端。

**兩個 logical operation 的 public request（exact；anti-injection）**

```
buildGovernanceSeedPreimage({ repoRoot, baseTreeOid })
produceChangedTestInventoryV2({ repoRoot, baseTreeOid, taskId })

兩者各**恰收一個 argument**。request 的 exact key set **兩者不同**，這是 **v1.17 的更正**:

  buildGovernanceSeedPreimage   : exact key set **恰為** { repoRoot, baseTreeOid }
  produceChangedTestInventoryV2 : exact key set **恰為** { repoRoot, baseTreeOid, taskId }

  —— **v1.16 曾**令兩者皆收 taskId 並皆對 G1 解析。**該規則撤回。** AC171 (B)(ix)／(xi)
     同時要求 current store **缺席**或為 canonical empty 時仍須成功（缺席 → canonical
     empty v2 digest），而空 store **沒有任何 TaskState** —— 兩項要求不可能同時成立。
     **實測**：缺席與 canonical-empty 兩案皆停在 unknown-task fail-closed，AC171 的
     該幾格經 public API **不可達**。
  —— 判準是「誰真的需要它」：shared v1.15 §9 的四集合 seed derivation **完全不需要**
     currentTaskDpIds；需要它的**只有** §7 的 REQ@DP 綁定，而那是 producer 的職責。
     taskId 因此屬於 producer 的 request，**不屬於** governance seed 的 request。
  —— taskId 仍**不是** injection：它不供給任何被觀測的輸入，只指名要對哪一個 task
     求值。producer **必須**說出它 —— 一個 store 可持有**多個** TaskState，
     從多個之中猜一個正是本模型別處一律 fail-closed 的那種推測。
  —— taskId 的解析（exact；**只**由 producer 執行，對 producer 自己那**一次** G1
     求值，**不得**另讀）:
       G1 的 taskStates 中**恰有一個** taskId 相符 → 採用它
       零個相符（unknown task）                     → **fail-closed**
       多於一個相符                                  → **fail-closed**（store 自身不一致）
       採用的 TaskState 之
         baseProvenance.treeOid != request.baseTreeOid → **fail-closed**
       —— **不得**在 baseProvenance 與 request 不符時偏袒任一方，
          **不得**因 store 只有一個 TaskState 就略過 taskId 或略過上述比對，
          **不得**由 DP 反推、由 head view 反推或以任何啟發式挑選 current task。
     採用的 TaskState 的 currentTaskDpIds 即 §7 所稱的 currentTaskDpIds。
  —— **buildGovernanceSeedPreimage() 不做上述任何一步**：它不收 taskId、不解析 task、
     **不選定也不使用任何 TaskState**，因此 current store 缺席或為 canonical empty 時
     它**仍必須成功** —— AC171 (B)(ix)／(xi) 因此回到可達。
     **v1.17 修訂 2**：前一稿在此寫「**不讀 taskStates**」，該寫法**撤回** ——
     它與「G1／G2 必須做完整 schema／invariant validation」自相矛盾。
     正確界線是：**seed derivation 不以 TaskState 為輸入或條件**，
     但 store validation **仍涵蓋整份 store，含 `taskStates`**。
  —— taskId **不進** GovernanceSeedPreimage 的四欄 carrier（仍**恰四欄**），
     **不進** ChangedTestInventoryV2 的七欄 envelope（仍**恰七欄**），
     **不** persisted、**不**進任何 digest 的 preimage。
下列 anti-injection 清單**對兩個 operation 同時適用**；對 buildGovernanceSeedPreimage
而言，**taskId 本身也在禁止之列** —— 它的 exact key set 恰為兩欄。
**不得**接受第二個 argument，**不得**接受下列任何一項
（多一 key 即 fail-closed；**不得**忽略未知 key，**不得**以 environment variable 當替代入口）:

  DiscoveryAnalysisPreimage —— 含其任一部分（baseModules／headModules／declarations）
  GovernanceSeedPreimage   —— 含其任一部分
  registry／registry path／registry root
  parser、ignore matcher
  Git executable、Git 參數或 environment
  filesystem adapter
  config object 或 config path
  modulePaths（或任何等價的 caller 提供 candidate 清單）
  prebuilt view／HeadViewSnapshot／AdapterContentView
  provenance store bytes、已 parsed 的 store、store digest
  lifecycleAffectedClauses、governanceHit set、reverse-closure 結果
  matcher 結果、pairs、entries、inventoryDigest
  clock、capture hook、component module path、output path
  上述任一項的等價 override、alias、test-only seam 或 environment argument
```

**理由（不是風格偏好）** —— caller 只要能提供上列任一值，就能讓 envelope 的 freshness carrier 與實際用於分類的 preimage **彼此分離**：送進一個舊的 store digest 搭配當下的 declarations，產生的 envelope 會宣告一個它從未據以分類的 pre-state，而該 envelope 之後會**原樣**成為 `batchSnapshot.inventorySnapshot`（§11b.9c）這個 committed witness。把注入點開在 producer 入口，等於在整條防線的**上游**把它繞過去。

**`GovernanceSeedPreimage` 的 exact carrier**
...
```
{
  ...
  headViewDigest,
  inputProvenanceStoreDigest,
  lifecycleAffectedClauses
}

exact key set 恰為上列四欄 —— 多一 key、缺一 key，或輸出任何自由格式 metadata
  一律 fail-closed。
baseTreeOid                : 沿用 shared v1.14 的 OID carrier 規則與拼法
headViewDigest             : §11b.9c／§11b.10 的既有公式與拼法，**不**重新定義
inputProvenanceStoreDigest : §11b.9c 的既有記法，**不**重新定義（見下 G1）
lifecycleAffectedClauses   : canonical clauseRef array
    每一項必須吻合 §2 既有 canonical clauseRef grammar
      —— ("REQ" | "DEC" | "ASSUM") "-" ULID；ULID 的 authority 仍在 intent-scan §8
    依 Unicode code point **嚴格遞增**
    **不得**重複 —— 相等即 fail-closed，**不是**去重
    **不得**夾帶 dpRef、path、free text 或任何第二欄

immutability : root 與所有 nested value **深度 frozen**。
persistence  : 本 carrier **只存在於記憶體** —— 不落盤、不進 inventory、
               不新增任何欄位，**也不是**第二份 governance authority。
```

**Store capture 與 seed binding（唯一規則）**

```
 1. 自固定路徑 .ctide/provenance.json **fresh-load** current store，稱為 **G1**
    （即 shared approved v1.15 §9 的 `C`）。
      —— 該 path 固定，**不得**由 caller 指定、覆寫，或以 environment 改寫。
 2. **同一次讀取所得的同一份 parsed store** 必須同時供:
      a. schema／invariant validation
      b. inputProvenanceStoreDigest
      c. lifecycleAffectedClauses 的導出
    **不得**在三者之間再讀第二次 —— 那正是 §11b.9c 對 registry 所禁止的
    同一種 TOCTOU 窗口，本條是 store 側的同一條規則。
 3. **Current-store version boundary**（沿用 shared 既有三分表，本文件**不放寬**）:
      G1 缺席          → 採 shared §2 的**唯一** canonical empty store 定義
                         （provenanceVersion 2；本文件**不重寫 literal**）
      G1 存在          → **必須**是 current provenanceVersion **2**
      G1 為 current v1 → **fail-closed** —— 必須先另走**既有** migration transaction；
                         producer **不得** migration、**不得**回寫
      其他版本值        → **fail-closed**
 4. **T0 取樣**：G1 完成 parse／schema validation **之後**、第一個 time-dependent
    判定（exception applicability、expiry）**之前**，自 host **UTC wall clock**
    **恰取樣一次**。本次 invocation 的**所有** time-dependent 判定一律使用**同一個** T0；
    取樣之後 system clock 前跳或後跳，**不改變**本次結果。
      —— **Node implementation boundary**（上游 shared 只寫 host UTC wall clock，
         這一層由本文件負責）：producer invocation 以 `Date.now()` 取得 T0，
         **恰一次**；其回傳的 **epoch milliseconds 本身就是 timezone-independent
         instant**，因此**不需**、**也不得**再做任何時區轉換或本地化。
      —— **不得**在 module import 時預先取樣或快取 T0 —— import-time 取樣會讓
         同一份已載入 module 的**所有**後續 invocation 共用一個早已過時的 instant，
         而那正是 expiry boundary 最容易被靜默跨過的地方。
      —— public request 仍**不得**接受 clock、now、timestamp、Date provider
         或任何 alias／test seam（見上 anti-injection）。
      —— T0 **不** persisted：carrier 仍恰四欄，envelope 仍恰七欄。
 5. digest 記法沿用 §11b.9c 的 provenance-store CAS 記法
    —— sha256(canonicalText(檔案文字))；**缺檔**取既有 canonical empty store digest
    （**不是** null、不是空字串）。**不得**改用 baseProvenance.storeDigest 的 raw-bytes 記法。
 6. **Base-store version boundary**：base governance state（shared approved v1.15 §9 的 `B`）
    **只能**來自 baseTreeOid 指向的 immutable Git tree。
      B 缺席        → 採**同一份** canonical empty store 定義
      B 存在        → provenanceVersion **1 或 2 皆可 read-only 分析**；
                      **不遷移、不回寫**，**不得**拿 normalized bytes 冒充 raw base witness
      B 其他版本值   → **fail-closed**
      —— historical base v1 **不得**被誤套 current-v1 的 migration-only 限制：
         那條限制只適用於 **current mutable** store。
    **不得**以 live current store 冒充 base —— 那會讓「本次是否變更」在同一份輸入上
    自我比較，得到恆為空的 lifecycleAffectedClauses。
      —— .ctide/provenance.json 是 §11b.10 判定順序第 1 步的 hard exclusion，
         因此 head 側 store 本來就不在 head view 內，只能由 G1 承載；
         兩者互補，**不**衝突。
 6b. **B 的驗證邊界（v1.17 新增；下游實作澄清，上游 normative clause 一字不動。
     **v1.17 修訂 3**：前一稿把邊界寫成「只驗 clock-free」，又以 cross-snapshot
     immutability 自證「B 的內容已由 C 驗過」——**兩者皆錯，一併撤回**）**:
     **被排除的恰恰只有「對本次 T0／H 求值」這一件事** ——**不是**整個
     `mechanicallyApplicable`、**不是** INV-4、**更不是** DP 完整性。
     **歷史驗證絕不可退化成 schema-only。**

     **兩個實測控制組（非推測；一正一負，必須同時成立）**:
       正例（**必須通過**）: B 的 exception-backed DP 在**其當時**完全合法、
         該 exception **現已過期**，而 C 已把該 DP 修回 active hard constraint
         且 C 自身通過完整驗證 → 目前實作以 `validateAll(base.store, { now: T0 })`
         得 **`E_INV4_NOT_APPLICABLE`（exception-expired）**，
         **擋掉上游要鼓勵的修復**（shared §9 state matrix：「已過期
         exception-backed REQ → 替換的 clause 或 `null`」判 **通過**；
         同節「前態是歷史事實」）。
       負例（**必須持續被拒**）: B 的 exception terminal 其 expiry 在**未來**（2099）
         卻**缺 `scopeRulingRef`** —— 它**通過** `validateStoreSchema()`，
         但在 applicability 檢查以 **`E_INV4_NOT_APPLICABLE`（scope-ruling-missing）**
         被拒；同一形狀在 C 修好之後合法。**把歷史驗證換成「只跑 schema」
         會讓這個非時間性的無效 B 通過 —— 那是 regression，明文禁止。**

     B **必須**受下列驗證（**一項不得省**）:
       raw base witness       : 見下方「raw base witness 的歸屬」（依 operation 分開）
       version boundary       : provenanceVersion 1 或 2 皆可 **read-only** 分析，
                                其餘 fail-closed；v1 以 read-only legacy decoder 讀
       版本自有語法           : **每一個歷史版本自己的欄位與 expiry grammar 原樣適用**
                                —— **不得**以 current 版本的語法覆蓋歷史版本
       schema／structural     : 型別、必填欄位、canonical 形狀
       identity               : typed ID 唯一、無重複
       reference              : 可解析性（dangling ref → fail-closed）
       non-temporal
       applicability          : `status(c)`（依生效 Transition 導出）、per-kind
                                `mechanicallyApplicable` 的**全部非時間肢** ——
                                REQ 的 **Check A** 與 exception chain 的
                                targetConstraintRef 可解析／target authority／
                                grantAuthorityRef == ownerRef；DEC／ASSUM 的
                                principal 合法、derivedFrom 可解析、basisRefs 可解析
       DP 專屬檢查            : terminal refs 三者互斥、status 與 terminal ref 型別
                                一致、**historical terminal coherence**、
                                **完整既有的 `scopeCovers` predicate**、
                                carrier coherence、reopen cause coherence
                                —— **全部保留**
                                —— **`scopeCovers` 取既有 predicate 的全部**（v1.17
                                   promotion 的 authorized precision correction）:
                                   `scopeRulingRef` 可解析 ∧ **`rulingKind` 為既有
                                   要求的 typed ruling** ∧ intent principal ∧
                                   `record.subjectRef == 該 DP` ∧ **`scopeCovers == true`**
                                   —— 上列括號**是舉例，不是窮舉替代**；
                                   **不得**只取其中一部分當作「機械肢」而漏掉其餘
       cross-snapshot         : B 的**每一個** immutable object 必須在 C 中以同一
                                typed ID 存在，且 canonical typed value **exact equal**

     B **不得**被課的，**恰為**下列「對**本次** T0／H 求值」的項目（**closed set**）:
       exception `expiryInstant` 與 **T0** 的比較，以及**僅由該比較**導出的
         `mechanicallyApplicable` false、`applicable` false、INV-4 否決，
         與「因已過期而必須 repoint／reopen」的連帶要求
       **Check B**（對 **H** 的 live-current occurrence）
       任何其他以**現在時刻**或 **H** 為輸入的判定
     —— expiry 欄位**本身仍必須**依**該歷史版本**的 exact grammar 解析合法；
        被排除的**只有**「與某個 instant 比大小」這一步。
     —— **不得**以 `now = 0`、`now = B 的某個欄位`、任何 fabricated timestamp
        或改寫 expiry 資料來規避本條。

     **證明邊界（v1.17 promotion 的 authorized precision correction；必須誠實陳述）**:
       本處**不** persist 任何歷史求值 instant，因此本驗證**不**證明、**也不得**被
         宣稱為「某個 grant 在過去某個 commit 時點為未過期」的 attestation。
       它保證的**只有**：B 的**非時間性**規則全部成立，違反者一律被拒。
       **兩個方向都不得被時間假設污染**：
         INV-4 要求 terminal **applicable**（正向）；
         source-2 **reopen cause** 驗證要求 successor **不 applicable**（否定向）。
         把未知的歷史 expiry 當成無條件 **`unexpired == true`**，會在否定向那一側
         **錯誤地拒絕**一個「正由 expiry 解釋」的合法歷史 reopen。
       因此實作**必須**把「expiry 相對某 instant 的真值」視為**未知**並**跳過該一肢**，
         **兩個方向一致**，其餘非時間性條件**照常**逐項判定 ——
         **不得**以任一常數真值替代未知，**不得**曝露 clock override。

     **為什麼這不是放寬（正確版本；前一稿的理由撤回）**:
       排除面是上列 **closed set**，其餘歷史規則**全部**照驗，因此一份在**當時**
         就不合法的 B **仍然**被拒（見上方負例）。
       **不得**再主張「B 的 immutable 內容已由 C 的驗證涵蓋」——**該論證錯誤**：
         C 的 DP **可能已不再指向** B 的那個 terminal，因此 C 的驗證
         **無法**替 B 的 DP 專屬檢查背書。

     **實作邊界（不得藉本條弱化任何現行 validator）**:
       得另立一個 **version-aware historical validator**，與現行驗證器共用
         **module-private primitive**；
       但 `validateAll`、`validateLegacyV1` 與所有 transaction 路徑的
         **current 語義一字不變**、**不得**因本條而放寬；
       B **不得**被 migration、normalization 或回寫。

     **審查邊界（誠實記錄）**:
       本條**只**規定 producer 自己怎麼讀 B，shared approved v1.15 §9 的 normative
       clause **一字未改**；上列排除面**恰好**是上游「前態不受現時效力課責」
       （§9 綁定兩相、檢查分層 preChangeBinding 列、state matrix「過期只擋後態、
       不擋前態」）的直接後果，其餘歷史不變量**全部保留**。
       **本條記為 Codex 獨立複審裁定的下游實作澄清**，**不是**上游 normative 修訂。

     **raw base witness 的歸屬（v1.17 修訂 3；兩個 public operation 分開）**:
       `buildGovernanceSeedPreimage`（**恰兩欄** request）:
         **沒有** taskId、**沒有** caller 提供的 baseProvenance witness，
         因此它**不得**選定任何 TaskState，**也不可能**做 witness 比對；
         B **只**由 `baseTreeOid` 這個 **content-addressed exact tree** 取得
         —— tree oid 本身即內容位址，B 的身分由它自證。
         current store **缺席或為 canonical empty 時它仍必須成功**
         （AC171 (B)(ix)／(xi)）。
       `produceChangedTestInventoryV2`（**恰三欄** request）:
         它**已**選定一個 TaskState，而該 TaskState **本來就**攜帶
         `baseProvenance`（canonical storePath、treeOid、storeDigest）。
         因此**在拿 B 去驗任何 binding 之前**，**必須**把該 witness 與**實際捕捉到的
         B** 逐項比對：storePath 為 runtime contract 的 canonical store path、
         `treeOid == request.baseTreeOid`、**storeDigest 對 B 檔的原始 bytes**；
         **B 缺席**時取既有 canonical empty store 的 digest。任一不符 → **fail-closed**，
         **不得**偏袒任一方。
         —— 這**不新增** public argument、**不新增** carrier 或 envelope 欄位：
            witness 本來就在 store 內，比對只是既有輸入之間的一致性檢查。
       **兩種 digest 記法不得互相冒充**:
         B 的 witness digest 取 **B 檔在該 tree 中的原始 bytes**（上游明文：
           驗**原始** bytes 的 storeDigest，且**不得**拿 normalized 後的 bytes
           與 raw digest 比對）；
         G1／G2 的 CAS digest 仍是 step 5 的 `sha256(canonicalText(檔案文字))`。
         **已知實作缺口**：`captureBaseStore()` 目前回傳的是
           `sha256(canonicalStoreBytes(store))`，且**無人使用** ——
           producer 的 witness 比對**必須**改用**原始 bytes**，
           **不得**沿用該 canonical 值。
 7. lifecycleAffectedClauses 的 membership **完全委派** shared **approved v1.15**
    §9 的 v1.15 lifecycle seed derivation:
      semanticallyChangedClauses ∪ transitionedClauses ∪ driftedClauses ∪ expiredClauses
    —— 其 `B`／`C`／`H`／`T0` 定義、cross-snapshot immutability（typed-ID set difference
       ＋ shared-ID exact-equality assertion）、四集合的 exact derivation，以及 canonical
       union 輸出，authority **一律在上游**。本文件**不重新定義、不擴張、不縮小**，
       **也不**另立第二個 seed 名稱。
    —— **本條已隨 shared approved v1.15 與本文件 approved v1.15 的
       2026-08-21 同步 promotion 生效；契約生效**不等於**已實作 ——
       producer **仍未被接受**（其 provisional 實作與 NOT ACCEPT 地位見 AC178 (2)）。**
 8. governance analysis 必須使用**一份通過 S1／S2 stability 的 head view**
    （即上游的 `H`），並把該 view 的 headViewDigest 帶入 carrier；
    head-view-unstable → 不回傳。
 8b. **Check B 的搜尋位元組（v1.17 新增；下游實作澄清，上游判定表一字不動。
     **v1.17 修訂 3**：前一稿寫「blob 無法以 UTF-8 安全解碼 → fail-closed」，
     那等於**新增**一條「repo 內每一個 regular blob 都必須是合法 UTF-8」的規則
     —— 上游**沒有**這條，一個與 Source 無關的二進位檔會因此讓整輪失敗，
     **該要求撤回**）**:
     上游 shared approved v1.15 §9 的 Check B 是「在**目前 repo 檔案的
     canonical bytes** 中搜尋 excerpt」；v1.15 lifecycle seed derivation 的
     「只讀 **H** 已捕捉的 regular blob bytes」規定的是**取哪一份 bytes**
     （**不得**改自磁碟重讀），**不是**對 canonical 化的豁免 —— 該段自書
     「判定語義沿用上文，本節**不改判定表**」。
     **本文件據此採用的 exact 形狀（bounded downstream interpretation；
     由 Codex 獨立複審裁定，上游文字不動）**:
       needle   ＝ 既有 **canonical UTF-8 excerpt bytes**（UTF-8、無 BOM、LF）
       haystack ＝ 對 **H 已捕捉的 raw blob bytes** 施以 **byte-preserving transform**:
                    ① 去掉**開頭**的 UTF-8 BOM（EF BB BF）
                    ② CRLF → LF、**單獨的 CR** → LF
                    ③ **其餘每一個 byte 原樣不動**
                  —— 這**恰為**本文件既有 `canonicalText` 的形狀（§11b.9c：
                     「去 BOM、CRLF/CR → LF」），差別只在**於 byte 層施加**：
                     **不 decode、不轉碼、不做 replacement**。對合法 UTF-8 文字，
                     此變換與上游 canonical text 規則的**結果相同**；對非 UTF-8
                     bytes，它**不**產生 U+FFFD、**不**製造 replacement-character
                     假匹配，**也不**把任何 path 排除在搜尋之外。
       occurrence 計數與「0／1／2+」三分依上游**原樣**；(path, byteStart) 落在
         變換後的 byte space，locator 仍只是 stale-tolerant hint，
         membership 只看**計數**，因此不受偏移改變影響
     **input universe 不變**：搜尋面**仍是** H 已捕捉的**全部** regular blob ——
       **不得**因某個 blob 不是合法 UTF-8 就把它排除、就把整輪 fail-closed，
       **更不得**把「repo 內存在二進位檔」本身說成 unanalysable source。
     **仍然 fail-closed 的是真正的失敗**：H 捕捉或讀取該 blob 的 IO 失敗、
       Source 的 excerpt 非字串或 canonical 後為空（既有 `E_SOURCE_UNANALYSABLE`），
       以及上游既有的其他分析失敗 —— 這些**一律不得**默認 not drift。
     **raw 與變換後是兩層，兩層都不得互相冒充**（同 AC106 的既有姿態）:
       `H` 本身、`headViewDigest` 與其 `contentDigest` **仍取 raw bytes**，
         **不得**因本條而被正規化；
       變換後的 bytes **只**在 Check B 的搜尋期間存在 —— **不**落盤、**不**回寫 repo、
         **不**改寫 store 內的 excerpt、**不**進任何 digest。
     **已知實作反例（隔離控制組，非推測）**：`countOccurrences()` 直接搜尋 **raw**
       haystack，於是在**同一組已存在於 B 與 C 的 clause／Source**、**無 Transition、
       無 expiry、無任何第二個 lifecycle 成因**之下：LF 與 BOM+LF 得**空** seed，
       而 **CRLF、CR、BOM+CRLF 錯誤地**把該 REQ 放進 `lifecycleAffectedClauses`
       —— 五組的 canonical source 值與 stored excerpt **完全相等**，故為 **false drift**。
       （最初那一組 CRLF 探針另外新增了一個 clause，因而帶進第二個 seed 成因，
       **不是**合格的 discriminator，**已被上列隔離控制組取代**。）
       這是**實作缺陷**，**不是**上游規則變更 —— 修復**不得**改動上游文字，
       **也不得**改動 raw snapshot 或 headViewDigest 的取值。
 9. closure 完成後、**回傳前**再 fresh-load current store 為 **G2**。
    **G2 必須套用與 G1 完全相同的 current-store version rule**
    （缺席 → canonical empty v2；存在 → 必須是 v2；current v1 → fail-closed），
    並要求 **G2 的 canonical digest == G1 的 digest**。
      —— **兩層分開判，不得互相冒充**：先判 version／schema 合法性，再判 digest
         equality。current v1、malformed 或不支援版本一律是 **version／schema
         failure**，**不得**被描述成 presence failure。
      —— **presence 不是獨立訊號**：缺檔與「存在、且內容 canonical-equivalent to
         canonical empty v2」在本比較中**語義等價** —— 兩者算出的 canonical digest
         就是同一個值。因此下列三者**都通過** digest equality：
           兩側皆缺席
           G1 缺席 → G2 為 explicit canonical empty v2
           G1 為 explicit canonical empty v2 → G2 缺席
      —— **missing ↔ present 只有在 canonical digest 不同時才 fail**
         （例如 G1 缺席，而 G2 是一份非空的合法 v2 store）。
         前一稿寫「missing ↔ present 必然不相等」是錯的，**該敘述撤回**。
      —— **不得**新增 presence carrier、`storeCarrierState`、任何 persisted field
         或 inventory 欄位來承載「檔案在不在」。
10. 下列任一情形 → **fail-closed、no result、no partial carrier、no-write**:
      G1 != G2
      head view 於 S1／S2 之間不穩定
      G1 或 G2 的 current-store version 不合法
      store schema／invariant 不合法
      baseTreeOid 非 exact tree，或其 base governance state 不可解析、版本不合法
      分析過程中的任何失敗
11. G2 成功之後 store 若再變動，由**既有 Step 5 pre-state CAS** 負責
    （intent-scan §8 的 expectedInputProvenanceStoreDigest）。
    **不得**把本條改寫成 Step 6 的 current-store comparison ——
    §11b.9d 已明確禁止拿當下 store 與 inputProvenanceStoreDigest 比對。
```

**Binding semantic validation（v1.16；exact）**

producer 在 emit **任何一筆** entry 之前，必須先驗 binding 的**語義有效性**。既有文字要求分類與反向閉包以 clause binding 為據，卻從未寫明由誰、對哪一份快照驗證；於是一個綁到根本不存在的 clause 的 test 也會被分類並 emit。**實測**：head 側綁 `REQ-…0Z`、repo 內**沒有任何** provenance store 時，producer 仍輸出 `added`。

本節**不重新定義**任何判準，**完整委派** shared approved v1.15 §9「綁定兩相」與本文件 §7；下列只寫明**由誰、對哪一份快照、在哪一個 T0** 求值。

```
pre-binding（base 側 tag）—— 對 **B**（baseTreeOid 指向的 immutable historical store）:
  clause binding : shared §9 兩相表的 preChangeBinding 列，**全部** ——
                   clause 與 Source 在 B 中**可解析** ∧ immutable snapshot
                   integrity（**Check A**）成立
  **明文不課**    : active、mechanicallyApplicable、Source live-current（Check B）、
                   exception 未過期
                   —— 前態是歷史事實；對它課現時效力會擋掉模型正要鼓勵的修復。
                      現時效力**只**決定 governanceHit。
  null / EXPL    : **不做** clause／Source resolution（同 shared §9 兩列）

post-binding（head 側 tag）—— 對 **C**（同一次 G1 parsed store，**不得重讀**）
                              與**同一個 T0**（§11b.10c step 4，**不得**重新取樣）:
  clause binding : shared §9 兩相表的 postChangeBinding 列，**全部** ——
                   clause `active ∧ mechanicallyApplicable`（per-kind §2）
                   ∧ Source 檢查（**Check A/B** 兩者）
                   ∧ **完整 exception chain 有效**（targetConstraintRef 可解析、
                     grantAuthorityRef 與 target REQ 的 ownerRef 相符、
                     依 §2 exact grammar 解析之 expiryInstant > T0）
                   —— **v1.17**：上列每一項的**失敗處置**改依下方
                      「Governance-affected witness carve-out」逐項判定。
                      判準本身**一字不變**，改的只是不合法 binding 的處置。
  EXPL           : **不做** clause／Source resolution
  post-state 測試不存在（deleted）: **無 post 驗證**；判定依 exists，
                   **不得**由 binding == null 反推（shared §9 INV-B1／B2）

REQ@DP —— 本文件 §7，**五項全部**，一項不得省:
  DP-y ∈ currentTaskDpIds（來自上方 taskId 解析出的那一個 TaskState）
  DP-y.status == resolved
  DP-y.resolvedBy == REQ-x
  applicable(REQ-x, DP-y)          ← 上游 §2 謂詞，同一 T0
  DP-y.scopeRulingRef.subjectRef == DP-y
  任一不成立 → **fail-closed**

  限定形式套用於 **exception-backed** REQ:
    非 exception-backed 的 REQ 帶 @DP qualifier → **fail-closed**
      —— §7「非 exception-backed 的 REQ 不需 DP 綁定」，硬接一個 DP 上去
         等於偽造一個本不存在的 scope 承諾

  裸形式 @src REQ-x（REQ 為 exception-backed）:
    **僅在** currentTaskDpIds 內反推 resolvedBy == REQ-x 的 DP（歷史 DP 不列入）
    恰好一個候選 → 自動綁定，**續驗上列五項**
    零個或多個候選 → **fail-closed**，訊息要求改用限定形式

失敗處置:
  **fail-closed、no result、no partial carrier、no entries、no-write**
  —— 例外**只有**下方 carve-out 明列的項目，且**只在**有對應 witness 時

  **錯誤的指名義務依「實際拒絕的層級」分開（v1.17；前一稿一律要求指名 testRef，
  該要求不可實現，撤回。**v1.17 修訂 2**：前一稿只分兩層，漏掉本節下方
  orchestration 順序 step 4 的 **mandatory lifecycle seed derivation** ——
  它在 matcher／binding **之前**，卻已經會以上游自己的 code 拒絕某些
  binding 義務對應的 store 形狀，**該兩層劃分撤回**）**:

    層級 1 —— **G1 的 version／schema／invariant validation**（本節順序 step 1
      與 T0 之後的 clock-armed 階段）。此時**尚未**有 matcher、pair 或 testRef，
      因此**不得**要求錯誤指名 testRef、side 或 obligation 編號；
      **必須**保留上游 authoritative 的 error code 與 cause 原樣。
      **本層已先行擋下的情形（實測，非推測）**：exception-grant 的
        targetConstraintRef 不可解析、target 非 `authority=hard-constraint`、
        grantAuthorityRef 與 target ownerRef 不符 —— 三者皆由 store validation
        以自己的 code 拒絕；**REQ 的 sourceRef 不可解析**同樣由 store 的 ref
        validation 以 **`E_DANGLING_REF`** 拒絕，且是在 **clock-free** 的
        schema 階段（**早於** T0，與時鐘無關）；DP terminal 非 active＋applicable
        亦於 clock-armed 階段由 **INV-4** 拒絕。

    層級 1b —— **mandatory lifecycle seed derivation**（本節下方 orchestration
      順序的 **step 4**）。seed 是**強制**階段、**不是**選配：producer 在 matcher
      與 binding validation 之前**必定**已導出四集合，因此 `driftedClauses` 的
      **direct Source walk** 會先行拒絕某些形狀 —— **DEC／ASSUM 的 direct Source
      （`basisRefs` 中的 `S-…`）在 C 中不可解析**時得 **seed derivation 層**的
      **`E_SOURCE_DANGLING`**（**實測**）；excerpt 不可分析時得
      **`E_SOURCE_UNANALYSABLE`**。**這兩個 code 出自 seed derivation 這一層
      （binding validation 的上一層），不是 store validation 的 code** ——
      證據**必須**指名是哪一層，**不得**把兩者混為一談。
      此時**同樣尚未**有 matcher、pair 或 testRef，因此**不得**要求指名 testRef、
      side 或 obligation 編號，**必須**保留該層的 code 與 cause 原樣。
      —— 該 clause 若**同時**是某 DP 的 current terminal，會**更早**在層級 1 由
         INV-4 拒絕（`mechanicallyApplicable` 的 basisRefs 可解析性一肢）；
         層級 1b 可達的只有**非 DP-terminal** 的 DEC／ASSUM。

    層級 2 —— **真正抵達 binding validation 的失敗**。錯誤**必須**指名該 testRef、
      該 binding、**是哪一側**（base 或 head），並指名**是十項義務中的哪一項**
      —— 只說「binding 無效」不足。

    **層級歸屬只決定證據要求，不減免任何義務**:
      在**合法 G1 ＋ 強制 seed** 之下**到不了** binding validation 的是：
        ob-2 的 **REQ 一肢**（止於層級 1）、ob-2 的 **DEC／ASSUM 一肢**
        （止於層級 1b）、ob-6／ob-6b／ob-7（止於層級 1）。
        **其證據因此在該層級**，**不得**要求層級 2 的指名。
      它們仍是 postChangeBinding 列的義務、仍**必須**被實作**課** —— 理由是
        **fail-closed 的 defence in depth**：binding validation **不得**把
        「上游一定已經擋掉」當成自己可以不驗的理由 —— 較早的任一層日後被改動，
        該義務就會**無聲**消失。
      —— **不得**據此宣稱它們會在 **B** 側被課到：pre-binding **明文不課**
         exception chain 的現時效力（見上方 pre-binding 段與 shared §9 檢查分層
         「Check B 與 exception 現時效力只課於 postChangeBinding 所引用者」）。
         **前一稿的括號理由「（B 側與未來放寬的 store 皆可能觸及）」不成立，撤回。**
      —— **不得**以「日後放寬 store validation 就會用到」作為理由：
         本文件**不**承諾、**也不**預告任何未來放寬。
      —— **更不得**為了「讓某項在較晚的層級被觀察到」而放寬、跳過或延後
         **任何**較早的 validation ——**包含**跳過 seed derivation、把 seed 改為
         lazy、或讓 direct Source walk 不 fail-closed。
      —— 對**真正抵達**層級 2 的案例，post-binding 的 Source 義務（ob-2／ob-4／
         ob-5）**照常必須**逐 Source 課到，並依層級 2 的規則指名。
```

**Governance-affected witness carve-out（v1.17；exact）**

**為什麼需要本節** —— v1.16 對 head binding 課 postChangeBinding 列的**全部**條件，而 §6 第 6 格 `governance-affected` 的三個可達 case（AC173 (b1)(b2)(b3)）**正是** clause 在 `C` 中 inactive、drift 或 exception 過期。同一個 binding 不可能既滿足現時效力、又因為不滿足現時效力而命中 seed —— **三格因此全部不可達**。**實測**：(b1) 得 head-binding-inactive、(b3) 得 head-binding-not-applicable；(b2) 在**漏驗 Check B** 的實作下才「通過」，一旦補上 Check B 同樣不可達。**前一稿宣稱 (b2) 不受影響，該宣稱撤回。**

**本節救回的是 (b1)(b2) 兩格，不是三格** —— **(b3) 已於 v1.17 撤回**：`expiredClauses` 只收 exception-backed REQ，其 head binding **必然**經 §7 的 `applicable(REQ, DP)`，expiry 令該項為 false，而 **§7 不在本 carve-out 之內**；更早一步，上游 **INV-4** 要求 DP 的 current terminal 必須 active ∧ applicable，故「DP 仍以過期 REQ 為 terminal」的 G1 **本身不合法**（**實測**：`E_INV4_NOT_APPLICABLE`）。witness-3 因此**沒有可達的正例**，只保留 DEC／ASSUM 側的**負例**（見下方可分離性一節與 AC176 (11d-4)）。**不得**為了讓 (b3) 可達而豁免 `applicable` 整項或放寬 INV-4。

本節**不放寬 binding 的有效性判準**（那是 shared §9 的，**不在本文件**），只改**不合法 binding 的處置**：在**有對應 lifecycle witness** 時，producer 產出一筆受影響 entry，而**不是**整輪 fail-closed。

```
postChangeBinding 列的**不可再分義務**與其可豁免性（closed；**恰十項**；
**v1.17 修訂**：前一稿寫「恰九項」，漏掉 exception chain 的 target authority 一肢，
**該計數撤回**）。**編號沿用 ob-1…ob-9 並插入 ob-6b**，共十項 —— 保留既有編號以免
AC176 既有引用整體位移:

  ob-1   clause 在 C 中可解析                                   **永不可豁免**
  ob-2   該 clause 的 **direct Source set 中的每一個** Source
         在 C 中可解析                                          **永不可豁免**
  ob-3   status(c) == active                                    可豁免 ⇔ witness-1
  ob-4   **direct Source set 中的每一個** Source 通過
         **Check A**（immutable snapshot integrity）             **永不可豁免**
  ob-5   **direct Source set 中每一個 driftMode=repo-file 的**
         Source 通過 **Check B**（對 H 的 live-current occurrence）可豁免 ⇔ witness-2
  ob-6   exception chain：targetConstraintRef **可解析**         **永不可豁免**
  ob-6b  exception chain：該 target **必須是 `authority=hard-constraint`
         的 REQ**（shared approved v1.15 §9 來源列，**v1.17 補上**）  **永不可豁免**
  ob-7   exception chain：grantAuthorityRef == target REQ
         的 ownerRef                                            **永不可豁免**
  ob-8   exception chain：expiryInstant > T0（§2 exact grammar）  可豁免 ⇔ witness-3
  ob-9   mechanicallyApplicable 的其餘 per-kind 項 —— DEC／ASSUM 的
         approvedBy／governedBy principal 合法、derivedFrom DP 可解析、
         basisRefs 中的 S-n／RecordRef 可解析                    **永不可豁免**

  **direct Source set 是集合，不是單數**（v1.17；前一稿以單數寫 ob-2／ob-4／ob-5，
  容許只驗第一個就通過，**該寫法撤回**）:
    direct Source set 的 exact 定義沿用 shared approved v1.15 §9
      —— REQ: sourceRef；DEC／ASSUM: basisRefs 中 discriminated union 為
         Source ref 的**全部**成員（**不遞迴**）。
    ob-2／ob-4 **對集合中每一個成員**分別課；ob-5 **對集合中每一個
      driftMode=repo-file 的成員**分別課（snapshot-only 依上游**永不算 drift**）。
    任一成員不成立即該義務不成立 —— **不得**因「至少一個成員通過」而放行。

  **義務之間的重疊必須照實記錄，不得為了讓測試互斥而改義務（v1.17 修訂 2）**:
    ob-2（direct Source 可解析）與 ob-9 的 basisRefs 一肢在 DEC／ASSUM 上**重疊** ——
      上游 §2 的 `mechanicallyApplicable` per-kind 定義**明文**含
      「basisRefs 中的 S-n／RecordRef 可解析」。本文件**不得**為了讓兩者互斥
      而把 `S-n` 自 ob-9 刪除，**也不得**改變 ob-9 的實質意義。
    **重疊只發生在 DEC／ASSUM 一側（v1.17 修訂 3；前一稿把 REQ 例子也寫進
      「同時打到 ob-9」，REQ 根本沒有 basisRefs 一肢，該寫法撤回）**:
      **REQ** 的 dangling `sourceRef` → **只**使 **ob-2** 不成立
        （REQ 的 per-kind `mechanicallyApplicable` 內**沒有** basisRefs 一肢，
        故 ob-9 對 REQ 為空）；於**層級 1（store validation）**得 `E_DANGLING_REF`。
      **DEC／ASSUM** 的 dangling direct Source（`basisRefs` 中的 `S-…`）
        → **同時**使 ob-2 與 ob-9 的 basisRefs 一肢不成立；**非 DP-terminal** 者
        於**層級 1b（seed derivation）**得 `E_SOURCE_DANGLING`，
        DP-terminal 者更早於層級 1 由 INV-4 拒絕。
        —— 其證據形式為 **deterministic first failure ＋ 受影響義務清單**，
           **不**要求單一義務指名。
      dangling **RecordRef**（**非 DP-terminal** 的 DEC／ASSUM，其 `basisRefs` 含一個
        不可解析的 `{ kind, ref }`，其餘 basis 合法）→ **只**使 ob-9 不成立：
        RecordRef **不在** direct Source set（上游 §9 明文「**不**沿 RecordRef
        展開」），seed 的 direct Source walk **不**碰它，store 的 ref validation
        亦**不**課 clause 的 basisRefs —— 這是 ob-9 在**層級 2** 唯一可獨立隔離的
        discriminator。**該 clause 必須非 DP-terminal**，否則 INV-4 會在層級 1
        先行拒絕，測到的就不是 binding 層。
      ob-9 的 principal（approvedBy／governedBy）與 derivedFrom 兩肢由 store
        validation 以**自己的 code** 先行拒絕 → 其證據留在**層級 1**，
        **不得**改寫成層級 2 的指名，**也不得**為此放寬 store validation。

  —— **重疊是 per-kind 的，必須照 §2 寫實（v1.17 修訂 2；前一稿一律說
     ob-3／ob-5／ob-8 三項都在 `mechanicallyApplicable(c)` 之內，與同段
     「applicable() 拿不到 H」自相矛盾，該寫法撤回）**:
       REQ        : §2 的 `mechanicallyApplicable(REQ)` ＝ status=active
                    ∧ **來源檢查（§9 Check A/B）** ∧（exception-backed 時：
                    未過期 ∧ targetConstraintRef 可解析）—— ob-3、ob-4、ob-5、
                    ob-6、ob-8 因此**在定義上**與該謂詞重疊。
       DEC／ASSUM : §2 的該謂詞**只**含 status=active、principal 合法、
                    derivedFrom 可解析、basisRefs 可解析 —— **不含任何 Source 檢查**。
                    DEC／ASSUM 的 ob-4／ob-5 **完全**來自 §9 postChangeBinding 列的
                    「Source 檢查（Check A/B）」，**不**來自該謂詞；其 ob-8 來自
                    §9 檢查分層「來源」列（postChangeBinding 所引用的 exception-grant
                    完整鏈），**也不**來自該謂詞。
     **豁免一次即對兩處同時生效**（否則 carve-out 對 REQ 恆為空集合：clause 仍會在
     `mechanicallyApplicable` 那一層被擋下）。該謂詞的**其餘** per-kind 項（ob-9）
     仍**照課**。
  —— **謂詞的定義**與**一次拿不到 H 的呼叫**是兩件事：任何**沒有 H** 的
     `mechanicallyApplicable()`／`applicable()` 呼叫**只能**清掉 Check A 那一肢，
     **不可能**清掉 Check B（ob-5）；對 DEC／ASSUM 而言，該謂詞內**連
     Check A／B 的對應肢都沒有**。因此 **ob-5 必須由實作對 H 明確求值**，
     **不得**以「呼叫 applicable() 即等於全驗」自證。
     **已知實作反例**：`69cff79` 只課 active 與 mechanicallyApplicable，
     ob-5 從未求值，而其註解宣稱 applicable() 已承載該列 —— **該註解不成立**。

witness（exact；三者皆取自**同一次** G1、**同一個** H、**同一個** T0 ——
  即下方「Producer 的 G1／G2 orchestration」順序中 **step 4（lifecycle seed derivation）**
  已導出的**那一份** seed，**不得**為此重讀 store、重取 T0 或重算 seed，
  **不得**接受 caller-provided witness set。
  —— 注意：**§11b.10c step 4** 在上方 store-capture 編號中指的是 **T0 取樣**，
     與 orchestration 順序的 step 4 **不是同一個編號系統**；本節指的是後者）:

  witness-1  tag.clauseRef ∈ transitionedClauses  （shared approved v1.15 §9 derivation）
  witness-2  tag.clauseRef ∈ driftedClauses
  witness-3  tag.clauseRef ∈ expiredClauses

  **必須逐項對應，不得以 union 代替**:
    以 `tag.clauseRef ∈ lifecycleAffectedClauses` 豁免任一義務 → **禁止**。
    只 drift 的 clause **不得**因此豁免 ob-3 或 ob-8；
    只過期的 clause **不得**因此豁免 ob-3 或 ob-5；
    只被 transition 的 clause **不得**因此豁免 ob-5 或 ob-8。
  witness **只**看 canonical clauseRef —— **不得**以 dpRef、path、bodyDigest、
    tag 字面相似度或任何自由文字推導（同下方 governanceHit 的 membership 規則）。
  `semanticallyChangedClauses` **不是** witness：它是「C 新增的 clause」，
    與 head binding 的現時效力失敗**無對應關係**。

  **三對的可分離性並不相同**（依 shared approved v1.15 §9 的 derivation 導出，
  記錄於此以免實作者去構造不存在的案例。**v1.17 修訂**：前一稿把 ob-8 判為
  「重合、不可構造」，**該判定錯誤並撤回** —— 見下）:

    ob-3 ↔ witness-1 : **可分離**。transitionedClauses 只收 `T.id ∉ ids(B.transitions)`
                       的**新增**生效 Transition；一個**在 B 就已被 retire** 的 clause
                       其 status != active，卻**不在** transitionedClauses ——
                       那不是本次的治理事件，而是綁到早已作廢的 clause，
                       **必須 fail-closed**。此即本 carve-out 的主要判別力所在。

    ob-5 ↔ witness-2 : **重合**。driftedClauses 的成員條件就是「direct、repo-file 的
                       Source 對 H 得 zero occurrence」，而 ob-5 課的正是同一組 direct
                       Source；driftMode=snapshot-only 依上游**永不算 drift**。
                       因此「ob-5 失敗但無 witness-2」**不可構造**。

    ob-8 ↔ witness-3 : **可分離，但可分離的那一半不是 REQ**。expiredClauses **只**收
                       「REQ 且其 sourceRef 指向 exception-grant」；上游明文
                       **DEC／ASSUM 不因 basisRefs 含 exception-grant 而入本集合**。
                       而 ob-8 課於 postChangeBinding **所引用的** exception-grant
                       Source —— DEC／ASSUM 的 basisRefs 含過期 grant 時，ob-8 失敗
                       而該 clause **不在** expiredClauses → **無 witness → fail-closed**。
                       **可分離的一半在 DEC／ASSUM 側，不在 REQ 側。**

    —— **REQ 側的 witness-3 carve-out 實際上到不了**（v1.17 記錄，**不假裝可達**）:
       exception-backed REQ 的 head binding **必然**經 §7，而 §7 課
       `applicable(REQ, DP)`；expiry 使 mechanicallyApplicable 為 false，
       `applicable` 因此為 false → §7 fail-closed，而 §7 **不在**本 carve-out 之內。
       更早一步：上游 **INV-4** 要求 DP 的 current terminal **必須 active ∧ applicable**,
       terminal 因 exception expiry 失效者**必須** repoint 或 reopen ——
       因此「DP 仍以該過期 REQ 為 terminal」的 G1 **本身就不合法**，
       在 clock-armed validation 即被拒（**實測**：`E_INV4_NOT_APPLICABLE`）。
       **結論**：witness-3 的可達案例**只有** DEC／ASSUM 那一側的**負例**；
       REQ 側既不能成為正例、也不能成為負例。**不得**為了讓它可達而豁免
       `applicable` 整項，**不得**放寬 INV-4。

    —— 重合**不表示** witness 比對可以省略：省略之後 union 與逐項對應**不可區分**，
       ob-3 那一對也會跟著失守。實作**仍必須**逐項比對。
    —— 上述「不可構造」與「不可達」**必須被記錄**，**不得**以任何近似案例假造，
       **更不得**改寫成一個實際上驗到別的東西的測試。

豁免的**效力邊界**（本節最重要的一條）:
  豁免**只**免除「整輪 fail-closed」，**不**使該 binding 成為**有效** binding。
  —— **不得**因此宣稱該 test 已通過 gate、已修復、或其 binding 已被接受；
  —— 下游 shared §9 gate **仍必須**對同一個 binding **fail-closed**，直到 binding
     被實際修復（改綁 successor、重新錨定 Source、換發 exception）——
     gate **自行**依 shared §9 重新求值，**不**消費 producer 的任何判定；
  —— **不得**為此在 entry 或 envelope 新增任何欄位：entry 仍為既有 closed key set，
     envelope 仍**恰七欄**，carrier 仍**恰四欄**。producer **不**攜帶「已豁免」旗標，
     gate 也**不得**消費任何這類旗標。

分類**不由本節決定**:
  豁免之後，該 logical test 的 status 仍**完全**依 §6 的 ordered classification 決定
  —— body 也變則為 `modified`、path 變則為 `moved`，餘類推。本節**不**強制第 6 格、
  **不**改 §6 precedence、**不**新增分類訊號；同一個 logical test 仍**至多一筆** entry。

**仍然一律拒絕**（**不得**因本節而放行）:
  ob-1／ob-2 不成立（dangling clause 或 dangling Source）
    —— **v1.17 修訂 2**：ob-2 在**合法 G1 ＋ 強制 seed** 之下**更早**被擋下
       （REQ 一肢於層級 1 得 `E_DANGLING_REF`；非 DP-terminal 的 DEC／ASSUM 一肢
       於層級 1b 得 `E_SOURCE_DANGLING`），但**仍是** post-binding 的義務、
       **仍必須**被課，見上方「錯誤的指名義務依實際拒絕的層級分開」
  ob-4（Check A）不成立 —— **沒有**任何 lifecycle 集合對應 immutable snapshot
    integrity：drift 是 **Check B**，**不是** Check A
  ob-6／ob-6b／ob-7 不成立 —— shared approved v1.15 §9 明文：malformed chain、dangling
    targetConstraintRef、target authority 不符、owner mismatch 是
    **validation failure，不是 membership**（三者在**合法 G1** 之下更早由
    store validation 擋下，見上方「錯誤的指名義務依實際拒絕的層級分開」）
  ob-9 不成立
  ob-3／ob-5／ob-8 不成立**且無對應 witness**
  §7 REQ@DP 五項的任一項不成立 —— **本節不豁免 §7**
  pre-binding（對 **B**）的任一項不成立 —— **本節只管 post 側**
```

```

Producer 的 G1／G2 orchestration（v1.16；v1.17 修訂 step 6 與內部 context 邊界；exact）:
  standalone buildGovernanceSeedPreimage() **可**保留其既有形狀 ——
    G1 → seed derivation → G2 → 回傳四欄 carrier。本節**不改**它。
    **v1.17**：它**不做** task resolution，其順序中因此**沒有**下方順序的
    step 3（taskId resolution）；current store 缺席或為 canonical empty 時
    它**仍必須成功**。
    **v1.17 修訂 2（前一稿寫「不讀 taskStates」，與「必須做完整 store validation」
    自相矛盾，該寫法撤回）**：正確的界線是 —— **seed derivation 不選定、不解析、
    不使用任何 TaskState**，四集合的推導**不得**以 TaskState 為輸入或條件；
    但 G1／G2 的 schema／invariant validation **仍涵蓋整份 store，含 `taskStates`**。
    **不得**把 taskStates 排除在 store validation 之外，**也不得**因本條而讓一份
    taskStates 不合法的 store 通過。

  produceChangedTestInventoryV2() **不得**照抄那個形狀。具體而言:
    **不得**先取得一份**已完成 G2** 的四欄 carrier，再於 G2 之後拿 G1
      （或任何 G1 衍生物）去驗 binding —— 那會讓 binding 的判定發生在
      「store 已宣告未變」之後，G2 因此證明不了它所宣稱的那段區間。
    producer **必須**以 **private／lexically retained context** 在自己的
      invocation 內保留**同一份** parsed B、parsed G1、選定的 TaskState 與 T0。
    **v1.17 修訂 2（明文；前一稿只寫「取得」，被讀成可以走 public 入口）**：
      producer **不得**呼叫**已完成**的 public `buildGovernanceSeedPreimage()`
      再拿其結果去驗 binding —— 該 public operation 的定義就是「**做完 G2** 才
      回傳」，且它**不**回傳選定的 TaskState，兩者皆與上列順序牴觸。
      producer **必須**在自己的 invocation 內，經**兩個 operation 共用的
      module-private orchestration helper 就地（inline）導出** governance seed，
      並在 **G2 之前**完成 binding validation。

  順序（exact；G1 與 G2 之間**不得**有第三次 current-store read）:
    1. G1 fresh-load ＋ parse ＋ clock-free schema validation
    2. T0 取樣（§11b.10c step 4，恰一次）
    3. taskId resolution（對同一份 G1 的 taskStates）
    4. lifecycle seed derivation（四集合）
    5. matcher ＋ §6 ordered classification 所需的一切 G1-dependent work
    6. **全部** pre-binding（對 B）與 post-binding（對同一 G1、同一 T0）驗證，
       含 §7 五項與裸形式反推；**v1.17**：post-binding 的 ob-3／ob-5／ob-8 若不成立，
       於此處對**本順序** step 4 已導出的**那一份** lifecycle seed 做 witness 比對
       （**不得**為此重讀 store、重取 T0 或重算 seed）
    7. **最後一次使用 G1 之後**才 G2 fresh-load ＋ 同一 version／schema rule
       ＋ canonical digest equality
    8. 通過後才組 envelope 並回傳

  G2 **必須**晚於步驟 6 的最後一次 G1 使用，且**必須早於**任何 result、
    partial carrier、entries 或 envelope 的回傳 —— 兩個方向都必須成立。

  內部 context **不得**（**v1.17 收緊**；下列每一條都是**獨立**的禁止）:
    成為 GovernanceSeedPreimage 的第五欄（carrier 仍**恰四欄**）
    成為新的 public operation、caller injection 入口或 persisted field
    **被 export** —— 承載它的 helper **必須 module-private**，
      **不得**出現在任何 module 的 export 清單中
    **接受 caller-supplied callback** —— **不得**以 `run(request, …, work)`
      這類形狀把 B、G1、選定的 TaskState、T0 或 lifecycle seed 交給外部函式；
      共用實作**只能**由 module-private helper 內部完成
      —— **已知實作反例**：`69cff79` 的 `export async function
         runWithGovernanceContext(request, argumentCount, operation, work)`
         把 `{ base, current, taskState, t0, snapshot, lifecycleAffectedClauses }`
         整包交給 caller 的 callback。**該形狀違反本條**，兩個 operation 之間
         的共用**必須**改為 module-private helper。
    成為 test seam —— 觀察它的**唯一**授權方式仍是 §11b.10c 形狀 B

快照邊界:
  parsed B、parsed G1、選定的 TaskState 與 T0 **可**在 invocation 內部保留供本節使用 ——
  它們本來就已在同一次 invocation 中被讀取／取樣，重讀或重新取樣反而會開啟
  §11b.10c step 2／step 4 明令關閉的窗口。
  但**不得**因此:
    加入 GovernanceSeedPreimage 的四欄 carrier（仍**恰四欄**）
    加入 ChangedTestInventoryV2 的七欄 envelope（仍**恰七欄**）
    加入 public request 之外的任何入口（producer 的 request 仍**恰為**
      { repoRoot, baseTreeOid, taskId }；buildGovernanceSeedPreimage 的
      仍**恰為** { repoRoot, baseTreeOid }）
    成為新的 public operation、persisted object 或 caller-supplied 入口
```

**`produceChangedTestInventoryV2()` 的唯一 orchestration**

```
producer 必須**自行**取得下列兩者，**不得**由 caller 傳入:
  DiscoveryAnalysisPreimage  ← §11b.10b
  GovernanceSeedPreimage     ← 本節

**v1.17**：三者的 request 不再是同一份。producer 的 request 為
  { repoRoot, baseTreeOid, taskId }；上列兩份 preimage 各以 producer request 的
  **{ repoRoot, baseTreeOid } 投影**取得 —— **恰為**該兩欄，**不得**多帶 taskId，
  **不得**改用第二個 baseTreeOid、第二個 repoRoot 或任何 caller 提供的替代值。
  taskId **只**用於 producer 自己的 task resolution（見上），**不**流入任一 preimage。

**「取得」的形狀（v1.17 修訂 2）**:
  GovernanceSeedPreimage 的四欄值**必須**由 producer 在自己的 invocation 內，
    經**共用的 module-private orchestration helper 就地導出** ——
    **不得**呼叫已完成 G2 的 public `buildGovernanceSeedPreimage()`。
    **投影約束的是「governance derivation 的邏輯輸入事實」**（v1.17 修訂 3；
    前一稿寫成「該 helper 只看得到這兩個值」，那被讀成對 private helper 的
    argument 數量限制，**該寫法撤回**）:
      **受約束的是**：seed derivation 只以 `repoRoot` 與 `baseTreeOid` 這兩個
        governance 輸入事實為據 —— **不得**改用第二個 `baseTreeOid`、第二個
        `repoRoot`、任何 caller 提供的替代值，`taskId` **不**流入 seed derivation。
      **不受約束的是**：private helper 的參數個數、內部 continuation 與
        invocation 內保留的 context ——**producer 在 G1 與 G2 之間**所需的
        task resolution、matcher、binding validation 等**內部控制**一律**允許**。
      **仍然禁止的是**：caller 提供或注入任何被觀察的 context 或輸入、
        任何把 `B`／G1／TaskState／T0／seed 交給外部函式的 **export**、
        callback 或 capture 入口。
      public 的 exact request key set 與 envelope／carrier 形狀**一字不變**。
  DiscoveryAnalysisPreimage 仍依 §11b.10b 由 producer **自行**取得。

  **實作佈局（原則已定；細節屬 promotion 後的實作階段）**:
    **一個** common internal implementation module 同時擁有**兩個 logical
      operation** 與那個 **private context helper**；
    既有 public facade 路徑（`governance-seed-preimage.mjs`、
      `changed-test-inventory-producer.mjs`）**保留不變**；
    export 檢查**必須同時**涵蓋兩個 facade **與**該 common implementation ——
      三者的 export 清單皆**不得**出現任何 context、callback 或 capture 入口。
    **明文不算違規**：error class，以及**既有**的 pure authority primitive
      （`statusOf`、`checkSourceIntegrity`、`parseCanonicalExpiry` 等）
      **不**構成第三個 snapshot-taking operation —— 它們**不讀** store，
      因此**不受**「G1 與 G2 之間不得有第三次 current-store read」的限制。

回傳前**強制**下列等式（exact code point 相等）:
  request.baseTreeOid == discovery.baseTreeOid == governance.baseTreeOid
  discovery.headViewDigest == governance.headViewDigest

任一不等:
  **fail-closed**
  **不得**自動挑其中一邊
  **不得**只重跑其中一份 preimage 再與另一份拼接
  **不得**回傳 partial matching、partial entries 或 envelope
  caller 需要重試時，**只能**重啟一次完整 invocation

registryDigest **只能**取 DiscoveryAnalysisPreimage 已綁定的值 ——
  producer **不得**另讀第二份 registry 再與之混用（§11b.9c v1.11 的
  「同一次 parsed root」義務在 producer 這一層同樣成立）。
```

**Reverse closure 與 `governanceHit` 的 exact boundary**

```
對 matcher 產生的每一個 logical test:
  governanceHit ⇔ head 側 tag 是 clause binding
                  ∧ tag.clauseRef ∈ governance.lifecycleAffectedClauses

  head 側 tag 為 { expl: true }、為 null，或該 test 沒有 head 側 declaration
    → governanceHit **false**
  membership **只**看 canonical clauseRef —— **不得**以 dpRef、path、bodyDigest、
    tag 字面相似度或任何自由文字推導
  producer **不得**接受 caller-provided hit set（見上 anti-injection）
```

這正是 shared v1.14「**目前**綁定指向 `lifecycleAffectedClauses` 的所有測試」的反向閉包：「目前」即 head 側 post-state binding，因此 base-only 的 residual（`deleted`）沒有 head 側 binding，**不可能**命中。`governanceHit` **只**進 §6 ordered table 的**第 6 格**；**不得**為已由 `added`／`deleted`／`moved`／`modified`／`retagged` 收錄的 logical test 再新增第二筆 entry —— 這是 §6 的既有規則，本節**不放寬**，也**不**把它重述成一條新規則。

**Entry projection 必須 exact**

```
producer 必須先呼叫已接受的 base／head one-to-one matcher（§6 matching），
再對每一筆 pair **逐筆**套用 §6 的 ordered classification。本節**不**新增分類訊號。

persisted side（六種 status 各自的取值來源；**不得**混用）:
  added                : testRef、framework、implementationIdentity、tag、bodyDigest
                         **全部取 head**
                         —— tagAfter ＝ head tag；headBodyDigest ＝ head bodyDigest；
                            tagBefore ＝ null；baseBodyDigest **必須缺席**
  deleted              : 上述**全部取 base**
                         —— tagBefore ＝ base tag；baseBodyDigest ＝ base bodyDigest；
                            tagAfter ＝ null；headBodyDigest **必須缺席**
  modified／retagged／
  moved／
  governance-affected  : testRef、framework、implementationIdentity **取 head**
                         tagBefore、baseBodyDigest **取 base**
                         tagAfter、headBodyDigest **取 head**
  unchanged            : **完全省略** —— 不 emit placeholder、不 emit 任何 marker

  reason               : **只有** governance-affected 使用 governance-affected；
                         其餘五種一律 content-change（§2 既有 iff 不變）
  每個 logical test **至多一筆** entry。

非 deleted 的 head tag 為 null:
  依 §2 既有 post-state binding 不變量（status != deleted ⇒ tagAfter != null）
  **fail-closed** —— **不得**自動補 EXPL、不得補任何 binding、不得省略該 entry。

ordering:
  entries 依既有 (path, adapterId, structuralId) 的 Unicode code point tuple
    **嚴格遞增**（deleted 取 base 側 testRef，其餘取 head 側）
  每個 object subtree 依既有 canonical source-key ordering
```

**Envelope 與副作用邊界**

```
produceChangedTestInventoryV2() 回傳 **exact** ChangedTestInventoryV2:
...
各欄的唯一來源（**不得**另尋第二個來源）:
  inventoryVersion           : 常數 2
  baseTreeOid                : 兩份 preimage 已驗相同的共同值
  registryDigest             : DiscoveryAnalysisPreimage
  headViewDigest             : 兩份 preimage 已驗相同的值
  inputProvenanceStoreDigest : GovernanceSeedPreimage
  entries                    : matcher ＋ §6 ordered classification
  inventoryDigest            : §11b.9c 的**唯一公式**，不得另立

immutability : 回傳 object 與所有 nested value **深度 frozen**。

副作用邊界（本 component **只**回傳記憶體物件）:
  **不寫** .ctide/output/**（含 inventory 自身）
  **不寫** provenance store
  **不**建立或修改 explicit config
  **不**修改 registry
  **不**解除 product gate —— §11b.12 的 unsupported-populated-inventory 原樣保留
  **不**呼叫 Step 5／6、ledger 或 arbiter
  invocation 之間**不得**快取 store、seed、preimage、entries 或任何 digest
  任一錯誤 → **整輪 no-result、no-write**，不回傳 partial envelope
```

**Defensive-branch 的 test-only evidence shape（本節 exact；不是 production seam）**

上方的 anti-injection boundary 使某些 defensive branch **沒有任何合法輸入可以抵達**：兩份 preimage 都由 producer 在單次 invocation 內自建，caller **不得**注入或替換其中一份，回傳的 carrier 又是**深度 frozen**。因此「把 result 破壞後由 runtime 拒絕」**不是**可實作的證據形狀 —— 對深度 frozen 的物件寫入是 no-op，該 branch 永遠不會被觸發，把它寫成 AC 會逼實作者在「加禁止的 seam／依賴競速／宣稱通過未觸發的分支」之間三選一。凡 AC 要求觀察此類 branch 者，**唯一**授權的證據形狀如下 —— 與本節 `T0` 的 child-process `Date.now` wrapper **同一姿態**：證據留在 test harness，production 一動不動。

```
形狀 A（disposable scratch repository 的檔案突變）:
  於**獨立 child process** 中對一份 **可丟棄的 scratch repo copy** 操作 ——
  在該 copy 內以檔案突變製造被測條件（.ctide/provenance.json 的內容、
  head view 內的檔案、registry 檔案等），再以該 scratch 路徑作為
  **正常的** repoRoot 呼叫**未修改**的 production API。
  **不得**對 repo working tree 或 installed module 操作。

形狀 B（disposable scratch source copy 的單點突變）:
  於**獨立 child process** 中把 component module 原始碼複製到 scratch 目錄，
  施加**單一點**編輯（停用 canonical sort、停用 dedup、
  令其中一份 preimage 取用另一個**合法**的 baseTreeOid／headViewDigest、
  令 producer 改讀第二份 registry 等），import **該 copy** 後
  斷言其 invariant guard 拒絕。
  parent process **只** import **未修改**的 production module。

共同要求（全部必須滿足）:
  determinism : **不得**依賴 timing race、**不得**對 live path 並行寫入、
                **不得**等待真實時鐘跨日。凡「invocation 途中 G2 改變」
                這類觀察（例如 AC171 (xi) 負例 2）一律以形狀 A／B 取得，
                **不得**以真實競速取得。
  no seam     : **不得**新增 production export、request key、
                environment argument、clock／capture hook、
                component module path 或任何 test-only seam；
                production request contract 仍**恰為** —— producer
                { repoRoot, baseTreeOid, taskId }、buildGovernanceSeedPreimage
                { repoRoot, baseTreeOid }（v1.17）—— 上方 anti-injection 清單**一字不變**。
  層級標明     : 斷言必須指名**實際拒絕的層級** —— source／schema validation、
                carrier guard、cross-binding guard 三者擇一指名，
                **不得**只斷言「失敗了」，**更不得**把上游 schema 層的拒絕
                誤稱為 carrier guard 或 cross-binding guard 的拒絕。
  無殘留       : scratch 目錄與其內容於測試結束後刪除，**不得**遺留，
                **不得**寫入 repo working tree。
```
...
**Current implementation status（status carrier，非 normative；截至 2026-08-16）** —— 上表六項前置條件的現況：**六項 rollout 前置條件已有五項實作並接受，只剩 producer。五項完成**仍不放寬**「六項全部滿足才可接受 populated inventory」的規則。**
...
implemented and accepted:
  ...
  canonical v2 inventory parser／reader           ← v1.14 observable invariants 已由 `9f42628`
                                                  實作並經獨立審查接受
...
```
現況（截至 2026-08-22 rollout acceptance，機械可查）:
  cressetide/skills/vigil/scripts/test-adapters.json  implementationId: "node-test-v2"
  cressetide/skills/vigil/vendor/vendor-manifest.json implementationId: "node-test-v2"
  compiled closed mapping（§11b.3）                    node-test-v2
  executable component 自報                            node-test-v2
  —— 四者**一致**，且已經獨立複審 **ACCEPT**（acceptance chain
     `0682553 → 02f8bc6 → 4c1ff43 → 7da3dae`，不得歸給任一單獨 commit）。
  —— 授權由 vendor manifest 的 approvedOn: 2026-08-22 ＋ 同日 ADR 修訂承載；
     該授權由 **Codex 依使用者委任的 roadmap decision authority** 作出，
     **不是**當日另一次使用者明確核准。
  —— 先前記載的「兩者仍逐欄授權 v1」是 **promotion 當下**的現況，
     已由本次 rollout 取代；node-test-v1 只保留為歷史資料的 identity。

v2 實作的一致性要求（四者**必須同時**為同一個 identity）:
  formal registry（test-adapters.json）
  vendor manifest（vendor-manifest.json）
  compiled closed mapping（§11b.3 的 enum ＋ binding table ＋ discovery 綁定值）
  executable component（node:test adapter 自身回報的 implementationId）
  —— 任一不一致即 **fail-closed**。
     **明確負例**：registry 單獨改為 v2 而 manifest 仍為 v1 → **必須 fail-closed**，
     **不得**被當成「已升版」。

授權與審查:
  identity 變更**仍須**重新 authorization 與**獨立 implementation review** ——
    與 v1 當初取得 acceptance 的門檻相同，**不得**因 spec 已 promote 而略過。
  既有 `node-test-v1` 的 acceptance **只保留為歷史 acceptance**，
    **不**自動移轉到 v2。

六項 rollout 前置條件的計數（closed set 不變，只是這一項的狀態隨 v2 移動）:
  v1.16 已 promote、v2 implementation 尚未接受   : **4／6**
      —— adapter registry 因 identity 尚未一致而**退回未完成**；producer 仍未完成
  **現況** —— v2 registry／manifest／executable chain 已接受 : **5／6**
      —— adapter registry 回到完成；**只剩 producer**
  producer 接受後                                : **6／6**

  —— `node-test-v2` executable component 仍是 **supporting component**，
     **不**新增第七項 prerequisite；它是既有第一項（adapter registry）
     能否算完成的一部分，不是另一項。
```

**Approved v1.15 effective boundary（status carrier，非 normative）** —— 上表的**六項 rollout 前置條件仍是同一個 closed set**，其內容、計數與語義**完全不變**：**六項已有五項實作並接受，只剩 producer**。approved v1.15 新增的 §11b.10c **不動這個計數**：

```
GovernanceSeedPreimage : producer 的 **mandatory supporting input component**，
                         **不是**第七項 rollout prerequisite。
approved v1.15 的範圍  : **只**定義未來 producer 如何取得並綁定它；
                         契約已於 2026-08-21 生效。
producer               : **已實作並經獨立複審接受（2026-09-05）**
                         （**status-only 更新**：接受歸屬於**完整 chain**，
                          **不**歸給 `69cff79` 一段 —— 見下方 v1.17 producer
                          acceptance addendum 與 AC179 (6)。
                          **先前狀態逐字保留為歷史**：`020e7ec`／`53801a9`／`69cff79`
                          在其各自受審當時皆經**獨立複審 NOT ACCEPT**、維持 provisional；
                          那些判定對**當時的候選**仍然正確。
                          再前一稿的「尚未實作」在上方 **dated 2026-08-16** 區塊中是
                          **當時**的事實，該 dated 敘述**保留不改**。）
GovernanceSeedPreimage : **已實作並隨同一次複審被接受**（同上）。
persisted batch reader : **實作中（in progress），尚待獨立複審**（2026-09-05 起）——
                         v2 record discriminator、inventorySnapshot 契約、legacy boundary、
                         chain 版本單調性與 raw nested inventory 的 ingestion 邊界。
                         **v2 writer 未啟用**，product gate **未動**。
其餘仍未完成           : artifact emission，以及 Step 5／6、ledger、arbiter wiring。
gate                   : unsupported-populated-inventory **不得解除**；
                         populated inventory **仍不得接受**。
AC118／AC136／AC137／AC138 : **仍不得宣稱已滿足**；**Phase 2 仍不得宣稱 READY**。
```

**v1.17 effective addendum（status carrier，非 normative；生效日 2026-09-05；不回改上方任何 dated 敘述）** —— 本文件已 promote 為 **approved v1.17**（Codex 依委任所作的獨立複審與核准，**非** panel、**非**使用者當日另一次直接核准；歸屬與受審檔案 hash 見 **AC179**）。**六項 rollout 前置條件的 closed set 與計數不變 —— 仍為 5／6，只剩 producer。** promotion **只**解除「v1.17 契約不得實作」這一條；`69cff79` **與 promotion 之後寫出的 producer 實作**仍為 **provisional／NOT ACCEPT**，（**status-only 後續**：該 producer 實作其後已於 **2026-09-05** 經獨立複審 **ACCEPT**，計數進為 **6／6** —— 見下方 producer acceptance addendum。本段其餘敘述是 **promotion 當下**的狀態，逐字保留。）`unsupported-populated-inventory` **不得解除**，populated inventory **仍不得接受**，**Phase 2 仍不得宣稱 READY**，artifact emission 與 Step 5／6、ledger、arbiter wiring **仍未開始**。

**Producer acceptance addendum（status carrier，非 normative；2026-09-05；status-only，不改任何 normative 要求，不回改上方任何 dated 敘述）** —— **producer／governance 實作已由 Codex 獨立複審 ACCEPT**（`docs/superpowers/reviews/2026-09-05-producer-implementation-codex-review.md`；Codex 依使用者委任的獨立複審權作出的 stage decision，**非** panel、**非**使用者當日另一次直接核准）。

```
歸屬            : **完整 implementation／remediation ancestry ＋ 受審的 working-tree 修訂**，
                  **不得**歸給 `69cff79` 一段，也不得歸給任一單獨 commit ——
                  seed chain `8da3729`／`11b2ce1`／`c76a1ff`／`e049284`、
                  initial producer `020e7ec`／`53801a9`、其間的 approved-spec 與
                  node-test-v2 rollout chain、provisional remediation `69cff79`，
                  再加上 HEAD `7f1b346` 之上 2026-09-05 的 phase3／3b／3c working-tree 修正。
                  受審檔案的 SHA-256 逐一列於該複審文件（14 個檔案），
                  其中含**未 commit** 的修訂；本段**不複製**那些 hash 值，以免造成第二份 authority。
先前判定        : `020e7ec`／`53801a9`／`69cff79` 的 NOT ACCEPT **仍是對其當時候選的正確判定**，
                  逐字保留為歷史；本次接受的是**提交的整條 chain**，不是回改那些判定。
獨立驗證        : clean detached worktree，node --test **1534 tests／1523 passed／11 skipped／
                  0 failed，exit 0**；structure validation 兩段皆過、deterministic eval 7／7，
                  皆 exit 0。細節與 machine receipts 見該複審文件。
六項 rollout 前置條件 : closed set 與語義**完全不變**，計數由 **5／6** 進為 **6／6**
                  —— 第六項（producer）此刻起計為完成。
**仍未接受**    : persisted batch reader／writer、Step 5／6 consumption、
                  artifact emission、ledger、arbiter integration。
                  —— 其中 **persisted batch READER 已在實作中（in progress），尚待獨立複審**
                     （2026-09-05 起；只做 reader 與 version／legacy boundary，
                      **未**啟用 v2 writer、**未**動 product gate）；
                     其餘各項**仍未開始**。**「已實作」不等於「已接受」。**
gate            : `unsupported-populated-inventory` **仍不得解除**；
                  populated inventory **仍不得接受**。
AC118／AC128／AC136／AC137／AC138 : **仍不得宣稱已滿足**；**Phase 2 仍不得宣稱 READY**。
                  **6／6 是 producer 前置條件的計數，不是 Phase 2 READY 的同義詞。**
```

**Reader acceptance addendum（status carrier，非 normative；2026-09-05；status-only，不改任何 normative 要求，不回改上方任何 dated 敘述）** —— **persisted provenance-batch READER 實作已由 Codex 獨立複審 ACCEPT**（`docs/superpowers/reviews/2026-09-05-batch-reader-codex-review.md`；Codex 依使用者委任的獨立複審權作出的 stage decision，**非** panel、**非**使用者當日另一次直接核准）。上方 producer addendum 中「persisted batch READER 已在實作中（in progress），尚待獨立複審」是**該段寫出當下**的狀態，逐字保留；本段只**接續**記錄其後的判定，不回改它。

```
接受範圍        : versioned persisted reader、historical legacy boundary、
                  original-text inventory ingestion；併同 dependency-free canonical
                  primitive 抽出（既有 store re-exports 不變）與三個 scratch-layout
                  dependency list 的窄幅更新。該複審文件內的 producer acceptance
                  status 補述為**非 normative**。
生效規格        : **完全不變** —— shared approved v1.15 ＋ intent-scan approved v1.10
                  ＋ test-provenance approved v1.17。本段**不新增任何 spec authority**，
                  **不**是新的 promotion，**不**需要也**未**取得新的使用者／panel 核准。
歸屬            : HEAD `7f1b346` 之上**完整提交的 working tree**，非任一單獨 commit；
                  受審檔案的 SHA-256（**21 個檔案**，含未 commit 的修訂）逐一列於該複審
                  文件，與 `reader-candidate-verification.json` 的 candidateFiles **同一集合**。
                  本段**不複製**那些 hash 值，以免造成第二份 authority。
                  —— 該複審文件**本身不在**受審 manifest 內（它在該次執行**之後**才建立，
                     文件自身已載明），故**不得**被計入這 21 個檔案。
獨立驗證        : clean detached worktree，node --test **1556 tests／1545 passed／
                  11 skipped／0 failed，exit 0**；structure validation 兩段皆過、
                  deterministic eval 7／7，皆 exit 0。machine receipts 見該複審文件。
六項 rollout 前置條件 : closed set、語義與計數**皆不變**，仍為 **6／6** ——
                  本次接受的是 reader，**不是**第七項前置條件，也**不**改變該計數的意義。
**仍未接受**    : persisted batch **WRITER**、Step 5／6 consumption、artifact emission、
                  ledger、arbiter integration。
                  —— 複審判定當下的 writer **仍是歷史 writer**；啟用 v2 write 是**下一個**
                     被授權的 slice。該 writer slice **已在實作中（in progress）、尚待獨立
                     複審**（2026-09-05 起）。**「已實作」不等於「已接受」。**
                  —— exact result-to-entry coverage、transaction-time captured-file CAS、
                     actual raw payload ingestion、successor／resolution atomicity 與
                     下游 Step 6 consumption 均為**各自獨立**的義務，本次一項都未接受。
                  —— `batchInventoryPreimage` 只在 store／record 驗證**之後**被使用；
                     **一個 getter 本身不是 validation，也不是 freshness proof。**
gate            : `unsupported-populated-inventory` **仍不得解除**；
                  populated inventory **仍不得接受**；**無任何 product／consumer 接受**。
AC118／AC128／AC136／AC137／AC138 : **仍不得宣稱已滿足**；**Phase 2 仍不得宣稱 READY**。
```
...
167. **Residual-side exclusivity 的 executable matrix**（carrier：§6 rule 3 ＋ §2 結構重整段 ＋ §11b.8）：本 AC 的每一格都必須以真實可執行的 case 覆蓋，且必須抓得出 **commit `898f81c` 的 provisional matcher**（它對第 3–8 格輸出 added＋deleted，因而不符）。**單側 residual（正例）**：(1) 只有 `Uhead` → 每筆 added；(2) 只有 `Ubase` → 每筆 deleted。**兩側 residual —— evidence shape（一律 fail-closed；四類，**不是**每格只改動一個變數）**：**(a) path-irrelevance discriminator pair**：(3) 與 (4) —— 兩格的判別維度是 **path relation**（同 path vs 不同 path），用以證明 path 相同或不同**都不改變**雙側 residual 的 fail-closed 結果；此處**不**宣稱兩份 fixture 逐 byte 只差一個欄位，只宣稱被比較的維度是 path relation。**(b) payload-irrelevance compound stress**：(5) —— 相對 (3) **刻意同時改變 `tag` 與 `bodyDigest` 兩個 payload signal**，**明確不是** single-variable case；用以證明 tag／body 相同或不同**都不得**被拿來猜 identity。**(c) residual-cardinality cases**：(6) 與 (7) —— 判別維度是 residual 的**基數**（1:1 vs many:many），用以證明 1:1 或 many:many **都不構成** identity evidence；**不**宣稱這兩格是 literal single-variable fixture pair，它們是 cardinality boundary case。**(d) no-partial-result compound stress**：(8) —— 在雙側 residual 之外**另外**加入一組成立的 exact pair 與一組成立的 moved pair，**明確是 compound case、不是 single-variable**；用以證明整輪 fail-closed，**已成立的 pair 也不得外洩**。以下為各格的輸入與預期結果：(3) 同 path、`tag` 與 `bodyDigest` 完全相同、`s:` structuralId 不同 → fail-closed；(4) 不同 path、`tag` 與 `bodyDigest` 完全相同、structuralId 不同 → fail-closed；(5) 同 path 且 `tag` 與 `bodyDigest` **也**改變、structuralId 不同 → **仍** fail-closed（證明結果不因 tag／body 是否相同而改變）；(6) 恰一筆 base residual ＋ 恰一筆 head residual，**即使看起來就是 genuine delete ＋ add** → fail-closed（證明不因 1:1 就猜成同一 declaration）；(7) 多筆 base residual ＋ 多筆 head residual → fail-closed；(8) 已有成立的 exact／moved pair，另**同時**留下兩側 residual → **整輪** fail-closed，**且不得回傳那些已成立的 pair**。**`@tid` 邊界**：(9) 同一 `tid:` 在 base／head 各一次且 path 相同 → 合法 exact pair；(10) 同一 `tid:` 在 base／head 各一次且 path 不同 → 合法 moved pair；(11) base 為 `s:…`、**只在 head 新增** `tid:…`（path、`tag`、`bodyDigest` 皆不變）→ **fail-closed**，**不得** adoption、**不得**因 path／`tag`／`bodyDigest`／宣告名相同而自動建立 `s:` → `tid:` alias；(12) base 與 head **兩側都已**帶同一 `tid:`，同時 container 改名 → `structuralId` 兩側皆為該 `tid:`、不受改名影響，配對成功。**拆分是可達路徑（必須實跑，不得只以散文宣稱）**：(13) 一次真正的 delete ＋ add 拆成兩個以不同 base 為界的 run —— 第一輪只剩 `Ubase` → deleted；以其結果為新 base 重跑，第二輪只剩 `Uhead` → added。**通則**：(14) 上列任一 fail-closed 都**不得** emit partial inventory 或 partial pairing result。本 AC **不**新增 `modified`／`retagged` 分類規則，**不**新增 alias carrier、migration record 或 persisted 欄位。**本次（approved v1.14）只更正 evidence-shape 敘述**：上列 14 格的輸入、expected outcome、residual-side exclusivity、`E_UNRESOLVED_IDENTITY_DRIFT`、no-partial-result、`@tid` 邊界與 split-run 可達路徑**一律不變**；**不得**再把 (3)–(8) 整體描述為「每格只改動一個變數」。
...
170. **Approval／implementation boundary（test-provenance v1.14）**（carrier：本文件 live status ＋ §11b.12 rollout boundary）：**v1.14 已於 2026-08-16 由使用者明確核准**（前置 draft 內容已由獨立 Codex 審查並 ACCEPT；**本次未執行 agent-duel panel，因此不稱為 panel 放行**）；前一 approved baseline 為 **test-provenance approved v1.13**，**current approved coupled set 現為 shared approved v1.14 ＋ intent-scan approved v1.10 ＋ test-provenance approved v1.14**，三者為同一生效集合，不得分開採用。**此 approval 只涵蓋 spec authority。****本次 promotion 不修改、不修正也不接受任何 implementation。****populated inventory producer 尚未實作**。**在 v1.14 promotion 當下**，canonical v2 reader 尚未實作本版新增的兩條 observable invariant（`modified` ⇒ 兩個 body digest 不同；`retagged` ⇒ 兩個 body digest 相同且 tag 不同），其既有 acceptance 不撤回，但仍須另行 remediation 與獨立審查；**這項 promotion-time boundary 仍是正確歷史，不得倒寫**。**promotion 之後**（status，非 normative；截至 2026-08-16）：commit `9f42628` 已實作兩條 invariant、補齊 AC169 reader cases (16)–(20)，並通過獨立審查 ACCEPT。**這是後續 implementation acceptance，不是 v1.14 promotion 自動造成的。**既有的 base／head one-to-one matcher、`AdapterContentView`、`DiscoveryAnalysisPreimage` 與 S3 source-freshness component 的 acceptance **一律不撤回**。**仍未完成**：populated inventory producer、governance reverse closure、artifact emission，以及 Step 5／6、ledger、arbiter wiring。因此：**不得**接受 populated inventory；**不得**解除 `unsupported-populated-inventory`；**AC118／AC136／AC137／AC138 一律不得宣稱已滿足**；**Phase 2 不得宣稱 READY**；**AC1–AC166 與 AC168 byte-identical**（含 AC27、AC33、AC36、AC104、AC109、AC110、AC138、AC147、AC155、AC165、AC166、AC168）；**AC167 僅更正其 evidence-shape overclaim**，其 cases、expected results 與 matcher contract **一律不變**。
171. **`GovernanceSeedPreimage` 的 exact carrier、store capture、lifecycle derivation 與 anti-injection**（carrier：§11b.10c ＋ shared approved v1.15 §9「v1.15 lifecycle seed derivation」＋ shared approved v1.15 §2 `Source.expiry`）：本 AC 定義的是**未來 component 的 executable contract**；**本 AC 已隨 shared approved v1.15 與本文件 approved v1.15 的 2026-08-21 同步 promotion 生效；契約生效**不代表**下列任何一格已有測試、已實作或已通過**。**(A) carrier 形狀**：(i) **request exact key set 恰為 `{ repoRoot, baseTreeOid }`（v1.17；v1.16 曾誤加 `taskId`，該規則撤回）** —— 多一 key 即 fail-closed，**`taskId` 本身也在拒絕之列**，第二個 argument 一律拒絕。**理由**：本 operation 不解析 task、不讀 `taskStates`，否則下方 (B)(ix)／(xi) 的 current store 缺席與 canonical-empty 兩格**在 public API 上不可達**（實測：兩案皆先停在 unknown-task fail-closed）。**`taskId` 屬於 `produceChangedTestInventoryV2()` 的 request（見 AC176 (15)(16)），不屬於本 operation**；(ii) **result exact key set 恰為 `{ baseTreeOid, headViewDigest, inputProvenanceStoreDigest, lifecycleAffectedClauses }`** —— 多一 key、缺一 key 或任何自由格式 metadata 皆 fail-closed；(iii) **合法 fixture 的 output assertion**：對一份**合法** repository fixture，emitted `lifecycleAffectedClauses` 的**每一項**皆吻合 §2 canonical clauseRef grammar —— 斷言施加於**輸出**，**不是**把破壞過的 result 交給 runtime 再期待 reject（該讀法無入口，且與 (vi) 自相矛盾，見 §11b.10c 的 test-only evidence shape）；**上游負例必須標明層級**：persisted store 內含不吻合上游 ULID grammar 的 clause id 者，於 **G1 parse／schema validation 層**依 (xii) 拒絕，**不得**宣稱由 carrier guard 拒絕；(iv) **strict ordering 的 output assertion**：輸出依 Unicode code point **嚴格遞增**，斷言施加於輸出；(v) **no-duplicate 的 output assertion**：輸出**不含**重複項（union 依 (xxiv) 去重），斷言施加於輸出；(iii)–(v) 若實作另行攜帶內部 self-check，**唯一**授權的觀察方式是 §11b.10c **形狀 B** 的 scratch source copy 單點突變，**不得**為此新增任何 public API 入口；(vi) **deep-freeze assertion**：root 與所有 nested value **深度 frozen** —— 斷言寫入嘗試 throw 或**無效果**，且嘗試之後 carrier 仍為 canonical 且與嘗試前**逐欄相同**；**不得**以 result mutation 宣稱抵達任何 rejection branch。**(B) store capture 與 version matrix**：(vii) **G1 的同一份 parsed store** 同時供 validation、`inputProvenanceStoreDigest` 與 seed derivation —— 必須有可執行證據顯示三者之間**沒有第二次讀取**。**evidence shape（定案）**：§11b.10c **形狀 B** —— 於獨立 child process 內對 scratch source copy **只**把固定的 internal store-loader import 指向一個 **counting proxy**，該 proxy 記錄 **ordered load trace**（每次 current-store fresh-load 一筆），其餘一律不動；以正常 `{ repoRoot, baseTreeOid }` 呼叫。**成功路徑的期望恰為兩筆**：(1) closure 之前的 **G1** fresh-load 恰一次；(2) validation、`inputProvenanceStoreDigest` 與 seed derivation **三者之間 0 筆**；(3) closure 之後的 **G2** fresh-load 恰一次。trace 總數 **恰為 2（G1 ＋ G2）**，順序必須是 G1 在前、G2 在後；**任何中間重讀**（總數 >2，或兩者之間出現第 3 筆）該測試**必須** fail。counting proxy **只**存在於 scratch source copy —— **不得**新增 production export、request key、loader provider、environment argument 或任何 production seam，production module 與 public API **一字不動**；(viii) **缺檔**時 digest 取既有 canonical empty store digest（**不是** null、不是空字串），記法為 `sha256(canonicalText(…))`，**單變數負例**：同一份 store 只加 BOM、或只把 LF 改成 CRLF，digest **必須不變** —— 會變者即誤用 raw-bytes 記法，**拒絕**；(ix) **current store（`C`）三案**（**v1.17**：本 operation 不解析 task，因此缺席與 canonical-empty 兩案**必須**經 public API 可達並成功 —— 若實作於此處要求 `taskId` 或讀 `taskStates`，本格**必須** fail）：缺席 → canonical empty v2；`provenanceVersion: 2` → 正常；**current v1 → fail-closed**，且 producer **不得** migration、**不得**回寫；另驗其他版本值 → fail-closed；(x) **base store（`B`）三案**：缺席 → canonical empty v2；historical **v1 → 可** read-only 分析；**v2 → 可** read-only 分析；另驗 base 為其他版本值 → fail-closed，且 **historical base v1 不得被誤套 current-v1 的 migration-only 限制**；(xi) **G1／G2 equality 只比 canonical digest，presence 不是獨立訊號** —— **正例 1**：G1 缺席 → G2 為 explicit canonical empty v2 → **通過**；**正例 2**：G1 為 explicit canonical empty v2 → G2 缺席 → **通過**；**正例 3**：兩側皆缺席 → **通過**；**負例 1**：G1 缺席 → G2 為一份**非空**且合法的 v2 store → **fail-closed**，理由必須是 **digest 不同**；**負例 2**：分析途中改動 `.ctide/provenance.json` 使 G2 的 canonical digest != G1 → **fail-closed、no result、no partial carrier、no-write**；(xii) **version／schema 與 digest equality 分層**：G2 為 current v1、malformed 或不支援版本時，失敗理由必須是 **version／schema failure**，**不得**被報成 presence failure 或 digest mismatch；G2 必須套用與 G1 **相同**的 current-store version rule；(xiii) head view 於 S1／S2 之間不穩定 → **fail-closed**。**evidence shape（定案）**：§11b.10c **形狀 B** —— 於獨立 child process 內以 scratch source copy 在 **S1 完成之後、S2 之前**同步修改 scratch repo 內**一個非 excluded 的 head file**（同步施加，**不是**並行寫入），再以正常 `{ repoRoot, baseTreeOid }` 呼叫；必須得到 `head-view-unstable` 或其對應的**穩定 error carrier**，且**no result、no partial carrier、no write**。**不得**依賴 timing race，**不得**對 live repo 並行寫入，**不得**等待任何真實時間窗口；(xiv) base governance state **只能**來自 `baseTreeOid` 指向的 exact tree，以 live current store 冒充 base 者**拒絕**；另驗實作**不得**新增 presence carrier、`storeCarrierState`、任何 persisted field 或 inventory 欄位。**(C) `T0` 與 expiry**：(xv) **T0 恰取樣一次**，位置在 G1 parse／schema validation 完成之後、第一個 time-dependent 判定之前，且依 §11b.10c 的 Node implementation boundary 以 `Date.now()` 取得；**evidence shape（定案；禁止以真實等待跨日或依賴競速代替，那是不穩定證據）** —— 於**獨立 child process** 中執行**兩階段** probe：**(1)** 在 production module `dynamic import` **之前**安裝 process-global `Date.now` wrapper，但保持 `armed == false`；**(2)** import 期間的 clock read 一律回傳固定的 **prelude value**，且**不計入** invocation counter；**(3)** dynamic import 完成後**立即** reset counter 並設 `armed = true`；**(4)** 第一次 armed read 回傳 `T0`；**(5)** 第二次及後續 armed read 回傳**已跨過 expiry boundary 的** `T1`；**(6)** invocation 完成後斷言 **armed read count 恰為 1**，且結果依 `T0` 判為 **not expired**；**(7)** 若實作在 import time 取樣並於 invocation 重用，armed count 會是 **0**，該測試**必須** fail；**(8)** 若實作在 invocation 內重讀時鐘，count 會 **>1** 且 `T1` 會把 membership 變成 expired，該測試**必須** fail；**(9)** wrapper **只**存在於 child-process test harness —— **不得**新增 production export、request key、clock provider、environment argument 或任何 test seam。(xvi) **expiry membership 三案**：`expiryInstant` 在 `T0` **之前** → expired；**恰等於** `T0` → **expired**；**之後** → not expired；(xvii) **expiry grammar 五案（只驗 grammar，不驗 membership）**：`0000-01-01` → **fail**；`1900-02-29` → **fail**（`1900 % 100 == 0` 且 `1900 % 400 != 0`，非閏年）；`2000-02-29` → **pass**；`2024-02-29` → **pass**；`2100-02-29` → **fail**；另驗帶時分秒、帶 timezone offset、帶前後空白、`2026-2-01`、`2026-13-01`、`2026-02-30` → 一律 **fail-closed**，**不得**寬鬆解析、補零或推測時區；(xviii) DEC／ASSUM **不**因 `basisRefs` 含 exception-grant 而進 `expiredClauses`。**(D) 四集合 derivation**：(xix) `B` 與 `C` 相同且無 drift／expiry → **四集合全空**；(xx) `C` 新增一個 Clause → **只**進 `semanticallyChangedClauses`；(xxi) same-ID Clause payload 改變，或 `B` 的 immutable object 在 `C` 中消失 → **fail-closed**（integrity failure，**不是** seed）；(xxii) `C` 新增合法且生效的 Transition → subject 進 `transitionedClauses`，新 successor **另**由 semantic set 納入；invalid／dangling／重複生效 Transition → **fail-closed**；(xxiii) Check B 三案（zero → **drift**；one 且位置已移動 → **not drift**；multiple → anchor-ambiguity observation、**not drift**）、同一 drifted Source 被兩個 current Clause **直接**引用 → **兩者皆納入**、`driftMode=snapshot-only` **永不** drift；(xxiv) 四集合互相 overlap → union **去重**並依 Unicode code point **嚴格遞增**排序，輸出**不含** dpRef、path、reason 或任何 metadata。**(E) anti-injection 逐項拒絕**：prebuilt `DiscoveryAnalysisPreimage`／`GovernanceSeedPreimage`、store bytes、已 parsed 的 store、store digest、`lifecycleAffectedClauses`、`governanceHit` set、reverse-closure 結果、registry／registry path／registry root、parser、ignore matcher、Git executable／參數／environment、filesystem adapter、config object／path、modulePaths、view／snapshot、**clock／now／timestamp／Date provider**、capture hook、component module path、output path，以及任何等價 override、alias、test-only seam 或 environment argument，**一律不得**成為 public API 的入口。
172. **兩份 preimage 的 cross-binding**（carrier：§11b.10c orchestration ＋ shared approved v1.15 §9）：本 AC 定義的是**未來 producer 的 executable contract**；**本 AC 已隨兩份 approved v1.15 的 2026-08-21 同步 promotion 生效；契約生效**不代表**下列任何一格已有測試、已實作或已通過**。**正例**：(1) `discovery.baseTreeOid == governance.baseTreeOid == request.baseTreeOid` → 得以繼續；(2) `discovery.headViewDigest == governance.headViewDigest` → 得以繼續。**單變數負例（各只改動一邊，其餘完全相同）**：(3) 只把其中一邊的 `baseTreeOid` 換成另一個**合法**的 tree OID → **fail-closed**；(4) 只把其中一邊的 `headViewDigest` 換成另一個**合法**的 digest → **fail-closed**。**(3)(4) 的 evidence shape（定案）**：兩份 preimage 都由 producer 在**單次 invocation 內自建**、caller **不得**注入，因此這兩格**沒有合法輸入入口**；**唯一**授權的觀察方式是 §11b.10c 的 **形狀 B** —— 於獨立 child process 內對 disposable scratch source copy 施加**單一點**編輯，使其中一邊取用另一個**合法**值，import 該 copy 後斷言 **cross-binding guard** 拒絕並**指名該 guard**（不得誤稱為上游 schema 層）；**不得**依賴 timing race，**不得**新增 production export、request key、environment argument 或 caller-supplied preimage／registry／seed，production request contract 仍**恰為** `{ repoRoot, baseTreeOid, taskId }`。**兩個負例都必須另外驗**：**不得**回傳任何 matcher pair、任何 entry、任何 envelope（**partial 亦不可**）；**不得**自動挑其中一邊；**不得**只重跑其中一份 preimage 再與另一份拼接 —— 重試只能是一次完整 invocation。另驗：(5) **（v1.16：exact key set 為三欄 `{ repoRoot, baseTreeOid, taskId }`；taskId 是必填事實，不是 injection —— 它不供給任何被觀測的輸入）** producer 的 public API **不接受** prebuilt `DiscoveryAnalysisPreimage`／`GovernanceSeedPreimage`／任何 digest／entries —— 傳入即 fail-closed，且**不得**存在任何等價 seam；(6) `registryDigest` 必須等於 `DiscoveryAnalysisPreimage` 已綁定的值，**單變數負例**：producer 另讀第二份 registry 後與之混用 → **拒絕** —— 同屬**無合法入口**的 defensive branch，evidence shape 同 (3)(4)（§11b.10c **形狀 B**）。**本 AC 的 cases、expected results 與 error boundary 一律不變** —— 本輪更正其 coupled draft authority 引用（`shared v1.14` → `shared approved v1.15`），並補上 (3)(4)(6) 先前**缺漏**的 evidence shape；**新增的只有證據形狀，cases 與 expected results 一格未動**。
173. **Populated inventory producer 的 end-to-end executable matrix**（carrier：§11b.10c ＋ §6 ordered classification ＋ §11b.9c envelope ＋ shared approved v1.15 §9 四集合）：本 AC 定義的是**未來 producer 的 executable contract**；**本 AC 已隨兩份 approved v1.15 的 2026-08-21 同步 promotion 生效；契約生效**不代表**下列任何一格已有測試、已實作或已通過**。**v1.16 註記**：AC175 修復本 AC (a)(c) 的**可達性**（`@src` 自 canonical declaration bytes 移除），**本 AC 的 cases、expected results、evidence shapes 與 error boundary 一格未動**；AC176 另加 binding semantic validation，其失敗一律先於本 AC 的任何 entry 產出。**本 AC 完整承載 AC169 的 producer cases (1)–(15)** —— 其輸入、evidence shape（anchor／control、pairwise precedence、compound stress 三類）與 expected results **一律原樣有效、不得改寫或弱化**；差別**只在**本 AC 要求它們由**真實 producer 端到端**產出 persisted entry，而非只驗 classification 表。另加：**(a) `semanticallyChangedClauses` 的 reachability 與 precedence（明確**不是** governance-only）** —— 唯一合法的 fixture 形狀：`B` 已有舊 Clause `A` 且 base 側該 test 綁 `A`；`C` 新增 Clause `X`；**同一個** logical test 在 head 側改綁 `X`；body **完全相同**。此時 `X ∈ semanticallyChangedClauses`，`governanceHit` 為 **true**，但 tag 由 `A` 變成 `X`，依 §6 precedence 第 5 格必須產出**恰一筆** `retagged`，`reason == content-change`，兩個 body digest **相等**；**不得**另產 `governance-affected` entry。**control**：不新增 Clause、tag 與 body 都不變 → **自 entries 省略**。可另加一個 **head-only** test 綁新 Clause `X` → 分類為 `added`，同樣**不得**另產 `governance-affected` entry。**reachability boundary（必須明寫）**：在 **valid base binding** 之下，**不存在**「新 Clause ＋ base／head tag 皆不變」的 governance-only fixture —— 新 Clause 不在 `B`，base 側 tag 指向它即是 **dangling preChangeBinding**，而 shared 要求 clause 形態的 pre-binding 必須在 `B` 可解析（§9 綁定兩相：`preChangeBinding ＝ clause binding` 時 clause 與 Source 必須**可解析** ∧ Check A 成立）。這是 **reachability boundary**，**不代表** `semanticallyChangedClauses` 不存在或無效，**不得**據此把它自 seed union 移除。**(b) 兩個真正可達的 governance-only case（same-tag、same-body）**（**v1.17**：前一稿列三個，**(b3) 已撤回**，見 (b0)）—— 同一個 body 與 tag 皆未變的 same-path pair，分別因 (b1) `transitionedClauses`（`C` 新增一筆生效 Transition，其 `subject` 正是該 test head 側 binding 的 `clauseRef`）、(b2) `driftedClauses`（該 clause 直接引用的 `driftMode=repo-file` Source 對 `H` 的 Check B 得 zero occurrence）而命中 → **兩者各自**產生 `governance-affected`，且 `reason == governance-affected`、兩個 body digest **相等**、`tagBefore` 與 `tagAfter` canonically **相等**、每格**只有一筆** entry；**單變數對照**：兩格各自搭配一個「seed 未命中、其餘完全相同」的 control，該 control 必須**自 entries 省略**。**fixture 的合法性前提（v1.17；不滿足則 store 本身不合法，測到的不是本格）**：(b1)(b2) 的 clause **不得**是任何 DP 的 current terminal —— 上游 **INV-4** 要求 terminal 必須 active ∧ applicable，retire 會使 (b1) 的 store 在 clock-armed validation 即遭 `E_INV4_NOT_APPLICABLE` 拒絕；且該 clause **不得** exception-backed，否則 head binding 落入 §7，與本格無關。**兩格皆以非 exception-backed、非 DP-terminal 的 clause 構造，並以裸 clause-form tag 綁定。****(b0) 與 AC176 (11) 的關係，以及 (b3) 的撤回（v1.17；必讀）** —— (b1)(b2)(b3) 的命中條件**正好就是** postChangeBinding 的失敗條件：(b1) 使 ob-3（active）不成立、(b2) 使 ob-5（Check B）不成立、(b3) 使 ob-8（expiry）不成立。在 v1.16 之下**三格全部不可達**（**實測**：(b1) head-binding-inactive、(b3) head-binding-not-applicable；(b2) 僅在漏驗 Check B 的實作下才「通過」）。**v1.16 稿曾宣稱 (b2) 不受影響，該宣稱撤回。** (b1)(b2) 自 v1.17 起經 §11b.10c 「Governance-affected witness carve-out」回到可達：ob-3／ob-5 **只有**在**同一 G1／H／T0** 導出的**對應**集合（依序 `transitionedClauses`／`driftedClauses`）命中時才豁免，**必須逐項對應，不得以 `lifecycleAffectedClauses` union 代替**。**(b3) 撤回（v1.17；不是延後，是撤回）**：`expiredClauses` 只收 exception-backed REQ，而 exception-backed REQ 的 head binding **必然**經 §7，§7 課 `applicable(REQ, DP)`；expiry 令 `mechanicallyApplicable` 為 false，`applicable` 因此為 false → §7 fail-closed，**而 §7 不在 carve-out 之內**。更早一步：上游 **INV-4** 要求 DP 的 current terminal **必須 active ∧ applicable**，因 exception expiry 失效者**必須** repoint 或 reopen —— 因此「DP 仍以該過期 REQ 為 terminal」的 G1 **本身就不合法**（**實測**：clock-armed validation 得 `E_INV4_NOT_APPLICABLE`）。**沒有**同時通過 authoritative G1 validation 與 §7 的 (b3) fixture 存在，因此本 AC **撤回 (b3)**，**不得**以豁免 `applicable` 整項或放寬 INV-4 來把它救回。witness-3 仍有一個可達的**負例**，但它在 DEC／ASSUM 側，見 AC176 (11d-4)。**(b0-neg) 本 AC 要求的判別負例（各一，共兩個；取代前一稿要求的三個 cross-witness 負例 —— 那三個之中有兩個不可構造，該要求撤回）**：**(i)** clause 在 `B` 就已 inactive、`C` **沒有**新增生效 Transition → 不在 `transitionedClauses` → **fail-closed**；**(ii)** 同一 fixture 再讓該 clause 的 Source drift（於是只有 witness-2）→ **仍須 fail-closed**，因為 ob-3 的 witness 是 witness-1；以 union 豁免的實作會錯誤放行，**必須** fail 本格。**豁免不等於 binding 有效**：本 AC **不得**據此宣稱該 test 已通過 gate，`.ctide/output/**` 仍不得寫出，`unsupported-populated-inventory` 仍不得解除。**本 AC 不宣稱四種 seed 各有 governance-only case** —— 可達的只有 (b1)(b2) 兩個：`semanticallyChangedClauses` 的理由見 (a) 的 reachability boundary，`expiredClauses` 的理由見 (b0)。**(c) precedence 不被 governance 覆蓋** —— body change ＋ governance hit → `modified`；tag-only change ＋ governance hit → `retagged`；moved ＋ governance hit → `moved`，三者 `reason` 皆為 `content-change`；**(d)** `unchanged` **完全省略**，**不得** emit placeholder；**(e)** 任一 logical test **不得**出現兩筆 entry，也**不得**帶第二個 status —— reverse closure **不得**為已由 `added`／`deleted`／`moved`／`modified`／`retagged` 收錄者再新增一筆；**(f) 六種 status 的 exact side projection 逐欄驗** —— `added` 全取 head、`deleted` 全取 base，其餘四種 `testRef`／`framework`／`implementationIdentity` 取 head 而 `tagBefore`／`baseBodyDigest` 取 base、`tagAfter`／`headBodyDigest` 取 head；且 `added` 的 `baseBodyDigest`、`deleted` 的 `headBodyDigest` **必須缺席**（**不是** null）；**(g)** entries 依 `(path, adapterId, structuralId)` code-point tuple **嚴格遞增**，nested object 依既有 canonical key ordering；**(h)** envelope **exact key set 恰為七欄**，且 `inventoryDigest` 以 §11b.9c 的唯一公式重算後相等 —— **不得**出現 `evaluationTime`／`producedAt`／`clockDigest` 或任何等價欄位，`inventoryVersion` 仍為 `2`；**(i)** producer 的輸出必須能通過 `parseCanonicalInventoryV2`；**(j)** **不得**以「產品 `parseInventory()` 已放行」作為成功證據 —— 產品入口**仍必須**以 `unsupported-populated-inventory` 拒絕同一份文件，兩者**同時**成立才算通過；**(k)** producer invocation 前後，`.ctide/provenance.json`、explicit config、`test-adapters.json` 與 `.ctide/output/**` 的 bytes **逐一不變**（含 store 未被 migration、未被回寫）。
174. **Approval／implementation boundary（coupled shared approved v1.15 ＋ test-provenance approved v1.15）**（carrier：本文件 live status ＋ shared approved v1.15 live status ＋ §11b.12 rollout boundary）：**兩份 spec 已於 2026-08-21 由使用者明確核准並同步 promotion** —— 前置 draft 內容已由**獨立 Codex 審查並 ACCEPT**；**本次未執行 agent-duel panel，因此不得稱為 panel 放行**。**兩者是同一次核准：任一份單獨採用皆不成立。** **前一 approved baseline 為 shared approved v1.14 與 test-provenance approved v1.14**，未撤回、未取代其歷史記載。**current approved coupled set 現為 shared approved v1.15 ＋ intent-scan approved v1.10 ＋ test-provenance approved v1.15**，三者為同一生效集合，**不得分開採用**。**本次 promotion 只使 spec authority 正式生效，不代表任何 implementation 已完成或被接受**：**populated inventory producer 未實作、未接受**、**`GovernanceSeedPreimage` 未實作、未接受**、**shared 的 v1.15 lifecycle seed derivation 未實作**、**governance reverse closure 仍未完成**、**shared approved v1.15 §2 所要求的既有 writer exact expiry grammar remediation 尚未完成**、**artifact emission 與 Step 5／6、ledger、arbiter wiring 仍未完成**；契約生效**不等於**已實作，**不得**以本次 promotion 宣稱上列任何一項自動完成。既有的 base／head one-to-one matcher、`AdapterContentView`、`DiscoveryAnalysisPreimage`、canonical v2 reader 與 S3 source-freshness component 的 acceptance **一律不撤回**；該等 acceptance 屬於**經 remediation 收斂後的 component**，**不歸功於其 introduction commit 單獨**。**六項 rollout 前置條件的 closed set 與「五項完成、只剩 producer」的計數不變** —— `GovernanceSeedPreimage` 與 reverse closure 都是 producer 的內部 stage／mandatory supporting input component，**不是**第七項 prerequisite。**本次不新增任何 persisted 欄位**：`GovernanceSeedPreimage` 恰四欄、`ChangedTestInventoryV2` 恰七欄、`inventoryVersion` 仍為 `2`、`inventoryDigest` 公式不變、`T0` **不** persisted；日後若要持久化 clock witness，**必須另立 inventory schema／version**。**producer 的 public request 仍恰為 `{ repoRoot, baseTreeOid }`，§11b.10c 的 anti-injection contract 一字不變。**因此：**不得**接受 populated inventory；**不得**解除 `unsupported-populated-inventory`；**AC118／AC136／AC137／AC138 一律不得宣稱已滿足**；**Phase 2 不得宣稱 READY**；**AC1–AC170 逐條 byte-identical**（含 AC116、AC118、AC129、AC138、AC165、AC167、AC169、AC170）；**AC171–AC173 的 cases、expected results、evidence shapes 與 error boundary 一格未動**，本次**只**更新其 authority label 與已過時的 draft freeze 語句；本次**不新增 AC175**。
175. **Canonical declaration bytes 排除合法附著的 `@src`（v1.16）**（carrier：§11b.8b ＋ §11b.8c）：本 AC 定義的是**未來 adapter／producer 的 executable contract**；**本 AC 已隨 test-provenance approved v1.16 的 2026-08-22 promotion 生效；契約生效**不代表**下列任何一格已有測試、已實作或已通過**。**(1) 正例**：同一個 declaration，唯一差異是合法附著的 `@src` 行由 `REQ-<ULID-A>` 改為 `REQ-<ULID-X>` —— 兩側 canonical declaration bytes **逐位元組相同**，`bodyDigest` **相等**。**(2) 同一演算法**：`@tid` 與 `@src` 於**同一次**逐行掃描中移除；**不得**存在第二個 range、第二個演算法或第二個 digest —— 驗實作**不得**新增任何以 `@src` 為條件的分支輸出。**(3) 一般註解保留**：一行 `// 這裡提到 @src 但不是 directive`（不符 §11b.8c exact lexical form，因此**不是**合法 directive）**必須保留**在 canonical bytes 內 —— 只把它改掉即使 `bodyDigest` 改變。**(4) malformed 與 unattached 不移除**：`//@src` 無空白、`// @src  TOKEN` 雙空白、`// @src TOKEN 尾隨內容`、以及與 declaration 之間隔了空白行的 `@src` —— 依 §11b.8c **已 fail-closed**，永不走到 hashing；本 AC **不**放寬其中任何一條。**(5) CRLF 不變性**：同一份邏輯內容以 LF 與 CRLF 各存一份，經 §11b.8b 步驟 0 後所有 digest **相等** —— 新增的 `@src` 移除**不得**破壞此既有不變量。**(6) 精確的下游後果**：在 (1) 的 fixture 上，§6 precedence 第 4 格**不**命中、第 5 格 `retagged` **命中**，且 persisted entry 的兩個 body digest **相等** —— 這是 **AC169 (2) 與 AC173 (a)(c) 的可達性修復**，**不改變**該兩條 AC 的任何 case、expected result、evidence shape 或 error boundary。**(7) 不得繞道**：**不得**以 producer 特例、第二份 digest、tag-aware 比較或分類時的 ad-hoc 例外達成 (6) —— 若 (1) 的兩個 digest 實際不相等而分類仍報 `retagged`，該實作**必須** fail 本 AC。**(8) implementation identity 必須升版**：本 AC 的 fixture 必須驗 `implementationIdentity.implementationId` 為 **`node-test-v2`**（v1.16 的**唯一** current mapping —— 本版**不**接受「或另一個新 identity」的說法），且 §11b.3 registry 的 exact binding 與 `registryDigest` 同步改變；**負例**：兩套 digest 演算法共用 `node-test-v1` → **必須 fail** 本 AC。**(8c) 四者一致才算升版（executable）**：`node-test-v2` 的 fixture 必須驗 **formal registry、vendor manifest、compiled closed mapping 與 executable component 四者的 `implementationId` 完全一致**；**負例**：registry 改為 `node-test-v2` 而 vendor manifest 仍為 `node-test-v1`（或任一組合不一致）→ **必須 fail-closed**，且**不得**被報成「已升版」。**(8d) spec promotion 不等於 artifact 升版**：驗 v1.16 promotion **本身**不改變 `test-adapters.json` 或 `vendor-manifest.json` 的 bytes —— 兩者在 promotion 前後**逐位元組不變**，且其 `implementationId` 仍為 `node-test-v1` 直到另有一次獨立授權與 implementation review 為止；**不得**以 spec 已 promote 作為 identity 已升版或已接受的證據。**(8a) 凍結 AC 的 identity token 隨 registry 移動（不回改凍結列）**：AC28、AC86、AC89 與 AC90 逐字寫著 `node-test-v1`，它們是 **approved v1.15 的凍結敘述，本版一字不改**（AC1–AC170 byte-identical）。在 v1.16 之下，**其 identity token 讀作 `node-test-v2`**：那四條的 case、輸入、expected result 與 error boundary **一格未動**，改變的**只有**它們所綁定的 current identity 名稱，而該名稱的唯一 authority 是 §11b.3 的 live closed enum、exact binding table 與 discovery binding authority（本版已於三處閉合 `node-test-v2`）。**驗**：v1.16 的 registry 之下重跑那四條，其 identity 斷言**必須**讀到 `node-test-v2`；讀到 `node-test-v1` 即表示 current mapping 未真正升版 → **必須 fail** 本 AC。**(9) 舊 persisted inventory 的 freshness**：一份以舊演算法（`@src` 入 digest、`node-test-v1`）產生的 v2 inventory **可能存在**，本 AC **不**宣稱不存在；驗其在新 registry 之下**不得**充當 current freshness proof —— 必須經既有 S3／registry mismatch 路徑**拒絕**，訊息指向 `registryDigest`／`implementationIdentity` 不符並要求**重建**；同時驗其作為**歷史 bytes** 仍可由 **canonical v2 reader**（`parseCanonicalInventoryV2`）解析 —— 那是一份**舊演算法產生的 v2 inventory**，**不是** legacy v1 envelope，因此**不得**把它描述成 legacy reader／legacy-v1 reader 的成功路徑。**不得**為此定義任何 migration。
176. **Binding semantic validation（v1.16）**（carrier：§11b.10c 「Binding semantic validation」＋ §2 binding 兩相 ＋ shared approved v1.15 §9）：本 AC 定義的是**未來 producer 的 executable contract**；**本 AC 已隨 test-provenance approved v1.16 的 2026-08-22 promotion 生效；契約生效**不代表**下列任何一格已有測試、已實作或已通過**。**(1) dangling head binding**：head 側 tag 為 `{ clauseRef }`，而該 clauseRef 在 **C** 中**不可解析** → **fail-closed**，**no result、no partial carrier、no entries、no-write**；錯誤必須指名 testRef、該 binding，並指名這是 **head** 側。**單變數對照**：其餘完全相同、clauseRef 改為一個在 C 中**滿足 (11) 全部 post-binding 義務**的 clause（可解析 ∧ `active ∧ mechanicallyApplicable` ∧ Source Check A/B ∧ exception chain 有效）→ 正常產出。**此對照與 (11) 共用同一個成功門檻** —— 本 AC **不得**存在兩套寬嚴不同的 positive 標準。**(2) 已知的實測反例**（本 AC 必須抓到）：一個 head-only test 綁 `REQ-<一個在 C 中不存在的合法 ULID>` —— v1.16 之前的 producer 輸出 `added`；本 AC 之下**必須** fail-closed（ob-1）。**fixture 更正（v1.17）**：原稿寫「repo 內**完全沒有** provenance store」，該 fixture 在三欄 producer request 之下**不可達** —— 空 store 沒有任何 TaskState，invocation 先停在 **unknown task**（AC176 (15)），根本到不了 binding 驗證，因此**證明不了本格**。**更正後的 fixture**：`C` 為一份**合法且存在**的 v2 store，其中**恰有一個** TaskState 的 `taskId` 與 request 相符、`baseProvenance.treeOid == request.baseTreeOid`，但該 store **不含**該 clauseRef。**必須額外斷言拒絕的層級是 binding 驗證（ob-1），不是 task resolution** —— 只斷言「失敗了」不合格。**(3) head binding 指向非 active clause**（**v1.17 收斂**：前一稿寫「非 active 一律 fail」，與 (11c) 的 witness carve-out **正面矛盾**，該寫法撤回）：clauseRef 在 C 中可解析但**非 active**（已 retire／已被 supersede）**且該 clauseRef 不在同一 G1／H／T0 導出的 `transitionedClauses` 之中** → **fail-closed**。**有** matching witness 時依 (11c) 正常分類，**不得** fail-closed。status 依 §2／§9 既有推導，**不得**另立第二套判準；witness 的取得與比對規則依 §11b.10c carve-out，**不得**在此另立。**(4) REQ@DP 形態**：其 REQ 與 DP 皆須在 C 中可解析，且該 DP 的 **current terminal 必須正是該 REQ**；不符 → **fail-closed**，**不得**降級為 `{ expl: true }`、**不得**省略該 entry。**單變數對照**：DP 的 current terminal 正是該 REQ，**且** §7 其餘四項（DP ∈ currentTaskDpIds、status == resolved、applicable(REQ, DP)、scopeRulingRef.subjectRef == DP）**與** (11) 的全部 post-binding 義務**皆成立** → 正常產出。**明文禁止**：**不得**單以「current terminal 相等」推出正常產出 —— 那會與 (12) 的五項門檻互相矛盾。**(5) 歷史 base binding 不得被現時效力誤擋**：base 側 tag 指向的 clause 在 **B** 中可解析、其 Source 在 B 中可解析、**且 Check A 成立**（即 (10) 的 pre-binding 義務**全部**滿足），但在 **C** 中已 retire／已 supersede／已 expire → **必須正常產出**，**不得** fail-closed。base binding 是歷史事實；現時效力**只**決定 `governanceHit`，**不**決定 base binding 是否合法。**(5-req) 本正例的獨立前提（v1.17 修訂 2；必須寫明，否則會被讀成「只要前態是歷史事實就一路放行」）**：本格的「必須正常產出」**只**免除對 **base 側**課現時效力；該 logical test 的**所有其他**要求**仍須各自成立** —— head 側若存在（例如**合法 retag**），其 binding **必須**通過 (11) 的**全部**十項義務，且若為 REQ@DP 形態還須通過 §7 五項；head 側**不存在**（`deleted`）時依 shared §9 INV-B1／B2 **無 post 驗證**；**G1 本身仍必須通過完整驗證**。**(6) dangling base binding**：base 側 clause-form tag 在 **B** 中**不可解析** → **fail-closed**，錯誤指名這是 **base** 側。**(7) `{ expl: true }` 與 `null`**：不進行 clause 解析，其既有 §2 規則不變 —— **不得**因本 AC 而被新拒絕。**(8) 快照與讀取邊界**：post-binding 驗證**必須**使用 §11b.10c step 1 的**同一份 G1 parsed** store，**不得**重讀 —— 可執行證據沿用 §11b.10c 形狀 B 的 counting proxy：成功 invocation 的 ordered load trace **仍恰為 G1、G2 兩筆**。**(8b) G1 → binding → G2 的 ordering evidence（總讀取數不足以證明）**：AC171 (vii) 與上方 (8) 只證明成功 invocation 的 current-store fresh-load **總數恰為 2**；那與「binding validation 發生在 G2 之後」**完全相容**，因此**不足以**證明本節要求的順序。**另加一格**，evidence shape 為既有 §11b.10c **形狀 B**：於獨立 child process 內對 scratch source copy 施加**單一點**編輯，在 **binding validation 完成之後、G2 fresh-load 之前**同步改動 scratch repo 的 `.ctide/provenance.json`（同步施加，**不是**並行寫入）。**期望**：該 invocation **必須**由 **G2 得到 store-moved fail-closed**（`G1 != G2` 的 digest 不等），且 **no result、no partial carrier、no entries、no envelope**。**這格為什麼有證據力**：若實作把 G2 放在 binding validation **之前**，這個改動會落在 G2 之後，G2 便看不到它、invocation 會**成功**回傳 —— 該測試因此 fail。**不得**依賴 timing race、**不得**對 live repo 並行寫入、**不得**新增任何 production seam。**(9) carrier 不得擴張，且 private context 不得公開**：`GovernanceSeedPreimage` 仍**恰四欄**、`ChangedTestInventoryV2` 仍**恰七欄**、§11b.10c anti-injection 清單**一字不變**；**request key set 依 v1.17 分開驗**：`produceChangedTestInventoryV2` **恰為** `{ repoRoot, baseTreeOid, taskId }`、`buildGovernanceSeedPreimage` **恰為** `{ repoRoot, baseTreeOid }`。parsed `B`／G1／選定的 TaskState／T0 只在 invocation 內部保留，**不得**成為 public operation、persisted object 或 caller-supplied 入口。**(9b) 共用實作的形狀（v1.17；新增，必驗）**：兩個 operation 之間的共用**只能**是 **module-private、未 export、不接受 caller-supplied callback** 的 helper。**可執行斷言**：對 production module 的 export 清單逐一檢查 —— **不得**存在任何 export 會把 `B`、G1、選定的 TaskState、T0 或 lifecycle seed 交給 caller 提供的函式。**已知實作反例（本格必須抓到）**：`69cff79` 的 `export async function runWithGovernanceContext(request, argumentCount, operation, work)` 將 `{ base, current, taskState, t0, snapshot, lifecycleAffectedClauses }` 整包交給 caller callback —— **該形狀不合格**，它同時是新的 public operation 與現成的 test seam。**(9b-layout) 受檢對象（v1.17 修訂 2）**：共用實作**只能**是**一個** common internal implementation module，同時擁有**兩個 logical operation** 與那個 private context helper；既有 public facade 路徑（`governance-seed-preimage.mjs`、`changed-test-inventory-producer.mjs`）**保留不變**。**export 檢查必須同時涵蓋兩個 facade 與該 common implementation** —— 三者的 export 清單皆**不得**出現任何 context、callback 或 capture 入口。**明文不算違規**：error class，以及**既有**的 pure authority primitive（`statusOf`、`checkSourceIntegrity`、`parseCanonicalExpiry` 等）**不**構成第三個 snapshot-taking operation —— 它們**不讀** store。**另驗**：producer **不得**呼叫已完成 G2 的 public `buildGovernanceSeedPreimage()` 取得 seed —— 該路徑會把 binding validation 推到 G2 之後（§11b.10c 順序），且它不回傳選定的 TaskState。**(10) pre-binding 完整委派**：驗 base 側 clause binding 只課 shared §9 preChangeBinding 列的**全部**條件（clause 與 Source 在 B 中可解析 ∧ **Check A**）；**負例**：Check A 不成立（B 中 Source 的 digest 與 excerpt 不符）→ **fail-closed**。**明文正例**：base clause 在 C 中已 retire／已 supersede／已 expire、Source 已 drift、exception 已過期 —— **全部必須正常產出**，**不得**課 active／mechanicallyApplicable／Check B／expiry。**同 (5-req) 的獨立前提**：這些正例的 head 側（`deleted`，或綁到一個**自身完全合法**的 clause 的**合法 retag**）**必須**各自滿足其**全部** current G1 與 post-binding 要求；本格只免除 **B 側**的現時效力。**(10b) 歷史 store 的驗證邊界（v1.17 新增；carrier：§11b.10c step 6b）**：**正例（必驗）** —— B 內有一個在**其當時**完全合法、其 exception **現已過期**的 exception-backed DP，而 **C 已把該 DP 修回其 active hard constraint、且 C 自身通過完整 T0 驗證** → **必須正常產出**，**不得**以 `E_INV4_NOT_APPLICABLE`（或任何**以現在的 T0 重算歷史 B**的判定）失敗（**已知實測反例**：實作對 B 呼叫 `validateAll(base.store, { now: T0 })`）。**非時間性無效歷史 store 的負例（v1.17 修訂 3 新增；必驗，且必須與上列正例同時成立）** —— B 的 exception terminal 其 expiry 在**未來**（例如 2099）卻**缺 `scopeRulingRef`**：該 B **通過** `validateStoreSchema()`，但**必須**在 applicability 檢查以 **`E_INV4_NOT_APPLICABLE`（scope-ruling-missing）** 被拒；**同一形狀在 C 修好之後必須合法**（current 對照組）。**本格證明歷史驗證不得退化成 schema-only**：把歷史路徑換成「只跑 schema」會讓這份**在其當時就無效**的 B 通過 —— 那是 regression，**明文禁止**。**其餘非法歷史 store 的控制組（各一，共四，皆必須 fail-closed）** —— ① B 的 `provenanceVersion` 為 1／2 以外之值；② B 內有 dangling ref；③ B 內有重複 typed ID；④ B 的某個 immutable object 在 C 中**缺席**或 canonical typed value **不相等**（cross-snapshot integrity）。**這些格與上列正例合起來證明：本邊界移除的恰為「對本次 T0／H 求值」這個 closed set，非時間性的 schema／identity／reference／applicability／DP 專屬檢查一項未減。** historical v1 仍以 read-only legacy decoder 讀，**每一個歷史版本自己的欄位與 expiry grammar 原樣適用**，**不得**遷移、正規化或回寫，**也不得**拿 normalized bytes 冒充 raw base witness。**(10c) raw base witness 依 operation 分開驗（v1.17 修訂 3 新增）**：**(i) 只課於 producer** —— `produceChangedTestInventoryV2` 已選定一個 TaskState，其 `baseProvenance` 的 canonical storePath、`treeOid` 與 **raw-bytes `storeDigest`** 必須與**實際捕捉到的 B** 逐項相符（**B 缺席**時取既有 canonical empty store digest），**在拿 B 去驗任何 binding 之前**比對；任一不符 → **fail-closed**，**不得**偏袒任一方，**不得**新增 public argument 或 carrier／envelope 欄位。**digest-mismatch 這一格只屬於 producer。****(ii) standalone 兩欄 operation 的正例（必須保留）** —— `buildGovernanceSeedPreimage` **沒有** taskId、**沒有** caller 提供的 witness，因此**不得**選定 TaskState、**不得**、**也不可能**做 witness 比對；它由 `baseTreeOid` 這個 content-addressed exact tree 取得 B，且在 **current store 缺席**與 **canonical empty** 兩種情形下**必須成功**（AC171 (B)(ix)／(xi) 原樣有效）。**(iii) 兩種 digest 記法不得互相冒充** —— B 的 witness digest 取 **B 檔的原始 bytes**；G1／G2 的 CAS digest 仍是 `sha256(canonicalText(檔案文字))`（**已知實作缺口**：`captureBaseStore()` 目前回傳 `sha256(canonicalStoreBytes(store))` 且無人使用，producer 的比對**必須**改用原始 bytes）。**(11) post-binding 完整委派，逐項可豁免性依 witness 判定（v1.17 改寫）**：驗 head 側 clause binding 課 shared §9 postChangeBinding 列的**全部**條件，於**同一 G1、同一 T0**，並依 §11b.10c 「Governance-affected witness carve-out」的**十項不可再分義務**逐項判定。**(11a) 十項全驗（v1.17；前一稿寫九項，漏掉 target authority 一肢，該計數撤回）**：ob-1 clause 可解析、ob-2 **direct Source set 的每一個成員**可解析、ob-3 `status == active`、ob-4 **每一個成員**通過 **Check A**、ob-5 **每一個 `driftMode=repo-file` 成員**通過 **Check B**（對 `H`）、ob-6 `targetConstraintRef` 可解析、**ob-6b target 必須是 `authority=hard-constraint` 的 REQ**、ob-7 `grantAuthorityRef == target REQ 的 ownerRef`、ob-8 `expiryInstant > T0`、ob-9 `mechanicallyApplicable` 的其餘 per-kind 項。**(11a-set) direct Source set 必須被量化，不得只驗第一個**：至少一個 **多 direct Source** 的 fixture（DEC 或 ASSUM，basisRefs 含**兩個以上** Source ref），**(11a-set-A) Check A 的非第一成員負例 —— 這一格有行為判別力，必驗**：其中**只有非第一個**成員的 **Check A** 失敗 → **必須** fail-closed；**只驗第一個成員的實作會照常產出 entry，兩者的 envelope 因此不同，該實作必然 fail 本格**（ob-4 **永不可豁免**，**沒有**任何 witness 對應 Check A）。**(11a-set-B) Check B 的多 Source 覆蓋仍是強制義務，但它的證據不是公開輸出**（v1.17 修訂 3；前一稿把「只有非第一個 `repo-file` 成員的 Check B 得 zero occurrence → 依 witness 規則處置」與「只驗第一個成員的實作必須 fail 本格」並列，**對 Check B 這半格該要求撤回**）：該形狀之下 zero occurrence 使該 clause 進 `driftedClauses` → witness-2 → 豁免，於是**正確實作與「只驗第一個成員」的實作產出相同的 public envelope**，**不得**宣稱它會改變公開輸出；ob-5 **仍必須**對**每一個** `repo-file` 成員逐一求值，其證據**依 (11a-ob5) 的結構／到達性證據**。**ob-5 必須對 `H` 明確求值** —— **明文禁止**以「已呼叫 `applicable()`」充當 ob-5 的證據：**沒有 `H` 的呼叫只能清掉 Check A 那一肢**，**不可能**清掉 Check B；DEC／ASSUM 在該謂詞內**連 Check A／B 的對應肢都沒有**（見 §11b.10c 的 per-kind 對照）。**(11a-ob5) ob-5 的證據形狀（v1.17 修訂 2；前一稿要求「本格必須含一個可執行的 red→green 證據：ob-5 未求值的實作必須 fail 本 AC」，該要求不可滿足，撤回）**：ob-5 的失敗條件與 witness-2 的成員條件**用的是同一組 direct Source 與同一個 `H`**，因此在 **public envelope 之上不存在** red／green discriminator —— ob-5 未求值的實作與正確實作，對**任何**合法輸入都產出 byte-identical 的 envelope 與 `inventoryDigest`（理由同 (11d-4) 的「ob-5 失敗但無 witness-2 不可構造」）。**本 AC 因此要求兩類證據，並必須如實標明其性質**：**(i) 結構證據（明確**不是**行為判別證據）** —— 對 production source 與控制流**逐一檢查**：post-binding 對**每一個** `driftMode=repo-file` 的 direct Source 都**確實**對 `H` 求值 Check B，且該求值**不是**由 `applicable()`／`mechanicallyApplicable()` 代勞；**(ii) 行為證據仍必須存在，且它明確是 Check A 那一半** —— 即 **(11a-set-A)**：多 Source fixture 中**非第一個**成員的 **Check A** 失敗必須被抓到，該半格**保有**完整判別力（v1.17 promotion 的 authorized precision correction：此處的行為引用**只**指 (11a-set-A)，**不**指 Check B；Check B 半格 (11a-set-B) **只**由結構／到達性證據承載）。**可選的到達性證明**：得以 §11b.10c **形狀 B**（獨立 child process、scratch source copy、**單一點**編輯）證明該求值**確實被抵達**；但**不得**把它包裝成「自然可達的 no-witness drift 反例」，**更不得**為此在 production 新增任何 seam。**已知實測反例仍必須被上列結構證據抓到**：`69cff79` 的 `assertPostBinding()` 只課 active 與 `mechanicallyApplicable()`，ob-5 從未求值，其註解宣稱 `applicable()` 已承載該列 —— **該註解不成立**。**(11b) 永不可豁免項的單變數負例，依**實際拒絕的層級**分開（v1.17 改寫；**v1.17 修訂 2**：前一稿只分兩層、並把 ob-2 放在層級 2，**該分層錯誤已由獨立複審逐項重現，撤回** —— 在**合法 G1 ＋ 強制 seed** 之下，dangling direct Source **到不了** binding validation）**：**(11b-層級 2；各一，共三)** 只讓 **ob-1** 不成立（head 側 tag 的 clauseRef 在 C 中不可解析）、只讓 **ob-4** 不成立（Check A：C 中該 Source 的 digest 與 excerpt 不符；該 clause **不得**是任何 DP 的 current terminal，且該 Source 若同時存在於 B，其 canonical value **必須相等**，否則會先違反 cross-snapshot integrity —— **實測**此形狀通過 G1 validation，確實抵達 binding）、只讓 **ob-9** 不成立（**dangling RecordRef**：一個**非 DP-terminal** 的 DEC／ASSUM，其 `basisRefs` 含一個不可解析的 `{ kind, ref }`，其餘 basis 合法 —— RecordRef **不在** direct Source set，seed 的 direct Source walk **不**碰它，store 的 ref validation 亦**不**課 clause 的 basisRefs；**該 clause 必須非 DP-terminal**，否則 INV-4 會在層級 1 先行拒絕）→ **各自 fail-closed**，錯誤必須**兩兩可區分**，且必須指名該 testRef、該 binding、哪一側，以及是十項中的**哪一項**（只說「binding 無效」不合格）。**(11b-層級 1；各一，共四)** ob-6（dangling `targetConstraintRef`）、**ob-6b**（target 非 `authority=hard-constraint`）、ob-7（`grantAuthorityRef` 與 target `ownerRef` 不符）、**ob-2 的 REQ 一肢**（`REQ.sourceRef` 在 C 中不可解析）——**四者在合法 G1 之下到不了 binding validation**：store validation 先以自己的 code 拒絕（**實測**：依序為 `E_DANGLING_REF`、`E_EXCEPTION_TARGET`、`E_EXCEPTION_OWNER`，以及 REQ 一肢的 `E_DANGLING_REF` —— 該肢在 **clock-free** 的 schema 階段即被拒，**早於** T0）。**(11b-層級 1b；一格)** **ob-2 的 DEC／ASSUM 一肢**：一個**非 DP-terminal** 的 DEC 或 ASSUM，其 `basisRefs` 含一個在 C 中不可解析的 `S-…` → 它**通過** G1 validation，卻在**強制的** lifecycle seed derivation（§11b.10c orchestration step 4 的 direct Source walk）被 **seed derivation 層**以 **`E_SOURCE_DANGLING`** 拒絕（**實測**；該 clause 若是 DP terminal，則**更早**由 INV-4 在層級 1 拒絕）。**層級 1 與層級 1b 的證據要求**：**必須**斷言**該層** authoritative 的 **error code 與 cause 原樣**（層級 1 為 store validation 的 code、層級 1b 為 seed derivation 的 code —— **不得**互相冒充），且**不得**要求錯誤指名 testRef、side 或 obligation 編號 —— 該時點**尚無** matcher、pair 或 testRef；ob-2 的兩肢**得**以「**deterministic first failure ＋ 受影響義務清單**」記錄，**不**要求單一義務指名（dangling `S-…` 本來就同時打到 ob-2 與 ob-9 的 basisRefs 一肢，見 §11b.10c 的重疊記錄）。**明文禁止**為了讓上列任一項在 binding validation 被觀察到，而放寬、跳過或延後 store validation **或** seed derivation（**含**把 seed 改為 lazy、或讓 direct Source walk 不 fail-closed）。**義務不因層級而減免**：ob-2（連同 ob-6／ob-6b／ob-7）**仍必須**在 post-binding 被逐 Source 課到，作為 fail-closed 的 defence in depth；**不得**以「上游一定已擋掉」為由略過，**也不得**據此宣稱它們會在 B 側、或在某個「未來放寬的 store」上被課到。**(11c) 有 witness 的正例（各一，共兩；v1.17：前一稿列三個，witness-3 的正例不可達，已撤回）**：ob-3 不成立且 `clauseRef ∈ transitionedClauses`／ob-5 不成立且 `∈ driftedClauses` → **不得** fail-closed，該 logical test 依 §6 ordered classification 正常產出（same-body、same-tag、same-path 時為 `governance-affected`）。**兩個 fixture 的 clause 皆不得是任何 DP 的 current terminal、且不得 exception-backed** —— 否則 store 先違反 INV-4 或先落入 §7，測到的不是本格。**witness-3 沒有可達的正例**：理由與撤回依據見 AC173 (b0) 與 §11b.10c 「三對的可分離性並不相同」；**不得**以豁免 `applicable` 整項或放寬 INV-4 把它救回。**(11d) witness 比對的判別力（v1.17；此格的可構造性不對稱，必須照實作）**：**(11d-1) ob-3 無 witness-1 → fail-closed**：clause 在 `C` 中非 active，但使其非 active 的生效 Transition **在 `B` 就已存在**（`T.id ∈ ids(B.transitions)`），因此**不在** `transitionedClauses` —— 綁到早已作廢的 clause **不是**本次治理事件，**必須 fail-closed**。**(11d-2) union 實作的判別**：同一個 fixture 再讓該 clause 的 Source 對 `H` drift（於是 `clauseRef ∈ driftedClauses` ⊆ `lifecycleAffectedClauses`）—— **逐項對應**的實作仍 fail-closed（ob-3 的 witness 是 witness-1，不是 witness-2），以 `clauseRef ∈ lifecycleAffectedClauses`（union）豁免的實作會**錯誤放行**，因此**必須** fail 本格。**(11d-3) 永不可豁免項不因 witness 而放行**：ob-4（Check A）不成立**且**該 clause 同時命中 `driftedClauses` → **仍須 fail-closed**（此為刻意的雙變數案例，不是 (11b) 的單變數負例）。**(11d-4) ob-5 與 ob-8 的可構造性並不相同（v1.17 更正；前一稿把兩者一併判為不可構造，該判定錯誤並撤回）**：**ob-5 ↔ witness-2 重合** —— `driftedClauses` 的成員條件就是 ob-5 的失敗條件（同一組 direct、`repo-file` Source；`snapshot-only` 依上游永不算 drift），因此「ob-5 失敗但無 witness-2」**沒有**可構造的 fixture；本 AC **不要求**該格，**禁止**以近似案例假造，實作與複審**必須**把它記為 **not constructible** 並引用上游 derivation。**ob-8 ↔ witness-3 可分離，且該負例必驗**：`expiredClauses` **只**收「REQ 且其 `sourceRef` 指向 exception-grant」，上游明文 **DEC／ASSUM 不因 `basisRefs` 含 exception-grant 而入本集合**；因此一個 **DEC 或 ASSUM**，其 `basisRefs` 含一個 `expiryInstant <= T0` 的 exception-grant Source → **ob-8 失敗、clause 不在 `expiredClauses`、無 witness → 必須 fail-closed**。該 fixture 的 clause **不得**是任何 DP 的 current terminal（否則先違反 INV-4）。**REQ 側則既無正例也無負例**：見 (11c) 與 AC173 (b0)。**(11e) 豁免的效力邊界**：(11c) 的**兩個**正例**不得**被記為「binding 有效」或「已通過 gate」；entry 與 envelope **不得**新增任何欄位承載「已豁免」，`GovernanceSeedPreimage` 仍**恰四欄**、envelope 仍**恰七欄**；產出之後 `.ctide/output/**` 仍**未寫出**，`unsupported-populated-inventory` 仍**未解除**。**(11f) Check B 的搜尋位元組（v1.17 新增；v1.17 修訂 3 收窄；carrier：§11b.10c step 8b）**：**fixture 的隔離要求（必須遵守，否則測到的不是本格）** —— 下列 newline／BOM 控制組**必須**使用 **B 與 C 中同一組既有的 clause／Source**，且**不得**存在**任何第二個 lifecycle 成因**（**不**新增 clause、**無** Transition、**無** expiry）；**已知**：最初那組 CRLF 探針因另外新增了一個 clause 而失去判別力，**已由隔離控制組取代**。**(i)–(v) 五組隔離控制（各一，`lifecycleAffectedClauses` 皆必須為空、entries 亦相同）** —— 同一份邏輯內容分別以 **LF**、**CRLF**、**單獨 CR**、**BOM+LF**、**BOM+CRLF** 存放，其 byte-preserving 變換形與該 Source 的 canonical excerpt **完全相等** → **五組一律不得** drift、**不得**進 `driftedClauses`、該 clause **不得**因此被判 ob-5 失敗（**已知實測反例**：直接搜尋 **raw** haystack 時，LF 與 BOM+LF 得空 seed，而 **CRLF、CR、BOM+CRLF** 錯誤地產出該 REQ —— **false drift**）；**(vi) 真 drift 對照** —— excerpt 在變換形之下**確實**不存在於 `H` → **仍必須** drift（zero occurrence），**不得**因這層變換而變成「總是找得到」；**(vii) raw 層不得被改動** —— 同一組 fixture 之下 `headViewDigest`（及其 `contentDigest`）**仍取 raw bytes**，因此 CRLF 版與 LF 版的 `headViewDigest` **必須不同**（與 **AC106** 的既有兩層姿態一致），且 store 內的 excerpt bytes 與 repo 檔 bytes **逐一不變**；**(viii) 無關的非 UTF-8 blob 不得影響任何判定（v1.17 修訂 3；取代前一稿的「不可解碼 → fail-closed」，該格撤回）** —— H 中存在一個與任何 Source **無關**的二進位／非法 UTF-8 regular blob，而該 Source 的文字仍存在 → **整輪必須成功**、該 clause **不得** drift、該 blob **不得**被排除於搜尋面之外、**不得**被說成 unanalysable source；**(ix) 不得由 replacement-character 解碼製造假匹配** —— 一個含非法 UTF-8 bytes 的 blob，其「以 U+FFFD replacement 解碼後」的文字**恰好等於**該 excerpt，而在 byte-preserving 形之下**不**含該 excerpt → **必須**判 drift（zero occurrence）；把 haystack 做 lossy decode 的實作會誤判 not drift，**必須** fail 本格；**(x) 真正的失敗仍 fail-closed** —— H 捕捉／讀取該 blob 的 IO 失敗、Source 的 excerpt 非字串或 canonical 後為空（`E_SOURCE_UNANALYSABLE`）→ **各自 fail-closed**，**不得**默認 not drift。**(12) §7 五項全驗**：限定形式 `@src REQ-x@DP-y` 逐項驗 DP-y ∈ currentTaskDpIds、status == resolved、resolvedBy == REQ-x、applicable(REQ-x, DP-y)、scopeRulingRef.subjectRef == DP-y —— **五個單變數負例各一**，任一不成立即 fail-closed。**(13) 非 exception-backed 的 REQ@DP**：一個非 exception-backed 的 REQ 帶 @DP qualifier → **fail-closed**。**(14) 裸形式的 0／1／many**：exception-backed REQ 的裸形式**只在** currentTaskDpIds 內反推 resolvedBy == REQ-x 的 DP —— 恰一個候選 → 自動綁定並**續驗 (12) 五項**；**零個** → fail-closed；**多於一個** → fail-closed，訊息要求改用限定形式；**明文負例**：候選存在於**歷史 DP**（不在 currentTaskDpIds）→ **不列入**，因此仍是零個候選 → fail-closed。**(15) taskId 解析**：G1 的 taskStates 中恰一個 taskId 相符 → 採用；**unknown task（零個相符）** → fail-closed；**多於一個相符** → fail-closed；採用之 TaskState 的 `baseProvenance.treeOid != request.baseTreeOid` → **fail-closed**（**base mismatch**，且**不得**偏袒任一方）。**(16) 多 TaskState discriminator**：store 持有**兩個以上** TaskState 時，**不得**由 DP、head view 或任何啟發式挑選 current task —— 未給 taskId、或給了但不唯一 → fail-closed；**單變數對照**：給出正確且唯一的 taskId → 正常產出。**store 只有一個 TaskState 時同樣必須給 taskId**，**不得**略過該 key 或略過 base mismatch 比對。**(17) taskId 不進 carrier／envelope**：驗四欄 carrier 與七欄 envelope **皆不含** taskId，且 taskId **不**進任何 digest 的 preimage。**(18) 失敗的副作用邊界**：上述任一 fail-closed 之後，`.ctide/provenance.json`、explicit config（`.ctide/test-adapters-config.json`）、shipped registry（`test-adapters.json`）與 `.ctide/output/**` 的 bytes **逐一不變**。
177. **Approval／implementation boundary（test-provenance approved v1.16）**（carrier：本文件 live status ＋ §11b.12 rollout boundary）：**本文件已於 2026-08-22 由使用者明確核准並 promote 為 approved v1.16** —— 前置 draft 內容已由**獨立 Codex 審查並 ACCEPT**；**本次未執行 agent-duel panel，因此不得稱為 panel 放行**。**核准歸屬於完整 remediation chain `4d49dd8 → 2bf9adc → fd14bb4 → f81d3e7`，不得歸給其中任一單獨 commit。** **本次只 promote 本文件**：v1.16 的 delta 不觸及上游，**shared approved v1.15 與 intent-scan approved v1.10 均不變**。**前一 approved baseline 為 test-provenance approved v1.15**，未撤回、未取代；**current approved coupled set 現為 shared approved v1.15 ＋ intent-scan approved v1.10 ＋ test-provenance approved v1.16**，三者為同一生效集合，不得分開採用。**v1.16 的新增契約現已生效** —— 三欄 public request、binding semantic validation、`@src` 排除與 `node-test-v2` identity 收斂；**契約生效不等於已實作**：上列**沒有任何一項**已有 implementation 被接受，**不得**以本次 promotion 作為它們已完成的證據。**既有 producer 實作（commits `020e7ec` 與 `53801a9`）為 provisional、未 ACCEPT**：其 AC172 (6) expected direction 寫反（Shape-B 污染副本目前**成功回傳**，規格要求 fail-closed）、缺 registry cross-binding guard、缺 binding semantic validation、AC173 (a)(c) 目前以 `modified` 作為通過結果、AC173 (i)(j) 使用空 entries 的 envelope、AC173 (k) 守的是不存在的 `.ctide/adapters.json`。**上列各項在本次 promotion 之後仍未修**，且**不得**以本 AC、或以「v1.16 已 promote」作為它們已修的證據 —— 修復需要另一輪 append-only remediation 與獨立複審。**本版不新增任何 persisted 欄位**：`GovernanceSeedPreimage` 恰四欄、`ChangedTestInventoryV2` 恰七欄、`inventoryVersion` 仍為 `2`、`inventoryDigest` 公式不變、`taskId` **不**進 carrier、**不**進 envelope、**不** persisted。**§11b.10c 的 anti-injection 清單一字不變**；改變的**只有** request 的 exact key set（二欄 → 三欄），因此**不得**再宣稱 request block byte-identical。**identity implementation boundary**：v1.16 把 current identity 收斂為 `node-test-v2`。**spec promotion 本身從不使任何 shipped artifact 升版或被接受** —— 該規則不變；**實際的升版與其 acceptance 是後續、獨立的一次 rollout**（2026-08-22，acceptance chain `0682553 → 02f8bc6 → 4c1ff43 → 7da3dae`），其後 shipped `test-adapters.json`、`vendor/vendor-manifest.json`、compiled closed mapping 與 executable component 四者皆為 `node-test-v2`。v2 需要 formal registry、vendor manifest、compiled closed mapping 與 executable component **四者一致**，任一不一致即 fail-closed，且**仍須**重新 authorization 與獨立 implementation review；`node-test-v1` 的 acceptance **只保留為歷史 acceptance**，不自動移轉。**六項 rollout 前置條件的計數**（closed set 不變）：promotion 後、v2 尚未接受 → 4／6（adapter registry 因 identity 未一致而退回未完成，producer 仍未完成）；v2 registry／manifest／executable chain 接受後 → 5／6（只剩 producer）；producer 接受後 → 6／6。**現況為 5／6。**`node-test-v2` executable component 仍是 **supporting component**，**不**新增第七項 prerequisite。**changed AC set 恰為 171、172、173、175、176、177**：AC171／AC172 只更新 request authority，AC173 只加入一則 draft authority note，AC175–AC177 為新增。**AC1–AC170 與 AC174 逐條 byte-identical** —— AC174 是 v1.15 的 approval boundary，屬凍結敘述，其兩欄 request 文字**刻意不回改**，v1.16 的現況由本 AC 承載；因此**不得**宣稱 AC1–AC172 byte-identical。因此：**不得**接受 populated inventory；**不得**解除 `unsupported-populated-inventory`；**AC118／AC136／AC137／AC138 一律不得宣稱已滿足**；**Phase 2 不得宣稱 READY**；**AC1–AC170 與 AC174 逐條 byte-identical**；**AC171／AC172 只更新 request authority**；**AC173 僅加入一則 draft authority note**，其 cases、expected results、evidence shapes 與 error boundary 一格未動；**AC175–AC177 為本版新增**。
178. **Approval／implementation boundary（test-provenance draft v1.17）與 producer 的 provisional 地位**（carrier：本文件 live status ＋ §11b.10c 「Governance-affected witness carve-out」＋ §11b.12 rollout boundary）：**本 AC 是 v1.17 的 draft 期紀錄，逐字保留為歷史；current 狀態見 AC179**（2026-09-05 promote 為 **approved v1.17**）。**(1) 已由該次 promotion 解除**（v1.17 契約現已生效、實作已獲授權開始）；**(2)～(6) 的義務原樣有效，未被 promotion 關閉** —— 特別是 **(2) `69cff79` 仍為 provisional／NOT ACCEPT**、**(2e) 三項證據仍必須落地並經獨立複審**、**(5) 的不變量仍全部有效**。以下為 draft 期原文：**本文件現為 draft v1.17 — 未審核**，2026-08-22 起草，**spec-only**：尚未 promotion、未經 panel 放行、未經使用者核准為 approved。**最後 approved baseline 仍為 test-provenance approved v1.16**（未撤回、未取代），**current approved coupled set 仍為 shared approved v1.15 ＋ intent-scan approved v1.10 ＋ test-provenance approved v1.16**。**(1) v1.17 的契約在 promotion 之前一律不得實作**，且在 v1.17 經獨立複審並 promote 之前**不得**再變動 production 或 tests。**(2) producer remediation commit `69cff79` 經獨立複審 NOT ACCEPT、維持 provisional**，**不得**計為完成，**不得**據以推進 rollout 計數。其五項阻擋原因為：**(2a)** 三欄 request 令 `buildGovernanceSeedPreimage()` 解析 task，與 AC171 (B)(ix)／(xi) 的 missing／empty G1 必須成功**自相矛盾**（空 store 無 TaskState）；**(2b)** AC173 (b1)(b2)(b3) **三格全部**與 AC176 (11) 衝突（**不只** (b1)(b3)）—— (b2) 的 Check B zero occurrence **本身就是** postChangeBinding 的失敗項；**(2c)** `assertPostBinding()` 只課 active 與 `mechanicallyApplicable()`，**未課 Check B**（ob-5），且註解宣稱 `applicable()` 已承載 postChangeBinding 列 —— **該宣稱不成立**；**(2d)** `runWithGovernanceContext()` 是**接受 caller callback 的 public export**，直接把 `B`、G1、選定的 TaskState、T0 交給外部，違反 private context 契約並構成 test seam；**(2e)** AC176 的 executable matrix、AC176 (8b) 的 G1 → binding → G2 ordering evidence，以及 AC173 (i)(j) 對**真正 populated** envelope 的驗證，**仍未撰寫**。**(3) 本地測試全綠不構成 acceptance 證據**：`69cff79` 當時的本機結果為 **0 fail**（另有 skip；兩個複審方回報的 total／skip 數不同，該差異本身尚未解釋），但其中若干斷言是**把上述規格衝突改寫成預期失敗**（例如把 (b1)(b3) 斷言為 `head-binding-inactive`／`head-binding-not-applicable`）。**把不可達的格子斷言成它實際的失敗**只證明衝突存在，**不**證明契約被滿足；本 AC 明文禁止以該類斷言充當任何一格的通過證據。**(4) v1.17 promotion 之後才可重啟的工作，且順序不得顛倒**：先依 v1.17 修正 production（兩欄／三欄 request 分家、**十項**義務全驗含 ob-5 與 ob-6b、direct Source set 量化、witness-matched carve-out、module-private helper、錯誤指名依層級分開），再補齊 (2e) 的三項證據；**(2e) 未落地之前，producer 前置條件不得計為完成**。**(5) 本 AC 生效期間的不變量**：`unsupported-populated-inventory` **仍在**、populated inventory **仍不得接受**、**Phase 2 仍 not READY**、**六項 rollout 前置條件仍為 5／6**；AC118／AC136／AC137／AC138 **不得**宣稱滿足；artifact emission、Step 5／6、ledger 與 arbiter wiring **均不得**開始。**(6) 本 AC 於 v1.17 promotion 時不自動消滅** —— 它記錄的是 `69cff79` 的地位與 (2e) 的缺口，**必須**在 (2e) 三項證據全部落地並經獨立複審 ACCEPT 之後才可改寫，且屆時 acceptance **必須**歸屬於完整 remediation chain，**不得**歸給其中任一單獨 commit。**(7) draft chain 自身的歸屬（v1.17；新增）**：v1.17 的 draft 亦為**多段 chain**，其**首段**為 `8594166`，且該首段經獨立複審 **REVISE／NOT ACCEPT，維持 provisional** —— 六項必修為：(b3) 在 §7／INV-4 之下仍不可達、(b0) 仍要求兩個不可構造的 cross-witness 負例、AC176 (3) 與 (11c) 正面矛盾、義務清單漏 target authority 且 direct Source 誤用單數、錯誤指名層級不可實現、AC178 與 status delta 未同步。**將來若 v1.17 獲 promotion，acceptance 必須歸屬於該完整 chain，不得只歸給其中任一段。****(7b) chain 的 exact 組成（v1.17 修訂 2；更正前一稿的單數寫法 `8594166 → follow-up`）**：`8594166 → 7bf57ab`，**加上** `7f1b346`（**status-only**：更新 shared 文件的 coupled-set 版號註記 —— 獨立複審**已核對 git 歷史，確認它未改動 shared 的任何 normative clause**），**再加上**本次**尚未 commit** 的 spec-only 修正。**(8) 本候選的當前審查狀態（2026-09-05；Codex 獨立複審，非 panel 裁決，亦非使用者新的直接核准）**：**判定 REVISE／NOT ACCEPT**。重現的阻擋項為 **R1**（前一稿的兩層錯誤證據要求：AC176 (11b) 要求一個在合法 G1 ＋ 強制 seed 之下**到不了** binding 層的 dangling-Source 負例）、**R2**（Check B 以 **raw** CRLF bytes 搜尋，與上游 canonical-bytes 規則牴觸，產生 **false drift**）、**R3**（**歷史 B 被課本次 T0 的現時效力**，擋掉合法修復），另有三項必須更正的事實矛盾（`mechanicallyApplicable` 與 ob-5 的 per-kind 關係、seed 與 `taskStates` 的關係、(5)／(10) 正例的獨立前提）。**本次修訂是 Claude Code 依該複審所作的 spec-only 更正** —— R1 改為**三層**（層級 1／1b／2）證據歸屬、R2 與 R3 各加一則**下游澄清**與 executable control、四項事實矛盾更正、狀態載體同步、chain 歸屬更正。**本次修訂未改動任何 production 或 test source**；`69cff79` 仍為 **provisional／NOT ACCEPT**，六項 rollout 前置條件仍為 **5／6**，`unsupported-populated-inventory` 仍在，下游仍凍結，Phase 2 仍 not READY。**acceptance 由 Codex 對修訂後候選的獨立複審裁定**；在該裁定之前，本文件仍為 **draft v1.17 — 未審核**。**R3 的 historical validation boundary 是否需要上游 normative 修訂，必須由複審分開裁定，不得被「delta 不觸及上游」一語掩蓋**（見 §11b.10c step 6b 的審查邊界）。**(9) 第二輪更正（v1.17 修訂 3；2026-09-05 Codex interim review 之後）**：複審對本文件的 phase-2 diff 提出**兩項實質問題**與數項一致性更正，**皆已更正**：**(9a)** step 6b 前一稿把歷史邊界寫成「只驗 clock-free」，又以 cross-snapshot immutability 自證「B 的內容已由 C 驗過」——**兩者皆錯，已撤回**。新的排除面**恰為**「對本次 T0／H 求值」這個 closed set；**非時間性 applicability、DP 專屬檢查（含 historical terminal coherence、Check A、`scopeCovers` 機械肢）一律保留**。**實測負例**：expiry 在 2099 卻缺 `scopeRulingRef` 的 B 通過 `validateStoreSchema()`、卻應以 `E_INV4_NOT_APPLICABLE`（scope-ruling-missing）被拒 —— **schema-only 的歷史捷徑會使它通過，屬 regression**；該負例已入 AC176 (10b)。**(9b)** step 8b 前一稿要求「blob 無法以 UTF-8 安全解碼 → fail-closed」，那等於**新增**一條「repo 內每個 regular blob 都必須是合法 UTF-8」的規則，上游沒有，**已撤回**；改採 **byte-preserving transform**（去開頭 BOM、CRLF／CR → LF、其餘 byte 原樣），needle 仍為 canonical UTF-8 excerpt bytes —— 對 UTF-8 文字與既有 canonical 規則**等價**，且**不**產生 U+FFFD 假匹配、**不**排除任何 path；AC176 (11f) 據此改列 (i)–(x)，含**無關二進位 blob 必須成功**與**不得由 replacement 解碼製造匹配**兩格。**(9c)** 另更正：ob-2／ob-9 的重疊**只**發生在 DEC／ASSUM（REQ 無 basisRefs 一肢）、ob-9 的 binding 層 discriminator 必須落在**非 DP-terminal** 的 DEC／ASSUM、projection 約束的是 **governance derivation 的邏輯輸入事實**而非 private helper 的參數個數或內部 continuation、(11a-set) 的行為判別力**只**屬 **Check A** 的非第一成員負例（Check B 半格改由結構／到達性證據承載）、raw base witness 的比對**只**屬 producer（standalone 兩欄 operation 無 taskId 也無 witness，absent／empty C 仍必須成功）。**上列更正同樣是 spec-only，未改動任何 production 或 test source**；候選仍為 **draft — 未審核**，acceptance 仍由 Codex 對修訂後候選的獨立複審裁定。

179. **Approval boundary（test-provenance approved v1.17）與 promotion 之後仍未關閉的 implementation evidence gap**（carrier：本文件 live status ＋ `docs/superpowers/reviews/2026-09-05-test-provenance-v1.17-codex-review.md` ＋ §11b.12 rollout boundary）：**本文件已於 2026-09-05 promote 為 approved v1.17**，由 **Codex 依使用者委任的 decision authority 作出獨立複審與核准** —— **不得**稱為 agent-duel panel 放行，**也不得**稱為當日使用者另一次直接核准。**核准歸屬於完整 chain `8594166 → 7bf57ab` ＋ `7f1b346`（status-only 的 shared coupled-set reference 更新）＋ 2026-09-05 複審當日的未 commit 修訂**，受審檔案的 SHA-256 為 `c032e7517f16834e8f7b27f2f089ed4e5aad10d29cb20da8ddd38b5bf457052c`；**不得**只歸給其中任一段。**current approved coupled set 現為 shared approved v1.15 ＋ intent-scan approved v1.10 ＋ test-provenance approved v1.17**，三者為同一生效集合、不得分開採用；**shared 與 intent-scan 版本未動、未重新 promotion、契約內容一字未改**，兩者只各自更新一則 status-only 的 coupled-set reference。**(1) promotion 解除的，只有「不得實作」這一條**：§11b.10c 的 producer／governance 修復與 AC178 (2e) 的三項證據**自此獲授權開始**，**順序不得顛倒**（先修 production，再補證據）。**(2) promotion 不關閉任何 implementation gap**：`69cff79` **與本次 promotion 之後寫出的 producer 實作**皆為 **provisional／NOT ACCEPT**，**不得**計為完成、**不得**推進 rollout 計數；**六項 rollout 前置條件仍為 5／6**、`unsupported-populated-inventory` **仍在**、populated inventory **仍不得接受**、**Phase 2 仍 not READY**、AC118／AC136／AC137／AC138 **仍不得宣稱滿足**；artifact emission、batch／consumer 整合、Step 5／6、ledger 與 arbiter wiring **仍在 producer 獨立接受之後**。**(3) 本次 promotion 一併寫入四項 authorized precision correction**（澄清既有解釋，**不新增機制、不放寬任何檢查**）：**(3a)** shared §9 base-witness 顯示中的 `storeDigest` 由「該檔 canonical bytes」更正為**該檔在該 tree 中的原始 bytes**，並明示與 current-store canonicalText CAS **不同** —— 同節 historical-read 條與下游 §11b.9c 早已要求 raw bytes，故此為**一致性 erratum，不是新的 shared 版本契約**；**(3b)** 歷史驗證**必須**保留**完整既有的 `scopeCovers` predicate**，**含**其 typed ruling 與 **`scopeCovers == true`** 檢查 —— §11b.10c step 6b 的括號例子**是舉例，不是窮舉替代**；**(3c) 歷史證明邊界必須誠實陳述**：本處**不** persist 任何歷史求值 instant，因此該驗證**不**證明某個 grant 在過去某個 commit 時點為未過期；它保留的是**非時間性**規則並拒絕其違反。**不得**以捏造的 instant、**也不得**以無條件的「unexpired == true」取代未知的歷史時刻 —— 尤其在 **否定式**的 applicability 條件（例如 source-2 reopen cause 要求 successor **不** applicable）上，那會**錯誤地拒絕**一個由 expiry 解釋的合法歷史 reopen；**(3d)** AC176 (11a-ob5) 中殘留的行為證據引用**明確指向 Check A 半格 (11a-set-A)**，Check B 半格**只**保留結構／到達性證據。**(4) 舊的 draft-review 判定是歷史紀錄，不是 current decision**：AC178 (7)(7b)(8)(9) 所載的 REVISE／NOT ACCEPT 與兩輪更正**逐字保留**，但**不得**再被引用為本文件的現行狀態。**(5) 日後 producer 若獲接受，acceptance 必須歸屬完整 remediation chain**，**不得**只歸給任一單獨 commit，且**必須**由獨立複審在**實際跑過**受影響檢查之後作出。**(6) 該接受已於 2026-09-05 作出（status-only 記錄，本 AC 的 normative 要求一字未改）**：Codex 依委任所作的獨立複審 **ACCEPT** 了**提交的整條 producer chain 與受審的 working-tree 修訂**（含未 commit 的修訂，其 SHA-256 逐一列於 `docs/superpowers/reviews/2026-09-05-producer-implementation-codex-review.md`），**不是** `69cff79` 一段；(5) 的兩項要求因此**都已被滿足**：歸屬是完整 chain，且複審在 clean worktree **實際跑過** `node --test`（1534／1523 passed／11 skipped／0 failed，exit 0）、structure validation 兩段與 deterministic eval 7／7。**(2) 所載 `69cff79` 的 provisional／NOT ACCEPT 逐字保留為當時候選的正確判定**，**不因本次接受而回改**。**本次接受只推進第六項 rollout 前置條件（5／6 → 6／6）**：persisted batch reader／writer、Step 5／6、artifact emission、ledger 與 arbiter **仍未接受、仍未開始**，`unsupported-populated-inventory` **仍不得解除**，**AC118／AC128／AC136／AC137／AC138 仍不得宣稱滿足**，**Phase 2 仍不得宣稱 READY**。
