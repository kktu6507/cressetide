...
digest 相容性     : `bodyDigest`／`declarationDigest` 的**公式不變**，
                  改變的是**輸入 bytes**。既有以 @src 入 digest 產生的
                  任何 persisted digest 因此**不再可重算相等** ——
                  本版**不**定義其 migration；populated inventory 仍不得接受，
                  gate 仍不得解除，因此目前**不存在** durable 受影響資料。
                  日後若出現，必須另立並核准 migration／rebuild policy。
...
produceChangedTestInventoryV2({ repoRoot, baseTreeOid })

兩者各**恰收一個 argument**，request **exact key set 恰為** { repoRoot, baseTreeOid }；
...
```
pre-binding（base 側 tag）:
  對 **B** —— `baseTreeOid` 指向的 **immutable historical** store —— 驗證:
    clause 形態的 tag: 其 clauseRef **必須**在 B 中可解析
    其 Source **必須**在 B 中可解析
  —— **只**對 B 驗。base binding 是**歷史事實**:
     一個在 C 中已 retire／已 supersede／已 expire 的 clause，
     其 base 側綁定**仍然合法**，**不得**以「現時不再有效」誤擋。
     現時效力只決定 governanceHit，**不**決定 base binding 是否合法。

post-binding（head 側 tag）:
  對 **C** —— **同一次 G1 parsed** current store，**不得**重讀 —— 驗證:
    clause 形態的 tag: 其 clauseRef **必須**在 C 中可解析
    該 clause 在 C 中**必須**為 active（依 §2／§9 既有 status 推導，
      **不**另立第二套 status 判準）
    REQ@DP 形態: 其 REQ 與 DP **必須**皆在 C 中可解析，
      且該 DP 的 current terminal **必須**正是該 REQ
      —— 不符即 fail-closed，**不得**降級為 EXPL、**不得**省略該 entry

{ expl: true } 與 null:
  無 clause 語義，**不**進行上述解析；其既有 §2 規則不變。

失敗處置:
  **fail-closed、no result、no partial carrier、no entries、no-write**
  —— 錯誤必須指名該 testRef 與該 binding，並指名**是哪一側**（base 或 head）

快照邊界:
  parsed B 與 parsed G1 **可**在 invocation 內部保留供本節使用 ——
  它們本來就已在同一次 invocation 中被讀取與驗證，重讀反而會開啟
  §11b.10c step 2 禁止的第二個讀取窗口。
  但**不得**因此:
    加入 GovernanceSeedPreimage 的四欄 carrier（仍**恰四欄**）
    加入 ChangedTestInventoryV2 的七欄 envelope（仍**恰七欄**）
    加入 public request（仍**恰為** { repoRoot, baseTreeOid }）
