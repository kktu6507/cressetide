...
```
buildGovernanceSeedPreimage({ repoRoot, baseTreeOid, taskId })
produceChangedTestInventoryV2({ repoRoot, baseTreeOid, taskId })

兩者各**恰收一個 argument**，request **exact key set 恰為** { repoRoot, baseTreeOid, taskId }；

  —— **v1.16**：taskId 是**新增的第三個 key**，理由是 §7 的 REQ@DP 綁定需要
     currentTaskDpIds，而它掛在 **TaskState** 上。一個 store 可持有**多個** TaskState，
     因此 { repoRoot, baseTreeOid } **無法**唯一決定 current task —— 從多個之中猜一個
     正是本模型別處一律 fail-closed 的那種推測。taskId 因此是 caller **必須**說出來的
     事實，**不是** injection：它不供給任何被觀測的輸入，只指名要對哪一個 task 求值。
  —— taskId 的解析（exact；對**同一次 G1** 求值，**不得**另讀）:
...
失敗處置:
  **fail-closed、no result、no partial carrier、no entries、no-write**
  —— 錯誤必須指名該 testRef、該 binding，並指名**是哪一側**（base 或 head）

Producer 的 G1／G2 orchestration（v1.16；exact）:
...
快照邊界:
  ...
  但**不得**因此:
    ...
    加入 public request 之外的任何入口（request 仍**恰為**
      { repoRoot, baseTreeOid, taskId }）
...
```
producer 必須**自行**以**同一份 exact request**（{ repoRoot, baseTreeOid, taskId }）取得:
...
共同要求（全部必須滿足）:
  ...
  no seam     : **不得**新增 production export、request key、
                ...
                production request contract 仍**恰為**
                { repoRoot, baseTreeOid, taskId }，上方 anti-injection 清單**一字不變**。
...
176. **Binding semantic validation（v1.16）**（carrier：§11b.10c 「Binding semantic validation」＋ §2 binding 兩相 ＋ shared approved v1.15 §9）：本 AC 定義的是**未來 producer 的 executable contract**；**本 AC 已隨 test-provenance approved v1.16 的 2026-08-22 promotion 生效；契約生效**不代表**下列任何一格已有測試、已實作或已通過**。**(1) dangling head binding**：head 側 tag 為 `{ clauseRef }`，而該 clauseRef 在 **C** 中**不可解析** → **fail-closed**，**no result、no partial carrier、no entries、no-write**；錯誤必須指名 testRef、該 binding，並指名這是 **head** 側。**單變數對照**：其餘完全相同、clauseRef 改為一個在 C 中**滿足 (11) 全部 post-binding 義務**的 clause（可解析 ∧ `active ∧ mechanicallyApplicable` ∧ Source Check A/B ∧ exception chain 有效）→ 正常產出。**此對照與 (11) 共用同一個成功門檻** —— 本 AC **不得**存在兩套寬嚴不同的 positive 標準。**(2) 已知的實測反例**（本 AC 必須抓到）：repo 內**完全沒有** provenance store，一個 head-only test 綁 `REQ-<不存在的 ULID>` —— 目前 producer 輸出 `added`；本 AC 之下**必須** fail-closed。**(3) head binding 指向非 active clause**：clauseRef 在 C 中可解析但**非 active**（已 retire／已被 supersede） → **fail-closed**；status 依 §2／§9 既有推導，**不得**另立第二套判準。**(4) REQ@DP 形態**：其 REQ 與 DP 皆須在 C 中可解析，且該 DP 的 **current terminal 必須正是該 REQ**；不符 → **fail-closed**，**不得**降級為 `{ expl: true }`、**不得**省略該 entry。**單變數對照**：DP 的 current terminal 正是該 REQ，**且** §7 其餘四項（DP ∈ currentTaskDpIds、status == resolved、applicable(REQ, DP)、scopeRulingRef.subjectRef == DP）**與** (11) 的全部 post-binding 義務**皆成立** → 正常產出。**明文禁止**：**不得**單以「current terminal 相等」推出正常產出 —— 那會與 (12) 的五項門檻互相矛盾。**(5) 歷史 base binding 不得被現時效力誤擋**：base 側 tag 指向的 clause 在 **B** 中可解析、其 Source 在 B 中可解析、**且 Check A 成立**（即 (10) 的 pre-binding 義務**全部**滿足），但在 **C** 中已 retire／已 supersede／已 expire → **必須正常產出**，**不得** fail-closed。base binding 是歷史事實；現時效力**只**決定 `governanceHit`，**不**決定 base binding 是否合法。**(6) dangling base binding**：base 側 clause-form tag 在 **B** 中**不可解析** → **fail-closed**，錯誤指名這是 **base** 側。**(7) `{ expl: true }` 與 `null`**：不進行 clause 解析，其既有 §2 規則不變 —— **不得**因本 AC 而被新拒絕。**(8) 快照與讀取邊界**：post-binding 驗證**必須**使用 §11b.10c step 1 的**同一份 G1 parsed** store，**不得**重讀 —— 可執行證據沿用 §11b.10c 形狀 B 的 counting proxy：成功 invocation 的 ordered load trace **仍恰為 G1、G2 兩筆**。**(8b) G1 → binding → G2 的 ordering evidence（總讀取數不足以證明）**：AC171 (vii) 與上方 (8) 只證明成功 invocation 的 current-store fresh-load **總數恰為 2**；那與「binding validation 發生在 G2 之後」**完全相容**，因此**不足以**證明本節要求的順序。**另加一格**，evidence shape 為既有 §11b.10c **形狀 B**：於獨立 child process 內對 scratch source copy 施加**單一點**編輯，在 **binding validation 完成之後、G2 fresh-load 之前**同步改動 scratch repo 的 `.ctide/provenance.json`（同步施加，**不是**並行寫入）。**期望**：該 invocation **必須**由 **G2 得到 store-moved fail-closed**（`G1 != G2` 的 digest 不等），且 **no result、no partial carrier、no entries、no envelope**。**這格為什麼有證據力**：若實作把 G2 放在 binding validation **之前**，這個改動會落在 G2 之後，G2 便看不到它、invocation 會**成功**回傳 —— 該測試因此 fail。**不得**依賴 timing race、**不得**對 live repo 並行寫入、**不得**新增任何 production seam。**(9) carrier 不得擴張**：`GovernanceSeedPreimage` 仍**恰四欄**、`ChangedTestInventoryV2` 仍**恰七欄**、public request 仍**恰為** `{ repoRoot, baseTreeOid, taskId }`、§11b.10c anti-injection 清單**一字不變** —— parsed B／G1 只在 invocation 內部保留，**不得**成為 public operation、persisted object 或 caller-supplied 入口。**(10) pre-binding 完整委派**：驗 base 側 clause binding 只課 shared §9 preChangeBinding 列的**全部**條件（clause 與 Source 在 B 中可解析 ∧ **Check A**）；**負例**：Check A 不成立（B 中 Source 的 digest 與 excerpt 不符）→ **fail-closed**。**明文正例**：base clause 在 C 中已 retire／已 supersede／已 expire、Source 已 drift、exception 已過期 —— **全部必須正常產出**，**不得**課 active／mechanicallyApplicable／Check B／expiry。**(11) post-binding 完整委派**：驗 head 側 clause binding 課 shared §9 postChangeBinding 列的**全部**條件，於**同一 G1、同一 T0**：`active ∧ mechanicallyApplicable`（per-kind）、Source **Check A 與 Check B 兩者**、**完整 exception chain**（targetConstraintRef 可解析、grantAuthorityRef == target REQ 的 ownerRef、expiryInstant > T0）。**四個單變數負例各一**：只讓 mechanicallyApplicable 不成立／只讓 Check B 不成立／只讓 owner 不相符／只讓 expiry 已過 → **各自 fail-closed**，且錯誤必須可區分。**(12) §7 五項全驗**：限定形式 `@src REQ-x@DP-y` 逐項驗 DP-y ∈ currentTaskDpIds、status == resolved、resolvedBy == REQ-x、applicable(REQ-x, DP-y)、scopeRulingRef.subjectRef == DP-y —— **五個單變數負例各一**，任一不成立即 fail-closed。**(13) 非 exception-backed 的 REQ@DP**：一個非 exception-backed 的 REQ 帶 @DP qualifier → **fail-closed**。**(14) 裸形式的 0／1／many**：exception-backed REQ 的裸形式**只在** currentTaskDpIds 內反推 resolvedBy == REQ-x 的 DP —— 恰一個候選 → 自動綁定並**續驗 (12) 五項**；**零個** → fail-closed；**多於一個** → fail-closed，訊息要求改用限定形式；**明文負例**：候選存在於**歷史 DP**（不在 currentTaskDpIds）→ **不列入**，因此仍是零個候選 → fail-closed。**(15) taskId 解析**：G1 的 taskStates 中恰一個 taskId 相符 → 採用；**unknown task（零個相符）** → fail-closed；**多於一個相符** → fail-closed；採用之 TaskState 的 `baseProvenance.treeOid != request.baseTreeOid` → **fail-closed**（**base mismatch**，且**不得**偏袒任一方）。**(16) 多 TaskState discriminator**：store 持有**兩個以上** TaskState 時，**不得**由 DP、head view 或任何啟發式挑選 current task —— 未給 taskId、或給了但不唯一 → fail-closed；**單變數對照**：給出正確且唯一的 taskId → 正常產出。**store 只有一個 TaskState 時同樣必須給 taskId**，**不得**略過該 key 或略過 base mismatch 比對。**(17) taskId 不進 carrier／envelope**：驗四欄 carrier 與七欄 envelope **皆不含** taskId，且 taskId **不**進任何 digest 的 preimage。**(18) 失敗的副作用邊界**：上述任一 fail-closed 之後，`.ctide/provenance.json`、explicit config（`.ctide/test-adapters-config.json`）、shipped registry（`test-adapters.json`）與 `.ctide/output/**` 的 bytes **逐一不變**。
