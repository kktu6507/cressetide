...
失敗處置:
  **fail-closed、no result、no partial carrier、no entries、no-write**
  —— 錯誤必須指名該 testRef、該 binding，指名**是哪一側**（base 或 head），
     並指名**是下方九項義務中的哪一項**（v1.17；只說「binding 無效」不足）
  —— 例外**只有**下方 carve-out 明列的三項，且**只在**有對應 witness 時
```
...
```
postChangeBinding 列的**不可再分義務**與其可豁免性（closed；**恰九項**）:

  ob-1  clause 在 C 中可解析                                    **永不可豁免**
  ob-2  該 clause 的 direct Source 在 C 中可解析                **永不可豁免**
  ob-3  status(c) == active                                     可豁免 ⇔ witness-1
  ob-4  Source **Check A**（immutable snapshot integrity）      **永不可豁免**
  ob-5  Source **Check B**（對 H 的 live-current occurrence）   可豁免 ⇔ witness-2
  ...
  **三對的可分離性並不相同**（依 shared approved v1.15 §9 的 derivation 導出，
  記錄於此以免實作者去構造不存在的案例）:
    ...
    ob-5 ↔ witness-2 : **重合**。driftedClauses 的成員條件就是「direct、repo-file 的
                       Source 對 H 得 zero occurrence」，而 post-binding 的 Check B
                       課的正是同一個 direct Source；driftMode=snapshot-only 依上游
                       **永不算 drift**。因此「ob-5 失敗但無 witness-2」**不可構造**。
    ob-8 ↔ witness-3 : **重合**。expiredClauses 的成員條件就是「REQ 的 sourceRef 指向
                       exception-grant 且 expiryInstant <= T0」，即 ob-8 的失敗本身；
                       DEC／ASSUM 不因 basisRefs 含 exception-grant 而入集合，
                       而 ob-8 也只對 exception-backed REQ 課。因此
                       「ob-8 失敗但無 witness-3」**不可構造**。
    —— 重合**不表示** witness 比對可以省略：省略之後 union 與逐項對應**不可區分**，
       ob-3 那一對也會跟著失守。實作**仍必須**逐項比對。
    —— 上述兩處「不可構造」**必須被記錄**，**不得**以任何近似案例假造，
...
**仍然一律拒絕**（**不得**因本節而放行）:
  ...
  ob-6／ob-7 不成立 —— shared approved v1.15 §9 明文：malformed chain、dangling
    targetConstraintRef、owner mismatch 是 **validation failure，不是 membership**
  ob-9 不成立
