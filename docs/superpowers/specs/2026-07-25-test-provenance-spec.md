...
mode／type mapping、base／head projection）:
  ...
  has(P) 其中 P 為 canonical repo-relative Git path 且**不在** view 內
                    -> **恰為 false**（不是 throw、不是 undefined）。
  entry(P) / read(P) 其中 P canonical 但**不在** view 內
                    -> **fail-closed**（**不得**回 null／undefined／空 buffer，
                       那會讓「不存在」與「存在但空」無法分辨）。
  has / entry / read 收到**非** canonical repo-relative Git path
                    （例如含 "\"、前導 "/"、drive prefix、"." 或 ".." segment、
                      空 segment、或大小寫與 Git literal 不符者）
                    -> **fail-closed**；
                       **不得** normalize、**不得** case-fold、
                       **不得**以 OS path 規則修補後再查。
