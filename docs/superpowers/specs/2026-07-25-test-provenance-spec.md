...
identity 必須升版  : **這是新的 digest 演算法，因此必須是新的 implementation identity。**
                  ...
                  `node-test-v1` 升為 **`node-test-v2`**（或另一個明確的新 identity），
                  §11b.3 registry 的 exact binding 同步改變，`registryDigest`
                  因此也隨之改變。
...
digest 相容性     : `bodyDigest`／`declarationDigest` 的**公式不變**，改變的是
                  **輸入 bytes** 與 **implementation identity**。
                  ...
                  本版**不**宣稱不存在、**不**捏造 migration:
                    歷史 bytes    : 仍可作為**歷史**文件讀取
                                   （legacy reader 路徑不受本版改動）
...
快照邊界:
  parsed B、parsed G1 與 T0 **可**在 invocation 內部保留供本節使用 ——
...
```
producer 必須**自行**以**同一份 exact request**（{ repoRoot, baseTreeOid }）取得:
...
共同要求（全部必須滿足）:
  ...
  no seam     : **不得**新增 production export、request key、
                ...
                { repoRoot, baseTreeOid }，上方 anti-injection 清單**一字不變**。
