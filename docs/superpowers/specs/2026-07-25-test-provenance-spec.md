...
closed enums（v1）:
  language         : "javascript"
  framework        : "node:test"
  implementationId : "node-test-v2"   ← **v1.16 draft**：`node-test-v1` 為歷史 identity，
...
**任何其他 `framework` 搭配 `node-test-v2` → fail-closed**；**v1.16 draft**：`node-test-v1` 是歷史 identity，出現在 current registry 中即 **fail-closed**；表中未列的欄位組合亦 fail-closed。每一欄都是**恰為**，不是「子集」或「可為空」—— v1 只有一個合法組合，registry 沒有自由度。
...
node-test-v2 的綁定值（closed；v1.16 draft —— 值不變，綁定的 identity 改變）:
...
### 11b.8b Canonical declaration bytes 與 directive 排除（exact；v1.16 draft 起含 `@src`）
...
hashing 之前   : 自該 range 逐行掃描，移除**合法且已附著**的 directive 行 ——
                 **v1.16 draft 起為 @tid 與 @src 兩者**，以**同一個**演算法、
...
**v1.16 draft 的直接後果（推導，非另立規則）** —— 移除合法附著的 `@src` 行之後：
...
兩者各**恰收一個 argument**，request **exact key set 恰為** { repoRoot, baseTreeOid, taskId }；
  ...
  —— **v1.16 draft**：taskId 是**新增的第三個 key**，理由是 §7 的 REQ@DP 綁定需要
...
**Binding semantic validation（v1.16 draft；exact）**
...
Producer 的 G1／G2 orchestration（v1.16 draft；exact）:
...
```
**v1.16 draft effective addendum（status carrier，非 normative；不回改上方 dated 2026-08-16 敘述，也不回改 AC174）** —— v1.16 把 current implementation identity 由 `node-test-v1` 收斂為 `node-test-v2`。**spec promotion 本身不使任何 shipped artifact 變成、或被接受為 `node-test-v2`。**
...
六項 rollout 前置條件的計數（closed set 不變，只是這一項的狀態隨 v2 移動）:
  v1.16 promotion 後、v2 implementation 尚未接受 : **4／6**
