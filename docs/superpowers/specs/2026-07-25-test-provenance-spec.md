...
兩者各**恰收一個 argument**。request 的 exact key set **兩者不同**，這是 **v1.17 的更正**:
  ...
  —— **buildGovernanceSeedPreimage() 不做上述任何一步**：它不收 taskId、不解析 task、
     **不讀 taskStates**，因此 current store 缺席或為 canonical empty 時它**仍必須成功**
     —— AC171 (B)(ix)／(xi) 因此回到可達。
  ...
  該要求不可實現，撤回）**:
    ...
    層級 1 —— **G1 的 version／schema／invariant validation**（本節順序 step 1
      ...
      **本層已先行擋下的情形（實測，非推測）**：exception-grant 的
        ...
        以自己的 code 拒絕；DP terminal 非 active＋applicable 亦於 clock-armed
        階段由 **INV-4** 拒絕。
      **因此**：ob-6／ob-6b／ob-7 在**合法 G1** 之下**到不了** binding validation。
        它們仍是 postChangeBinding 列的義務、仍必須被實作**課**（B 側與未來
        放寬的 store 皆可能觸及），但**其證據在層級 1**，**不得**要求層級 2 的
        指名，**更不得**為了「讓它在層級 2 被觀察到」而放寬 store validation。
...
AC176 既有引用整體位移:
  ...
  **direct Source set 是集合，不是單數**（v1.17；前一稿以單數寫 ob-2／ob-4／ob-5，
  ...
  —— ob-3、ob-5、ob-8 **同時**出現在 shared §2 `mechanicallyApplicable(c)` 之內
     與 §9 postChangeBinding 列之上。**豁免一次即對兩處同時生效**，否則
     carve-out 恆為空集合（clause 仍會在 mechanicallyApplicable 那一層被擋下）。
     `mechanicallyApplicable()` 的**其餘**項（ob-9）仍**照課**。
  —— **不得**以「呼叫 applicable() 即等於全驗」自證：`applicable()` 拿不到 **H**，
     因此**不可能**承載 ob-5。ob-5 必須由實作對 H 明確求值。
...
Producer 的 G1／G2 orchestration（v1.16；v1.17 修訂 step 6 與內部 context 邊界；exact）:
  standalone buildGovernanceSeedPreimage() **可**保留其既有形狀 ——
    ...
    **v1.17**：它**不做** task resolution、**不讀** taskStates，其順序中
    因此**沒有**下方順序的 step 3（taskId resolution）；current store 缺席或為 canonical empty 時
    它**仍必須成功**。
...
approved v1.15 的範圍  : **只**定義未來 producer 如何取得並綁定它；
                         契約已於 2026-08-21 生效，**但尚未實作、尚未接受**。
producer               : **尚未實作、尚未接受**。
GovernanceSeedPreimage : **尚未實作、尚未接受**。
