...
六項 rollout 前置條件的計數（closed set 不變，只是這一項的狀態隨 v2 移動）:
  **現況** —— v1.16 已 promote、v2 implementation 尚未接受 : **4／6**
      —— adapter registry 因 identity 尚未一致而**退回未完成**；producer 仍未完成
  v2 registry／manifest／executable chain 接受後  : **5／6**
      —— 只剩 producer
  producer 接受後                                : **6／6**
