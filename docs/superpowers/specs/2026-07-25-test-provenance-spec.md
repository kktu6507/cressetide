 ...
 9. closure 完成後、**回傳前**再 fresh-load current store 為 **G2**。
    **G2 必須套用與 G1 完全相同的 current-store version rule**
    ...
    並要求 **G2 的 canonical digest == G1 的 digest**。
      —— G1 與 G2 **任一缺席**都代表**同一份** canonical empty v2 store，
         因此「兩側皆缺席」是**相等**；而 **missing ↔ present 的轉換必然不相等**，
         必定被本條擋下。
...
173. **Populated inventory producer 的 end-to-end executable matrix**（carrier：§11b.10c ＋ §6 ordered classification ＋ §11b.9c envelope ＋ shared draft v1.15 §9 四集合）：本 AC 定義的是**未來 producer 的 executable contract**；**本輪 spec-only，不代表下列任何一格已有測試、已實作或已通過**，且**在兩份 draft v1.15 同步 promotion 之前一律不得實作**。**本 AC 完整承載 AC169 的 producer cases (1)–(15)** —— 其輸入、evidence shape（anchor／control、pairwise precedence、compound stress 三類）與 expected results **一律原樣有效、不得改寫或弱化**；差別**只在**本 AC 要求它們由**真實 producer 端到端**產出 persisted entry，而非只驗 classification 表。另加：**(a) 四種 governance seed 各自的 end-to-end governance-only case** —— 同一個 body 與 tag 皆未變的 same-path pair，分別因 (a1) `semanticallyChangedClauses`（`C` 新增的 Clause 正是該 test head 側 binding 的 `clauseRef`）、(a2) `transitionedClauses`（`C` 新增一筆生效 Transition，其 `subject` 正是該 `clauseRef`）、(a3) `driftedClauses`（該 clause 直接引用的 `driftMode=repo-file` Source 對 `H` 的 Check B 得 zero occurrence）、(a4) `expiredClauses`（該 REQ 的 exception-grant Source 之 `expiryInstant <= T0`）而命中 → **四者各自**產生 `governance-affected`，且 `reason == governance-affected`、兩個 body digest **相等**、`tagBefore` 與 `tagAfter` canonically **相等**、每格**只有一筆** entry；**單變數對照**：四格各自搭配一個「seed 未命中、其餘完全相同」的 control，該 control 必須**自 entries 省略**。**(b) precedence 不被 governance 覆蓋** —— body change ＋ governance hit → `modified`；tag-only change ＋ governance hit → `retagged`；moved ＋ governance hit → `moved`，三者 `reason` 皆為 `content-change`；**(c)** `unchanged` **完全省略**，**不得** emit placeholder；**(d)** 任一 logical test **不得**出現兩筆 entry，也**不得**帶第二個 status —— reverse closure **不得**為已由 `added`／`deleted`／`moved`／`modified`／`retagged` 收錄者再新增一筆；**(e) 六種 status 的 exact side projection 逐欄驗** —— `added` 全取 head、`deleted` 全取 base，其餘四種 `testRef`／`framework`／`implementationIdentity` 取 head 而 `tagBefore`／`baseBodyDigest` 取 base、`tagAfter`／`headBodyDigest` 取 head；且 `added` 的 `baseBodyDigest`、`deleted` 的 `headBodyDigest` **必須缺席**（**不是** null）；**(f)** entries 依 `(path, adapterId, structuralId)` code-point tuple **嚴格遞增**，nested object 依既有 canonical key ordering；**(g)** envelope **exact key set 恰為七欄**，且 `inventoryDigest` 以 §11b.9c 的唯一公式重算後相等 —— **不得**出現 `evaluationTime`／`producedAt`／`clockDigest` 或任何等價欄位，`inventoryVersion` 仍為 `2`；**(h)** producer 的輸出必須能通過 `parseCanonicalInventoryV2`；**(i)** **不得**以「產品 `parseInventory()` 已放行」作為成功證據 —— 產品入口**仍必須**以 `unsupported-populated-inventory` 拒絕同一份文件，兩者**同時**成立才算通過；**(j)** producer invocation 前後，`.ctide/provenance.json`、explicit config、`test-adapters.json` 與 `.ctide/output/**` 的 bytes **逐一不變**（含 store 未被 migration、未被回寫）。
