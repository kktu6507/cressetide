...
**AdapterContentView 與 DiscoveryAnalysisPreimage 是已接受的 foundation component，**不是**新的 rollout 前置條件** —— 上表六項前置條件的內容、「已有五項實作並接受、只剩 producer」的計數與語義**完全不變**，它們也**不使** populated inventory producer 自動完成。
...
168. **Draft／implementation boundary（test-provenance v1.13）**（carrier：本文件 live status ＋ §11b.12 rollout boundary）：**v1.13 仍是 draft、未審核** —— 2026-08-15 由使用者明確核准起草，**尚未 promotion、未經 panel、未經使用者核准為 approved**；**本輪未執行 agent-duel panel，因此不得稱為 panel 放行**。**最後 approved baseline 仍為 test-provenance approved v1.12**，**current approved coupled set 仍為 shared approved v1.14 ＋ intent-scan approved v1.10 ＋ test-provenance approved v1.12**，未撤回、未取代。**本輪只建立 spec authority，不修改、不修正也不接受任何 implementation。**commit `898f81c` 的 base／head declaration matcher **不因本 draft 成為 ACCEPT**，仍是 **provisional、未 ACCEPT** 的實作，且在 draft v1.13 下**已知不符**（residual 兩側同時存在時它輸出 added＋deleted）；**matcher remediation 尚未實作**，須另行取得 component implementation 授權、完成實作並通過獨立審查。`modified`／`retagged` 的分類問題**本版未閉合，也未順手決定**。**仍未完成**：populated inventory producer、governance reverse closure、artifact emission，以及 Step 5／6、ledger、arbiter wiring。因此：**不得**接受 populated inventory；**不得**解除 `unsupported-populated-inventory`；**AC118／AC136／AC137／AC138 一律不得宣稱已滿足**；**Phase 2 不得宣稱 READY**；**AC138 及其附加限制不因本 draft 而弱化**（AC138 本輪 byte-identical，AC166 亦 byte-identical）。
