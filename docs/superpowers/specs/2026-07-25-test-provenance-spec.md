...
### 11b.10c Populated inventory producer input carrier 與 governance-closure boundary（v1.15 draft）
...
```
 ...
 6. **Base-store version boundary**：base governance state（shared draft v1.15 §9 的 `B`）
    ...
    **不得**以 live current store 冒充 base —— 那會讓「本次是否變更」在同一份輸入上
    自我比較，得到恆為空的 lifecycleAffectedClauses。
      —— .ctide/provenance.json 是 §11b.10 判定順序第 1 步的 hard exclusion，
         ...
         兩者互補，**不**衝突。
 7. lifecycleAffectedClauses 的 membership **完全委派** shared **draft v1.15**
    ...
    —— 其 `B`／`C`／`H`／`T0` 定義、cross-snapshot immutability（typed-ID set difference
       ...
       **也不**另立第二個 seed 名稱。
    —— **本條在 shared draft v1.15 與本文件 draft v1.15 同步 promotion 之前，
       一律不得實作。**
...
```
**Draft v1.15 boundary（status carrier，非 normative）** —— 上表的**六項 rollout 前置條件仍是同一個 closed set**，其內容、計數與語義**完全不變**：**六項已有五項實作並接受，只剩 producer**。draft v1.15 新增的 §11b.10c **不動這個計數**：

```
GovernanceSeedPreimage : producer 的 **mandatory supporting input component**，
                         **不是**第七項 rollout prerequisite。
draft v1.15 的範圍     : **只**定義未來 producer 如何取得並綁定它；
                         在 v1.15 promotion 之前，其新增契約一律不得實作。
producer               : **尚未實作、尚未接受**。
