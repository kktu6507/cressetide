...
> **`ULID` 的 authority 在上游（v1.8 新增）** —— 上式中的 `ULID` **唯一引用 intent-scan draft v1.10 §8 的 canonical ULID grammar**。
...
>
> - 在 intent-scan v1.10 尚未 promotion 之前，**生效的 coupled baseline 仍是 intent-scan approved v1.9 ＋ test-provenance approved v1.7**；draft 規則**不得實作**。
...
path 引數位置    : index 0，且必須是下列**唯一**合法形式：
                     new URL(<字串字面值>, import.meta.url)
                   —— 選 URL 形式是因為它相對於**模組自身**解析，結果與 process cwd 無關；
                      裸的 cwd-relative 字面值（readFileSync("fixtures/a.json")）
                      **unsupported**，它的意義取決於呼叫者的工作目錄，不可重現。
相對解析根       : 該 import.meta.url 所屬模組在**同一 view** 內的 canonical path
...
108. **snapshot-golden 的 exact 契約**（carrier：§11b.9 snapshot-golden 區塊）：正例 —— `readFileSync(new URL("fixtures/golden.txt", import.meta.url))`，specifier 為 `node:fs`，path 引數在 index 0，相對於該模組在同 view 的 canonical path 解析 → 產生預期 `depRef`。**單變數負例（其餘皆同）**：(i) 改用未列於 allowlist 的 fs API（例如 `readdirSync`）→ unsupported fail-closed；(ii) 改成裸 cwd-relative 字面值 `readFileSync("fixtures/golden.txt")` → unsupported（意義取決於 process cwd，不可重現）；(iii) 改成 computed 或變數 path → unsupported；(iv) path 逃逸 repo root → fail-closed。
...
152. **Draft／rollout boundary（v1.8）**（carrier：本文件 live status ＋ §11b.12 rollout boundary）：test-provenance v1.8 **仍是 draft**，最後 approved baseline 仍為 **v1.7**。node-test-v1 executable component **尚未因本輪而實作**；`structuralId`、tag attachment、`effectiveOracleDeps` 三者的 implementation readiness **不得由本輪推出**。**AC136／AC137／AC138 未因此滿足**；`unsupported-populated-inventory` **不得解除**；**不得**宣稱 Phase 2 READY。本輪只做兩件事：把 §2 的 `ULID` 指向上游 intent-scan §8 的唯一 authority，並補上 adapter 端的 accounting AC（AC151）。**AC1–150 的編號與文字一字未動**，AC138 亦未弱化。
