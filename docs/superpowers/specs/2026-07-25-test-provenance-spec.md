...
3. tracked .gitignore exclusion —— 適用於**其餘所有** path:
     被 tracked .gitignore 忽略者**排除**（ignore authority 見下）。
...
92. **Explicit config carrier 與 read-only 邊界**（carrier：§11b.4 tracked carrier 判定 ＋ §11b.10 snapshot metadata）：`.ctide/test-adapters-config.json` 的 `configVersion != 1`、未宣告 key、`assignments[].path` 重複、`adapterId` 未註冊 → 各自 fail-closed。另驗它是 **tracked committed project configuration**，**producer 對它 read-only** —— 檔案不存在時**不得**自動建立，內容不合法時**不得**改寫或補齊，只能 fail-closed。**v1.11 新增可區分案例（三者之間只差 `tracked` 一個變數）—— 本 AC 的 fixture repo 必須帶一份 tracked `.gitignore`，其中含 `/.ctide/`**（真實 consuming project 的常態；少了它就測不到 §11b.10 判定順序第 2 步，本 AC 要擋的東西也就測不到）：(i) **tracked control** —— 一份合法 bytes 的 config，其 path 位於 S3 snapshot 的 index stage-0 set（`entry(path).tracked == true`）→ **合法 carrier**，其 parsed object 進入 `registryDigest` preimage；(ii) **純 untracked、同 path、逐 byte 相同的 config** —— **即使被該 `/.ctide/` 規則忽略，它仍必須出現在 snapshot 內**（§11b.10 判定順序第 2 步）且 `entry(path).tracked == false` → **invalid config carrier，fail-closed** —— **不得**接受、**不得**視為「config 不存在」、**不得**以 `explicitConfig: null` 續行、**不得** `git add`／建立／刪除／改寫或修補任何檔案（失敗前後 carrier bytes 必須逐字相同）；(iii) **該 path 實際不存在於 worktree**（因而也不在 snapshot 內）→ 此時、也**只有**此時，`explicitConfig` 才**恰為** `null`；**不得**把 (ii) 的 untracked 情形讀成本情形。**判定只用 snapshot metadata**：consumer **不得**在 snapshot 建立後再做第二次 Git 或 filesystem discovery。另須明確斷言：untracked config 仍依 §11b.10 的完整 universe 規則**進入 `headViewDigest`** —— **「進入 head universe」不等於「有資格成為 explicitConfig carrier」**，兩者是各自獨立的判準（見 AC160）。
