...
3. digest 記法沿用 §11b.9c 的 provenance-store CAS 記法
...
4. base governance state **只能**來自 baseTreeOid 指向的 immutable Git tree。
   **不得**以 live current store 冒充 base —— 那會讓「本次是否變更」在同一份輸入上
   ...
   —— .ctide/provenance.json 是 §11b.10 判定順序第 1 步的 hard exclusion，
      ...
      兩者互補，**不**衝突。
5. lifecycleAffectedClauses 的 membership **完全委派** shared approved v1.14:
     semanticallyChangedClauses ∪ transitionedClauses ∪ driftedClauses ∪ expiredClauses
   本文件**不重新定義、不擴張、不縮小**該集合，**也不**另立第二個 seed 名稱。
6. governance analysis 必須使用**一份通過 S1／S2 stability 的 head view**，
   並把該 view 的 headViewDigest 帶入 carrier；head-view-unstable → 不回傳。
7. closure 完成後、**回傳前**再 fresh-load current store 為 **G2**，
   要求 **G2 的 canonical digest == G1 的 digest**。
8. 下列任一情形 → **fail-closed、no result、no partial carrier、no-write**:
     G1 != G2
     head view 於 S1／S2 之間不穩定
     store schema／invariant 不合法
     baseTreeOid 非 exact tree，或其 base governance state 不可解析
     分析過程中的任何失敗
9. G2 成功之後 store 若再變動，由**既有 Step 5 pre-state CAS** 負責
...
171. **`GovernanceSeedPreimage` 的 exact carrier、store capture 與 anti-injection**（carrier：§11b.10c）：本 AC 定義的是**未來 component 的 executable contract**；**本輪 spec-only，不代表下列任何一格已有測試、已實作或已通過**。**逐項驗**：(i) **request exact key set 恰為 `{ repoRoot, baseTreeOid }`** —— 多一 key 即 fail-closed，第二個 argument 一律拒絕；(ii) **result exact key set 恰為 `{ baseTreeOid, headViewDigest, inputProvenanceStoreDigest, lifecycleAffectedClauses }`** —— 多一 key、缺一 key 或任何自由格式 metadata 皆 fail-closed；(iii) `lifecycleAffectedClauses` 每一項吻合 §2 canonical clauseRef grammar，**單變數負例**：其餘完全相同，只把其中一項改成不吻合上游 ULID grammar 的字串 → **拒絕**；(iv) **strict ordering**，**單變數負例**：其餘完全相同，只把相鄰兩項互換 → **拒絕**；(v) **duplicate 拒絕**，**單變數負例**：其餘完全相同，只把其中一項重複一次 → **拒絕**（**不是**去重）；(vi) root 與所有 nested value **深度 frozen**，任何寫入嘗試皆不得生效；(vii) **G1 的同一份 parsed store 同時供 validation、`inputProvenanceStoreDigest` 與 seed derivation** —— 必須有可執行證據顯示三者之間**沒有第二次讀取**；(viii) **缺檔**時 `inputProvenanceStoreDigest` 取既有 canonical empty store digest（**不是** null、不是空字串），且記法為 `sha256(canonicalText(…))`，**單變數負例**：同一份 store 只加 BOM、或只把 LF 改成 CRLF，digest **必須不變** —— 會變者即是誤用 raw-bytes 記法，**拒絕**；(ix) **G1／G2 store mutation**：分析途中改動 `.ctide/provenance.json` 使 G2 != G1 → **fail-closed、no result、no partial carrier、no-write**；(x) head view 於 S1／S2 之間不穩定 → **fail-closed**；(xi) base governance state **只能**來自 `baseTreeOid` 指向的 exact tree，以 live current store 冒充 base 者**拒絕**；(xii) **anti-injection 逐項拒絕** —— prebuilt `DiscoveryAnalysisPreimage`／`GovernanceSeedPreimage`、store bytes、已 parsed 的 store、store digest、`lifecycleAffectedClauses`、`governanceHit` set、reverse-closure 結果、registry／registry path／registry root、parser、ignore matcher、Git executable／參數／environment、filesystem adapter、config object／path、modulePaths、view／snapshot、clock、capture hook、component module path、output path，以及任何等價 override、alias、test-only seam 或 environment argument，**一律不得**成為 public API 的入口。
172. **兩份 preimage 的 cross-binding**（carrier：§11b.10c orchestration）：本 AC 定義的是**未來 producer 的 executable contract**；**本輪 spec-only，不代表下列任何一格已有測試、已實作或已通過**。**正例**：(1) `discovery.baseTreeOid == governance.baseTreeOid == request.baseTreeOid` → 得以繼續；(2) `discovery.headViewDigest == governance.headViewDigest` → 得以繼續。**單變數負例（各只改動一邊，其餘完全相同）**：(3) 只把其中一邊的 `baseTreeOid` 換成另一個**合法**的 tree OID → **fail-closed**；(4) 只把其中一邊的 `headViewDigest` 換成另一個**合法**的 digest → **fail-closed**。**兩個負例都必須另外驗**：**不得**回傳任何 matcher pair、任何 entry、任何 envelope（**partial 亦不可**）；**不得**自動挑其中一邊；**不得**只重跑其中一份 preimage 再與另一份拼接 —— 重試只能是一次完整 invocation。另驗：(5) producer 的 public API **不接受** prebuilt `DiscoveryAnalysisPreimage`／`GovernanceSeedPreimage`／任何 digest／entries —— 傳入即 fail-closed，且**不得**存在任何等價 seam；(6) `registryDigest` 必須等於 `DiscoveryAnalysisPreimage` 已綁定的值，**單變數負例**：producer 另讀第二份 registry 後與之混用 → **拒絕**。
173. **Populated inventory producer 的 end-to-end executable matrix**（carrier：§11b.10c ＋ §6 ordered classification ＋ §11b.9c envelope）：本 AC 定義的是**未來 producer 的 executable contract**；**本輪 spec-only，不代表下列任何一格已有測試、已實作或已通過**。**本 AC 完整承載 AC169 的 producer cases (1)–(15)** —— 其輸入、evidence shape（anchor／control、pairwise precedence、compound stress 三類）與 expected results **一律原樣有效、不得改寫或弱化**；差別**只在**本 AC 要求它們由**真實 producer 端到端**產出 persisted entry，而非只驗 classification 表。另加：(a) **governance-only** —— body 與 tag 皆未變，但其 head 側 `clauseRef` ∈ `lifecycleAffectedClauses` → `governance-affected`，且 `reason == governance-affected`；(b) **precedence 不被 governance 覆蓋** —— body change ＋ governance hit → `modified`；tag-only change ＋ governance hit → `retagged`；moved ＋ governance hit → `moved`，三者 `reason` 皆為 `content-change`；(c) `unchanged` **完全省略**，**不得** emit placeholder；(d) 任一 logical test **不得**出現兩筆 entry，也**不得**帶第二個 status；(e) **六種 status 的 exact side projection 逐欄驗** —— `added` 全取 head、`deleted` 全取 base，其餘四種 `testRef`／`framework`／`implementationIdentity` 取 head 而 `tagBefore`／`baseBodyDigest` 取 base、`tagAfter`／`headBodyDigest` 取 head；且 `added` 的 `baseBodyDigest`、`deleted` 的 `headBodyDigest` **必須缺席**（**不是** null）；(f) entries 依 `(path, adapterId, structuralId)` code-point tuple **嚴格遞增**，nested object 依既有 canonical key ordering；(g) envelope **exact key set 恰為七欄**，且 `inventoryDigest` 以 §11b.9c 的唯一公式重算後相等；(h) producer 的輸出必須能通過 `parseCanonicalInventoryV2`；(i) **不得**以「產品 `parseInventory()` 已放行」作為成功證據 —— 產品入口**仍必須**以 `unsupported-populated-inventory` 拒絕同一份文件，兩者**同時**成立才算通過；(j) producer invocation 前後，`.ctide/provenance.json`、explicit config、`test-adapters.json` 與 `.ctide/output/**` 的 bytes **逐一不變**。
174. **Draft／implementation boundary（test-provenance v1.15）**（carrier：本文件 live status ＋ §11b.12 rollout boundary）：**v1.15 仍是 draft、未審核、未 promotion** —— 2026-08-16 由使用者明確核准起草本次 spec-only closure，**尚未經 panel 放行、未經使用者核准為 approved**；**本輪未執行 agent-duel panel，因此不得稱為 panel 放行**。**最後 approved baseline 仍為 test-provenance approved v1.14**，**current approved coupled set 仍為 shared approved v1.14 ＋ intent-scan approved v1.10 ＋ test-provenance approved v1.14**，三者為同一生效集合，不得分開採用。**本輪只閉合 spec authority**：**populated inventory producer 未實作**、**`GovernanceSeedPreimage` 未實作**、**artifact emission 與 Step 5／6、ledger、arbiter wiring 仍未完成**；§11b.10c 的新增契約在 v1.15 promotion 之前**一律不得實作**。既有的 base／head one-to-one matcher、`AdapterContentView`、`DiscoveryAnalysisPreimage`、canonical v2 reader 與 S3 source-freshness component 的 acceptance **一律不撤回**。**六項 rollout 前置條件的 closed set 與「五項完成、只剩 producer」的計數不變** —— `GovernanceSeedPreimage` 是 producer 的 mandatory supporting input component，**不是**第七項 prerequisite。因此：**不得**接受 populated inventory；**不得**解除 `unsupported-populated-inventory`；**AC118／AC136／AC137／AC138 一律不得宣稱已滿足**；**Phase 2 不得宣稱 READY**；**AC1–AC170 逐條 byte-identical**（含 AC116、AC118、AC129、AC138、AC165、AC167、AC169、AC170）。
