...
**Current implementation status（status carrier，非 normative；截至 2026-08-13）** —— 上表六項前置條件的現況。**六項全部滿足才可接受 populated inventory 的規則不因前三項已完成而放寬。**
...
not implemented:
  ...
  canonical v2 inventory parser
  base／head one-to-one matcher
  governance reverse closure
  consumer freshness（S3 重算）
...
92. **Explicit config carrier 與 read-only 邊界**（carrier：§11b.4 tracked carrier 判定 ＋ §11b.10 snapshot metadata）：`.ctide/test-adapters-config.json` 的 `configVersion != 1`、未宣告 key、`assignments[].path` 重複、`adapterId` 未註冊 → 各自 fail-closed。另驗它是 **tracked committed project configuration**，**producer 對它 read-only** —— 檔案不存在時**不得**自動建立，內容不合法時**不得**改寫或補齊，只能 fail-closed。**v1.11 新增可區分案例（三者之間只差 `tracked` 一個變數）**：(i) **tracked control** —— 一份合法 bytes 的 config，其 path 位於 S3 snapshot 的 index stage-0 set（`entry(path).tracked == true`）→ **合法 carrier**，其 parsed object 進入 `registryDigest` preimage；(ii) **純 untracked、同 path、逐 byte 相同的 config** → **invalid config carrier，fail-closed** —— **不得**接受、**不得**視為「config 不存在」、**不得**以 `explicitConfig: null` 續行、**不得** `git add`／建立／刪除／改寫或修補任何檔案（失敗前後 carrier bytes 必須逐字相同）；(iii) **該 path 不在 snapshot 內** → 此時 `explicitConfig` 才**恰為** `null`。**判定只用 snapshot metadata**：consumer **不得**在 snapshot 建立後再做第二次 Git 或 filesystem discovery。另須明確斷言：untracked config 仍依 §11b.10 的完整 universe 規則**進入 `headViewDigest`** —— **「進入 head universe」不等於「有資格成為 explicitConfig carrier」**，兩者是各自獨立的判準（見 AC160）。
...
160. **`tracked` 是 carrier-validity metadata，不是 digest 的第四欄**（carrier：§11b.10 snapshot metadata ＋ §11b.4）：HeadViewSnapshot 對每個 path 保留 immutable `tracked: boolean`，由**建立 snapshot 當次已捕捉的 index observation** 導出，並以**唯一**公開 carrier `entry(path).tracked` 暴露。**正例（單變數）** —— 把一個既有 path 由 untracked 改為 staged，bytes 與 mode **完全不變** → `entry(path).tracked` 由 `false` 變 `true`，而 **`headViewDigest` 必須完全不變**，因為 canonical map 仍**恰為** `path → { mode, type, contentDigest }` 三欄。**負例** —— 任何把 `tracked` 併入 canonical map 或 `headViewDigest` 公式的實作 → 上述正例會失敗，本 AC 必須抓出。**另驗**：同一份 bytes 的 config 在 tracked 與 untracked 兩態下 `headViewDigest` **相同**，但**只有 tracked 態**是合法 explicit-config carrier（AC92）—— 兩個判準各自獨立、互不代換；且 consumer **不得**在 snapshot 建立之後再做 live Git probe 取得 `tracked`。
...
162. **Draft／implementation boundary（test-provenance v1.11）**（carrier：本文件 live status ＋ §11b.12 rollout boundary）：**v1.11 仍是 draft、未審核、未 approved** —— 未經 panel 放行，也未經使用者 promotion 核准；**current effective coupled set 仍為 shared approved v1.14 ＋ intent-scan approved v1.10 ＋ test-provenance approved v1.10**。commit `711ec14` 的 S3 source-freshness component 是 **provisional、未 ACCEPT** 的實作；**本 draft 不修改也不修正任何 implementation**，canonical v2 reader（commit `4f44b6e`）的既有 acceptance **不因本 draft 撤回**。本 draft **只閉合三項 reader／carrier authority**，因此：**不代表**完整 Step 6、populated inventory producer、base／head one-to-one matcher、governance reverse closure 或 artifact emission／Step 5／6、ledger、arbiter wiring 已實作；**不得**接受 populated inventory；**不得**解除 `unsupported-populated-inventory`；**AC118／AC136／AC137／AC138 一律不得宣稱已滿足**；**Phase 2 不得宣稱 READY**；**AC138 及其附加限制不因本 draft 而弱化**（AC138 本輪 byte-identical）。
