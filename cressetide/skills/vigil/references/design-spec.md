...
The format follows the community `design.md` convention (Google `design.md` / VoltAgent `awesome-design-md`): a human-readable markdown document, optionally fronted by machine-readable tokens. ctide consumes it as text — there is nothing to parse or configure.

## Why it exists (the recall lever for UI)

Without a contract, `ui-ux-reviewer` judges consistency against "nearby patterns / the design language" it has to **re-infer every run**, and what it infers differs between runs. A `design.md` turns that into an objective check: a finding cites the **violated token or section**, not a taste judgment. The mechanism is that the decision is **recorded once, with its sources**, so later work cites the contract instead of re-deriving the design language (the same "derive once, reuse" discipline as the filtered diff in `references/review-packet.md`) and every reviewer judges against the same stated baseline rather than a freshly inferred one. That is a consistency and traceability property; no claim is made here about what it costs or saves.

## Format (10 sections)

The nine community sections plus one ctide extension:
...
`design.md` is a **committed** repository artifact at `.ctide/design/design.md` — it travels with the repo like source, unlike the gitignored run scratch under `.ctide/output/`. It is owned by the consuming project; ctide drafts and updates it but the project commits it. A repo-root `design.md` is the **compatibility location**: still detected (and guarded by `hooks/contract-guard.js`, which matches `design.md` by basename anywhere), but one-time-migrated post-approval on first consume (below).

**One-time migration (detected in planning, performed post-approval).** It applies **only** when detection finds a legacy root `design.md` and no `.ctide/design/design.md`. Detection happens in plan mode, so **planning reads the legacy file where it is and records that the migration is owed** — it moves nothing, because no working-tree write is allowed in plan mode (`SKILL.md`, *Plan Gate*). The move is a write, so it follows the same actor and timing as every other ctide migration: the **main thread performs it post-approval, before this run finishes** — unconditionally, even on a clean success — per the shared *One-time migration procedure* (`references/verification-gate.md`, whose step order and single-writer rule are unchanged here): source root `design.md` → destination `.ctide/design/design.md`, a single-file source with no siblings. Site deltas: git-tracked is the **normal** case for this committed artifact — still run the shared trackedness check (step 2) rather than assume; the tracked branch of the shared move-by-trackedness step (step 4) is a `git mv` whose result the project commits with the PR (destination directory created immediately before the move, per the shared per-path directory-creation step, step 3); disclosure e.g. "migrated design.md to .ctide/design/". When **both** locations exist, use `.ctide/design/design.md`, move or overwrite nothing, and disclose once that a legacy root `design.md` remains for manual cleanup/merge. After the migration, all `design.md` reads and writes use `.ctide/design/design.md` only — never the root location again; the planning read that triggered it is the one and only permitted read of the legacy path.

## Detect → Use → Else-Disclose

Follow `references/external-capabilities.md`.

1. **Detect** — does a `design.md` exist (`.ctide/design/design.md`, else the legacy root location — a legacy hit is read in place, recorded, and one-time-migrated post-approval, per *Where it lives*), and is the task UI / design-system / interaction scope? (`navigator` does this during planning, `references/plan-grounding.md` Stage A.)
...
4. **Write** *(post-approval implementation)* — write `design.md` to `.ctide/design/design.md`; the `implementer` does it post-approval so plan mode stays read-only.
...
When the repo has an existing UI but no `design.md`, `navigator` **detects the gap and recommends** establishing one (role split: *Invariants* below). Prefer a **separate, explicit bootstrap pass** over auto-bundling into an unrelated task, so the "establish the contract" decision (which needs deliberate sign-off) is kept distinct from the "follow the contract" work. The user chooses: do it within the current task, or as its own pass. Either way, detection is read-only in planning, the draft is blessed at `ExitPlanMode`, and the write is post-approval.

## Extraction guide (drafting descriptively from an existing UI)

The draft is produced **as a step, not by a new agent** (role split: *Invariants* below). When `ui-ux-pro-max` is absent the extraction still works — the token sources below are read directly (mechanical) and the prose falls back to the `ui-ux-reviewer` baseline — disclose that pro-max was not used. Never a hard dependency.
...
- **Interaction/Operation is the ctide half.** Spend the effort the community (visual-only) format skips: capture the navigation model, the empty/error/loading/success conventions, destructive-action confirmation, and keyboard/focus behavior from the real components — this is what governs "operation-mode" changes.
...
- **Google `design.md`** contributes the format concept (tokens + prose). Its `lint` / `diff` / `export` CLI is an **optional, user-wired check** (a project may add it as its own validation step) — never a ctide default or dependency.
...
- **Language.** Per `SKILL.md` *Language And Text Integrity* — `design.md` follows the repository's language; technical contracts verbatim.
