...
**Relationship to `design.md`.** `ui-ux-pro-max` is the *generation* layer (universal design intelligence); a project's `design.md` (`references/design-spec.md`) is the *consistency* layer (this project's own contract); the `ui-ux-reviewer` baseline is the inviolable *safety* floor beneath both. For an existing pattern, `design.md` governs (reuse it); for **net-new** design, consult `ui-ux-pro-max` and **record the decision back into `design.md`**, so the project's contract absorbs the new pattern and `ui-ux-pro-max` is consulted less over time. These three answer different questions and do not compete — see the three-layer arbitration in `references/design-spec.md`. When **bootstrapping** a `design.md` from an existing UI, an available `ui-ux-pro-max` is also the drafting aid the orchestrator borrows for structure/rationale; absent it, the extraction reads the real token sources directly and falls back to the `ui-ux-reviewer` baseline (disclosed) — never a hard dependency (`references/design-spec.md`, *Extraction guide*).
