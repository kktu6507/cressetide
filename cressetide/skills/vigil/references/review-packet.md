...
- Acceptance criteria (user-approved): a numbered checklist of observable, verifiable outcomes that define done — approved by the user as part of the plan at ExitPlanMode, not a separate approval step. The `arbiter` checks each item (met / unmet / deferred; see `agents/arbiter.agent.md`), and an unmet, non-deferred criterion blocks `READY`. On high-risk work these are the plan-grounding **sharpened contract**.
- Task contract pointer: when the task contract exists (`.ctide/output/contract.md`, or the legacy pre-migration path — `references/task-contract.md`), the packet's Acceptance criteria / In-Out scope / Verification evidence are **derived from it** (the single source), and the deterministic `scripts/contract-check.mjs` report is included as scope/AC evidence. Fill the packet fields from the contract; include its path; do not paste the whole file.
...
- Must-not-change (invariants): the behaviors, contracts, and semantics the change must leave intact (from the plan / the task contract's `mustNotChange`). Reviewers check the diff against this list; the `arbiter` treats a violated invariant as release-blocking. Write "none stated" when the plan declared none.
...
- Assumptions: only assumptions that affected implementation or verification. When the task contract carries an assumption register (`references/task-contract.md`), fill this field from it; assumption entries are first-class review targets — an implementation faithful to a wrong assumption is still an omission vs intent (the `intent-reviewer` lens).
- Implementation summary: what changed and why.
- Changed files: paths and short purpose for each relevant changed area.
- Changed diff (filtered): a filtered, capped unified diff produced once by the orchestrator and given to every reviewer as a shared starting point, so no reviewer re-runs the base diff. Produce it by piping the base diff through the packer — `git diff <base> -- <paths> | node ${CLAUDE_PLUGIN_ROOT}/skills/vigil/scripts/pack-review-diff.mjs` — which reorders hunks by language/size, down-ranks (never removes) deletion-only/whitespace-only hunks, and adds new-side line numbers; it strengthens focus without ever hiding content — a starting point, not a cap. State a **retrieval pointer** alongside it — what was trimmed (the cap, the path scope, excluded paths, any truncated hunks, any `--max-lines` trim) and the exact command to regenerate the *same-scoped* untrimmed diff — so a reviewer who needs more follows a pointer instead of reconstructing what was dropped or running a bare `git diff` that yields a differently-scoped view. When inlining a large diff into every reviewer's packet is wasteful, the orchestrator MAY instead write it once to `.ctide/output/review/diff.patch` and hand that **path** (plus the same retrieval pointer); small diffs stay inline, and on any write failure fall back to inline. Path-vs-inline is a token/cache optimization only (it slims the per-reviewer prompt and keeps the shared prefix byte-stable — `references/runtime-policy.md`, Context Ordering); it never changes what a reviewer may read. The file is a kept run artifact, gitignored (`references/verification-gate.md`, Artifact Hygiene). This applies only to the diff; do not also spin out a separate "task brief" file — the Review Packet itself is the brief.
- Verification evidence: a structured per-check table — one row per check with command, check type (build / test / typecheck / lint / …), required? (yes/no), ran? (yes/no), real exit status (0 / non-zero / —), and blocked-with-reason when not run — plus a one-line summary. The `arbiter` reads the real exit status as authority over reviewer prose (see `agents/arbiter.agent.md`, "Command-evidence authority"); a single rollup is surfaced as `ctide:verify=pass|fail|unrun|na`.
...
- Migration status: whether this run consulted a legacy-only `ai/FAILURE_MEMORY.md` (`references/verification-gate.md`, *Failure Memory*) and, if so, whether the one-time `git mv` to `.ctide/memory/FAILURE_MEMORY.md` was actually performed — `migrated` / `NOT migrated` / `n/a` (tier 1 already existed, or failure memory was not consulted at all this run). The `arbiter` checks and reports this field; when `NOT migrated`, it names the outstanding `git mv` as a required post-verdict action for the main thread (auto-remediate — it does not block `READY`).

On high-risk work where the plan-grounding step ran (`references/plan-grounding.md`), populate the Acceptance criteria and Reviewer scope from its **sharpened contract**, and fill the Must-not-change and Edge checklist fields from its contract + **implied edge checklist** (also seeding Verification evidence / expected tests). This contract-level intent is the dominant recall lever (see `references/reviewer-selection.md`, *Multi-lens review rationale*), so carry it into the packet rather than re-deriving a vaguer version.
...
Changed diff (filtered): <inline diff, or a path like `.ctide/output/review/diff.patch` for a large diff>
...
- Report findings as `blocker` / `major` / `minor`; do NOT emit a PASS/CONCERNS/BLOCK verdict — only `arbiter` issues `READY` / `FIX REQUIRED` / `NOT READY`.
...
- When the packet lists numbered acceptance criteria, evaluate coverage against each within your discipline and flag any not demonstrably met (the `arbiter` makes the final per-criterion ruling).
...
- Non-mutating: inspect, don't change. You hold `Read`/`Grep`/`Glob`/`Bash` but not the editor tools; use `Bash` for read-only inspection only — `git diff`/`git log`/`git show`, `rg`, and read-only build/test/lint runs to gather evidence. Do not modify the working tree, write or delete files, change git state, or run state-changing commands; propose the smallest safe fix — applying it is the `implementer`'s job ("read-only" is enforced by this instruction and the absence of editor tools, not by a sandbox). When a claim needs to be verified by mutating and observing a file (e.g. confirming a regression test actually goes red under a specific change), do this against a scratch/temp copy held outside the working tree under review, never the shared working tree in place — a confirmed, byte-exact restore afterward is not sufficient, because a parallel-running sibling reviewer, or the orchestrator's own concurrent verification, could observe the intermediate mutated state before it is restored. When inspecting, filter noise, not signal — run at minimal verbosity and pull only the decision-relevant output for the content type (diffs → the changed hunks; tests/builds → the failing assertion + first failing frame; logs → error lines + context; searches → `rg -l`/`-c` to locate, then pull context); never drop detail you need to judge correctness.
...
All reviewers report findings as `blocker` / `major` / `minor` (unified vocabulary). Only `arbiter` issues a `READY` / `FIX REQUIRED` / `NOT READY` verdict.

- `intent-reviewer`: requirement fidelity, business rules, contract behavior, hidden behavior-changing assumptions.
...
Note: Claude Code reinforces this structurally — each subagent runs in an isolated context window that does not inherit the main thread's history by default (`references/runtime-policy.md`, *Context Isolation*). Still give each reviewer a focused packet; do not paste raw history.
