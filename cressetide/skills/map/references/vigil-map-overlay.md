...
3. **Plan read-only.** Use `navigator` where available, which also disposes any flagged ledger-reconciliation candidates. Define numbered observable acceptance criteria, must-not-change scope, assumptions, affected files, risk, reviewer panel, checks, rollout, and rollback. No working-tree writes are allowed in plan mode.
