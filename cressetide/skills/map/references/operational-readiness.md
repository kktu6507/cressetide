...
Each individual entry in these three fields (every Access-inventory row, the Rollback line, every Feature-flags bullet) also ends with a machine-readable tag: `<!-- CTIDE:TRUST:<tier>[:<date>] -->`, e.g. `<!-- CTIDE:TRUST:verified:2026-07-01 -->` or `<!-- CTIDE:TRUST:unverified -->`. This invisible HTML comment is what `map verify` actually reads to check trust-marker validity — it never inspects the surrounding prose for this purpose. Fill it in alongside the human-readable text whenever you set or update a trust marker, so the machine check never has to guess how you chose to word or format the sentence around it. An entry that has genuinely been filled in but whose tag is left out, mis-cased, or buried behind other text inside the same comment is reported as a `missing trust tag` finding for that field — forgetting the invisible comment while updating the visible prose is never a silent pass.
...
2. **Read recent-change intel** from Run-Card scratch under `.ctide/output/` when present — recent dev-flow runs show what changed lately and where risk concentrated.
   - One-time legacy migration: if legacy `.ctide/legacy-output/` Run-Card scratch exists and `.ctide/output/` is absent, move the whole tree to `.ctide/output/`, each path to its matching location, per the shared *One-time migration procedure* (`../../vigil/references/verification-gate.md`): the shared trackedness check (step 2), the shared per-path directory-creation step (step 3) immediately before each move, then the shared move-by-trackedness step (step 4). Keep the directory self-gitignored, disclose the migration in one line, and never touch the legacy path again. When both trees exist, use `.ctide/output/`, move or overwrite nothing, and disclose once that the legacy copy remains for manual cleanup/merge. Apply the same pattern to any other legacy file this skill must read (`ai/FAILURE_MEMORY.md` → `.ctide/memory/FAILURE_MEMORY.md`, root `design.md` → `.ctide/design/design.md`).
