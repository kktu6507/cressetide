...
| Skill | 用途 | 細節 |
|---|---|---|
| `vigil` | dev flow。在非瑣碎的開發工作上自動啟用：計劃閘門式的實作 → 驗證 → 依風險挑選 reviewer → verdict。手動啟動：`/ctide:vigil`。 | 見「運作方式」一節 |
| `salvage` | incident flow。在正式環境事故的語言上自動啟用：先止血，再把修復交給 dev flow。手動：`/ctide:salvage`；操作準備由 `/ctide:map` 負責。 | 見「事故應變流程」一節 |
| `map` | 建立、刷新、驗證具來源依據的系統與操作就緒 Map（`/ctide:map`）；承接上游 preparation 契約。 | 見「快速開始」一節 |
| `doctor` | hooks + 環境的本機健康自檢（`/ctide:doctor`）；無 telemetry。 | 見「快速開始」一節 |
...
文件所列的相容佈局（`ai/FAILURE_MEMORY.md`、repo 根目錄的 `design.md`、`.ctide/legacy-output/`）會被一次性遷移：workflow 把每個檔案搬到現行位置、移除已遷移的副本，並在該次執行中揭露這個動作。
...
當「done」必須等於「可發佈」時使用 ctide：合併到 `main`、上線、或動到 authentication、data、contracts、migrations、production behavior、高風險 UI flow。

打錯字、純格式化、低風險小改、或只是 quick look 時通常不用 ctide。能用更便宜且確定性的工具時，先用那些工具。
...
- **健康檢查：** gate 沒擋、hook 沒反應、或 Node 可能不存在時，跑 `/ctide:doctor`。

## 好任務長什麼樣子

ctide 最吃任務品質。好的任務會交代 intent、acceptance criteria、不能改什麼、預期驗證，以及風險區域。
...
Linters 抓機械問題。Tests 驗證已知預期行為。Static analysis 抓已知 vulnerability patterns。ctide 判斷 AI-made change 是否滿足陳述意圖、是否 ready to ship。
...
| **6 · Fix** | 交給 dev flow：一次 `vigil --lite` run，以「事故 repro 轉綠」為主要 acceptance criterion。`--lite` 在真正的高風險訊號存在時仍保留一位直接相關的 safety reviewer——事故修復通常帶著這類訊號。 |
...
非目標，一句話：不做 paging/on-call、不做 status-page 自動化、不做 SLO 套件、不做完整 RBAC、不做 DFIR 級鑑識、不做多 repo 事故指揮（見「非目標」一節）。完整的階段契約在 skill 的 references（`cressetide/skills/salvage/references/`）：`wartime.md`、`reproduction-and-repair.md`、`reentry-and-closure.md`；Map 的操作就緒契約在 `cressetide/skills/map/references/operational-readiness.md`。

## 11 個 subagent

你無需手動挑選 reviewer；ctide 依**風險**組成面板——打錯字不啟用任何 reviewer，動到認證則納入 security reviewer。完整名單：

| Agent | 角色 | 何時加入 | 模型 |
|---|---|---|---|
| `navigator` | 用真實程式碼紮根計畫、草擬方法、預選面板、偵測/建議 `design.md`（可從既有 UI 立基）（唯讀；輔助計畫核准，絕不取代） | 高風險／正確性關鍵的規劃 | inherit |
...
- [`examples/ready-run.md`](examples/ready-run.md)（英文）- illustrative `READY` report shape。
- [`examples/fix-required-run.md`](examples/fix-required-run.md)（英文）- illustrative `FIX REQUIRED -> READY` repair-loop shape。
...
因為 ctide **沒有 telemetry**，real-world validation 以人工記錄為準。`EVIDENCE.md` 是唯一真實來源：
...
| `contract-guard.js` | `PreToolUse` | 在 Write/Edit/MultiEdit 會移除/放寬既有 contract 的 acceptance criterion、`mustNotChange` 項目、scope path，或調降 `risk`，或整段刪除 `design.md` section 之前先詢問。監看 `.ctide/output/contract.md` 加上舊版 `.ctide/legacy-output/contract.md`。也會在 Write/Edit/MultiEdit 對 `.claude/settings.json` 或 `.claude/settings.local.json` 的修改，會把下方四個 guard flag 中任一個從啟用翻轉為停用（以其有效、依優先順序解析後的值判斷）之前詢問——包含透過全新建立的 settings 檔案的情況。 |
...
這些 hooks 不會刪檔、不會改系統設定、不會改權限、不會開 subprocess、不會下載 code，也不會傳送 code/transcript。它們是 guardrails，不是 sandbox。詳見 [`SECURITY.md`](SECURITY.md) 與 [`ARCHITECTURE.md`](ARCHITECTURE.md)。hooks 也絕不遷移、寫入或刪除 ctide 的專案檔案——舊佈局的一次性遷移是由 workflow 本身執行，在你的 session 裡以看得見的 tool 動作完成。
...
| `contractGuard` | `contract-guard.js`——Write/Edit/MultiEdit 會弱化 `.ctide/output/contract.md`（或舊版 `.ctide/legacy-output/contract.md`）或刪除 `design.md` section 之前的詢問；也包含 Write/Edit/MultiEdit 對 `.claude/settings.json` / `.claude/settings.local.json` 的修改會關閉這四個 guard flag 中任一個之前的詢問 |
...
| `CTIDE_HOOK_DEBUG` | 設成 `1` 會讓每個 hook 都多印一行 debug trace（`/ctide:doctor` 與手動排除故障會用到） |
...
```text
/ctide:vigil 修好登入的 bug。修復迴圈卡住時可以用 Codex。
```
...
/ctide:vigil --report full 幫公開 API 加上 rate limiting。
```
...
incident flow 在關鍵處很省：戰時回合都很短（一次一張 decision card，不寫長文）、正式修復只花一次普通的 ctide run（`--lite`）、Map refresh 是有界的 repo 掃描。

需要省成本時用 `/ctide:vigil --lite`；需要最大審查深度時用 `--deep`；需要詳細 per-agent activity 與 cost 時用 `--report full`。在小型低/中風險變更上，自動的 **fast lane** 會更進一步：當執行證據已回答該 reviewer 的問題（每個 behavior-changing criterion 都有 red→green 測試、full required suite 全綠），`test-reviewer` 會被證據替代，並以 `ctide:panel=substituted:test-reviewer` 披露 — 同樣的證據、更少 agents；高風險 / deep run 永不適用。
